 ...
 *  $Id: wlc_cfp.c 807508 2022-01-24 04:59:14Z $
 */
#include <wlc_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
#define WLC_CFPMOD_ARG(str) \
	"CFPMODULE", str, wlc->pub->unit, flowid, scb_cfp, \
...
wlc_cfp_scb_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\tIV Len     :: %d\n", SCB_CFP_IV_LEN(scb_cfp));
	bcm_bprintf(b, "\tICV Len    :: %d\n", SCB_CFP_ICV_LEN(scb_cfp));
	bcm_bprintf(b, "\tTX cache ID:: %d\n", SCB_CFP_CACHE_GEN(scb_cfp));
	...
	bcm_bprintf(b, "\n");
	bcm_bprintf(b, "\tPS Q Pkts \t\t ::%d", wlc_apps_scb_txpktcnt(wlc, scb));
	/* Dump TCB info */
...
wlc_cfp_tcb_cache_invalidate(wlc_info_t *wlc, int16 flowid)
{
	...
	if (flowid == SCB_ALL_FLOWS) {
		WLC_CFP_TRACE(("%s : Invalidate ALL TCB flows\n", __FUNCTION__));
	...
	void *pkt, uint32 cfp_exptime)
{
	wlc_info_t *wlc;
	struct scb *scb;
#if (CFP_DEBUG_LEVEL & CFP_TX_TRACE_ENABLE)
	scb_cfp_t *scb_cfp; /* SCB CFP cubby */
	wlc_cfp_t	*cfp;		/* Module */
#endif /* CFP_DEBUG  */
	...
	WLC_CFP_TX_TRACE((WLC_CFPMOD_FMT "scb %p\n",
		WLC_CFPMOD_ARG("PKTPREPARE"), scb));
}
	...
	void *pktlist_head, void *pktlist_tail, uint16 pkt_count)
{
	...
	/* Release frames stuck in Common Q */
...
#if defined(DONGLEBUILD)
/* Loop through the subframes and confirm they are properly encrypted */
static bool
wlc_cfp_amsdu_validate_sf_encryption(wlc_info_t *wlc, struct scb *scb, void *p, uint16 fc)
{
	void * pkt;
	uint16 len = 0;
	...
	while (pkt != NULL) {
		...
		if (!wlc_amsdu_validate_sf_encryption(wlc, scb, pkt, len, fc)) {
			return FALSE;
		}
...
#if defined(STS_XFER_PHYRXS)
		/* Set this PhyRx Status as current message */
...
/* dump_flag_qqdx */
		...
		if (WLPKTTAG(p)->flags & WLF_HWAMSDU) {
			 ...
			 */
			if (!wlc_cfp_amsdu_validate_sf_encryption(wlc, scb, p, fc)) {
		...
		if (WLPKTTAG(p)->flags & WLF_HWAMSDU) {
			/* Strip MAC header, move to start of AMSDU body */
			PKTPULL(wlc->osh, p, body_offset);
			if (wlc_cfp_amsdu_deagg_hw(wlc->ami, p, &pkt_cnt, scb, fc)) {
			...
			if (ntoh16(eh->ether_type) == ETHER_TYPE_802_1X) {
				WL_ASSOC(("wl%d %s "MACF" received 802.1X M2/M4 Msg\n",
...
wlc_cfp_rxframe(wlc_info_t *wlc, void* p)
{
...
#if ((CFP_DEBUG_LEVEL & CFP_RX_TRACE_ENABLE) && defined(BCMDBG))
	...
	if (D11REV_LT(wlc->pub->corerev, 128)) {
		...
		if (flowid != ID16_INVALID) {
			...
			if (!scb) {
				/* d3lut might have stale entry even
				 * after the scb is deleted.
				 */
				 WL_INFORM(("wl%d [%s] scb deleted but d3lut exist for "MACF""
					"flowid %d\n", wlc->pub->unit, __FUNCTION__,
					ETHERP_TO_MACF(&h->a2), flowid));
				wlc_cfp_rx_sendup(wlc, p);
				return;
			}
			/* Extract cubby pointer */
...
#ifdef BCMDBG
	WLC_CFP_RX_TRACE(("pktclass %x filtermap %x \n ",
		pktclass, filtermap));
	WLC_CFP_RX_TRACE(("%s FC %x A1 %s A2 %s A3 %s \n",
		__FUNCTION__, h->fc,
	...
	WLC_CFP_RX_TRACE(("Total Enqueud pkts scb cfp %p cnt %d \n",
		scb_cfp, pktq_n_pkts_tot(SCB_CFP_RCB_CHAINEDQ(scb_cfp))));
