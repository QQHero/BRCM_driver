 ...
 *
 * $Id: wlc_musched.c 806006 2021-12-10 09:24:16Z $
 */
...
#define MUSCHED_HEMSCH_SIGBMCS(a)	((a & MUSCHED_HEMSCH_SIGBMCS_MASK) >> \
		MUSCHED_HEMSCH_SIGBMCS_SHIFT)
...
static int wlc_musched_write_aggx_params(wlc_muscheduler_info_t *musched);
...
BCMATTACHFN(wlc_muscheduler_attach)(wlc_info_t *wlc)
{
	...
	musched->dyn_sigbmcs = FALSE;
	wlc->dsmps_war = FALSE;
	musched->wfa20in80 = FALSE;
	musched->omutafwin = FALSE;
	musched->aggx = TRUE;
	musched->txdur_thresh_mu = MUSCHED_TXDUR_THRSH_MU_DFLT;
	musched->txdur_thresh_su = MUSCHED_TXDUR_THRSH_SU_DFLT;

	if ((D11REV_IS(wlc->pub->corerev, 130) && D11MINORREV_LT(wlc->pub->corerev_minor, 2)) ||
		D11REV_IS(wlc->pub->corerev, 131)) {
		/* Do not admit 160 MHz clients for DLOFDMA
		 * while operating in BW:160 for corerev 130.0/.1 and 131
		 */
		musched->mixbw = FALSE;
	} else {
		musched->mixbw = TRUE;
	...
	char *outbuf, int outlen)
{
	...
	if (!strncmp(params->keystr, "lowat", strlen("lowat"))) {
		...
		if (!HE_DLOMU_ENAB(musched->wlc->pub))
			return BCME_UNSUPPORTED;
	...
	} else if (!strncmp(params->keystr, "aggx", strlen("aggx"))) {
		bcm_bprintf(&bstr, "%d\n", musched->aggx);
	} else if (!strncmp(params->keystr, "mu_thresh", strlen("mu_thresh"))) {
		bcm_bprintf(&bstr, "mu_thresh %d us\n", musched->txdur_thresh_mu);
	} else if (!strncmp(params->keystr, "su_thresh", strlen("su_thresh"))) {
		bcm_bprintf(&bstr, "su_thresh %d us\n", musched->txdur_thresh_su);
	} else {
...
static int
wlc_musched_write_sigbmcs(wlc_muscheduler_info_t *musched)
{
#if defined(WL_PSMX)
	wlc_info_t *wlc;
	uint offset;
	...
	offset = MX_HEMSCH0_BLK(wlc);
	uval16 = wlc_read_shmx(wlc, offset);
	uval16 &= ~MUSCHED_HEMSCH_SIGBMCS_MASK;
	uval16 |= ((musched->dyn_sigbmcs << MUSCHED_HEMSCH_SIGBMCS_SHIFT)
		& MUSCHED_HEMSCH_SIGBMCS_MASK);
	wlc_write_shmx(wlc, offset, uval16);
#endif
	return BCME_OK;
}
...
static int
wlc_musched_write_aggx_params(wlc_muscheduler_info_t *musched)
{
#if defined(WL_PSMX)
	...
	wlc_write_shmx(wlc, MX_AGGX_SUTHRSH(wlc), musched->txdur_thresh_su);
	wlc_write_shmx(wlc, MX_AGGX_MUTHRSH(wlc), musched->txdur_thresh_mu);
	wlc_bmac_mhf(wlc->hw, MXHF0, MXHF0_AGGX, musched->aggx ? MXHF0_AGGX : 0, WLC_BAND_ALL);
#endif /* defined(WL_PSMX) */
	return BCME_OK;
}
...
wlc_musched_cmd_set_dispatch(wlc_muscheduler_info_t *musched, wl_musched_cmd_params_t *params)
{
	...
	if (!strncmp(params->keystr, "lowat", strlen("lowat"))) {
	...
	} else if (!strncmp(params->keystr, "sigbmcs", strlen("sigbmcs"))) {
		musched->dyn_sigbmcs = params->vals[0];
		wlc_musched_write_sigbmcs(musched);
	} else if (!strncmp(params->keystr, "dsmps_war", strlen("dsmps_war"))) {
		if (wlc->pub->up) {
			return BCME_NOTDOWN;
		}
		...
		if (wlc->pub->up) {
			if (wlc_mutx_is_hedlmmu_enab(wlc->mutx) || HE_DLOMU_ENAB(wlc->pub)) {
	...
	} else if (!strncmp(params->keystr, "omutafwin", strlen("omutafwin"))) {
		if (params->vals[0]) {
			uval16 = MXHF1_OMUTAFWIN;
			musched->omutafwin = TRUE;
		} else {
			uval16 = 0;
			musched->omutafwin = FALSE;
		}
		wlc_bmac_mhf(wlc->hw, MXHF1, MXHF1_OMUTAFWIN, uval16, WLC_BAND_ALL);
	} else if (!strncmp(params->keystr, "aggx", strlen("aggx"))) {
		if (params->vals[0]) {
			uval16 = MXHF0_AGGX;
			musched->aggx = TRUE;
		...
		wlc_bmac_mhf(wlc->hw, MXHF0, MXHF0_AGGX, uval16, WLC_BAND_ALL);
	} else if (!strncmp(params->keystr, "mu_thresh", strlen("mu_thresh"))) {
			musched->txdur_thresh_mu = params->vals[0];
			if (wlc->clk) {
				err = wlc_musched_write_aggx_params(musched);
			}
	} else if (!strncmp(params->keystr, "su_thresh", strlen("su_thresh"))) {
			musched->txdur_thresh_su = params->vals[0];
			if (wlc->clk) {
				err = wlc_musched_write_aggx_params(musched);
			}
	} else {
...
wlc_muscheduler_wlc_init(void *ctx)
{
	...
	wlc_musched_write_use_murts(musched);
	wlc_musched_write_sigbmcs(musched);
	wlc_musched_write_aggx_params(musched);
...
wlc_musched_dump_policy(wlc_muscheduler_info_t *musched, bcmstrbuf_t *b)
{
	...
	bcm_bprintf(b, "  ofdma_en: %d schidx: %d flag: 0x%04x dyn_sigbmcs: %d ",
		MUSCHED_DLOFDMA(musched), musched->dl_schidx, musched->flags, musched->dyn_sigbmcs);
	...
	bcm_bprintf(b, "           RU alloc mode: %s (%d), mu-rts: %d, dsmps_war: %d "
		...
		musched->mixbw);
	bcm_bprintf(b, "           dlofdma_users min %d max %d num %d omutafwin %d "
		"aggx %d su_thresh %d mu_thresh %d\n",
		musched->min_dlofdma_users[0], max_dlofdma_users, musched->num_dlofdma_users,
		musched->omutafwin, musched->aggx,
		musched->txdur_thresh_su, musched->txdur_thresh_mu);
	bcm_bprintf(b, "           maxn ");
	for (i = 0; i < D11_REV128_BW_SZ; i++) {
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (musched_scb && musched_scb->scb_ru_stats) {
			...
			bcm_bprintf(b, ""MACF" (aid 0x%x) is_dlofdma: %d is_dsmps: %d\n",
				...
				SCB_DLOFDMA_ADM(scb), wlc_stf_is_scb_dynamic_smps(wlc, scb));
...
wlc_musched_upd_ru_stats(wlc_muscheduler_info_t *musched, scb_t *scb, tx_status_t *txs)
{
	...
	ru = TX_STATUS128_HEOM_RUIDX(TX_STATUS_MACTXS_S4(txs));
	tx_cnt = TX_STATUS40_TXCNT_RT0(TX_STATUS_MACTXS_S3(txs));
	txsucc_cnt = TX_STATUS40_ACKCNT_RT0(TX_STATUS_MACTXS_S3(txs));
...
wlc_musched_dp_stats(wlc_muscheduler_info_t *musched, tx_status_t *txs, uint8 *count)
{
	...
	*count = TX_STATUS40_ACKCNT_RT0(TX_STATUS_MACTXS_S3(txs));
...
wlc_musched_scb_state_upd(void *ctx, scb_state_upd_data_t *notif_data)
{
	...
	if (D11REV_LT(wlc->pub->corerev, 129) || !HE_DLOMU_ENAB(wlc->pub) ||
	...
	if (wlc_txbf_is_mu_bfe(wlc->txbf, scb) &&
	   ((wlc_mutx_is_hedlmmu_enab(wlc->mutx) || HE_DLOMU_ENAB(wlc->pub)))) {
		return;
	}
	...
	if ((!WSEC_ENABLED(bsscfg->wsec) && !(oldstate & ASSOCIATED) &&
		 ...
		 */
		if (musched->num_dlofdma_users < wlc_txcfg_max_clients_get(wlc->txcfg, DLOFDMA)) {
			SCB_MUSCHED_SET_DLUL_ASSOC(wlc, musched_scb, TRUE);
...
wlc_scbmusched_enable_dlofdma(wlc_muscheduler_info_t *musched, scb_t* scb, bool enable)
{
	...
	wlc_taf_scb_state_update(wlc->taf_handle, scb, TAF_NO_SOURCE,
	                         TAF_PARAM(SCB_DLOFDMA_ADM(scb)), TAF_SCBSTATE_MU_DL_OFDMA);
#endif
...
wlc_musched_scb_isdlofdma_eligible(wlc_muscheduler_info_t *musched, scb_t* scb)
{
	...
	if (musched_scb && SCB_HE_CAP(scb) &&
		HE_DLOMU_ENAB(musched->wlc->pub) && !(musched->wlc->dsmps_war &&
...
{
	wlc_info_t *wlc = musched->wlc;
	if (D11REV_GE(wlc->pub->corerev, 129) && D11REV_LT(wlc->pub->corerev, 133)) {
...
wlc_musched_minru_update(wlc_muscheduler_info_t *musched, scb_t *scb, uint8 minru)
{
	...
	if (!HE_DLOMU_ENAB(musched->wlc->pub) || SCB_INTERNAL(scb) || !SCB_HE_CAP(scb))
		return BCME_UNSUPPORTED;
