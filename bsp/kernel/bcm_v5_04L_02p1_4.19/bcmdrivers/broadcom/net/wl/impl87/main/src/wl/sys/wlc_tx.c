...
extern unsigned int tdextend_jiffies;
#endif
#endif
...
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
#define XMESH_TYPE_HEART_BEAT                               0x000F  //广播
...
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
static bool td_xmesh_lowst_rate(wlc_info_t *wlc, void *p)
{
    struct ether_header *eh;
    uint8 *ip_body = NULL;
    struct bcmudp_hdr *udph = NULL;
    char xmesh_type = 0;
    uint16 src_port = 0;
    uint16 dst_port = 0;
    ...
    if (eh->ether_type == hton16(ETHER_TYPE_IP)) {
        ip_body= (uint8 *)eh + sizeof(struct ether_header);
        if (IPV4_PROT(ip_body) == IP_PROT_UDP) {
            udph = (struct bcmudp_hdr *)((uint8 *)ip_body + IPV4_HLEN(ip_body));
            xmesh_type = *((uint8 *)udph + UDP_HDR_LEN + 0xf);
            src_port = ntoh16(udph->src_port);
            dst_port = ntoh16(udph->dst_port);
            if (XMESH_PORT == src_port && XMESH_PORT == dst_port && XMESH_TYPE_HEART_BEAT == xmesh_type) {
                return TRUE;
            }
        }
    }
...
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
    if (td_xmesh_lowst_rate(wlc, (void *)sdu) == TRUE) {
        WLPKTTAG(sdu)->xmesh_low_rate = TRUE;
    }
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    /* 如果已经处理了组播转单播，这里应该是单播报文，不会走进来，wds包会在添加80211头部时
       将组播转换成单播，丢弃所有的backhaul bss的组播包，因为实际上组播报文
       会从 wds 等接口出去 */
    if (ETHER_ISMULTI(eh->ether_dhost) && wlc->pub->mcast_limit_enable) {
#if defined(CONFIG_UGW_BCM_DONGLE)
        if(wlc->pub->mcast_stats_timeout >= tdextend_jiffies) {
            wlc->pub->mcast_stats_timeout = tdextend_jiffies + 1;
            wlc->pub->mcast_pkts_len = 0;
        }
#else
        if (time_is_before_eq_jiffies(wlc->pub->mcast_stats_timeout)) {
            wlc->pub->mcast_stats_timeout = jiffies + HZ;
            wlc->pub->mcast_pkts_len = 0;
        }
#endif
        /* 这里不增加wds包长度原因是从wds接口出去的都是单播报文，避免误判
         * 但是如果整个band的组播速率达到阈值还是会进行丢包，因为实际达到
         * 阈值之后，包会被缓存发不出去，提前丢弃
         */
        if (!wds) {
            wlc->pub->mcast_pkts_len += PKTLEN(osh, sdu);
        }

        if (wlc->pub->mcast_pkts_len > wlc->pub->mcast_limit_thresh ||
           (!wds && ((MAP_ENAB(bsscfg) && bsscfg->map_attr & MAP_EXT_ATTR_BACKHAUL_BSS) && !wlc_backhaul_bss_assocscb_getcnt(wlc, bsscfg)))) {
            /* 复用TX_PKTDROP_RSN_WMF_DROP */
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
            /*xmesh的心跳包是广播包，该处不进行限制，直接放行*/
            static int heartbeat_counts = 0;
            heartbeat_counts++;
            goto out;
#endif
            toss_reason = TX_PKTDROP_RSN_WMF_DROP;
            wlc->pub->mcast_drop_cnt++;
            goto toss;
        }
    }
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
    ...
    const wlc_key_info_t *key_info, uint8 prio, uint16 *pushlen)
{
    ...
    *pushlen = (is_amsdu) ? offset : (offset + ETHER_HDR_LEN);
    ...
    if (a4) {
        ...
        if (bsscfg->ai_mesh_enable) {
            bcopy(eh->ether_shost, h->a2.octet, ETHER_ADDR_LEN);
        }
#endif
            ...
            switch (k) {
                case WLC_RLM_SPECIAL_RATE_RSPEC : {
                    if (RSPEC_ACTIVE(wlc->band->rspec_override)) {
                        ...
                        fixed_rate = TRUE;
#ifdef GAME_SPEEDUP
                    } else if (wlc->special_fixrate) {
                        rspec = wlc->special_fixrate;
                        fixed_rate = TRUE;
#endif
    ...
    ratespec_t rspec_override)
{
    ...
    if (SCB_PROXD(scb)) {
        ...
        special_rate_block = WLC_RLM_SPECIAL_RATE_RSPEC;
#ifdef GAME_SPEEDUP
    } else if (wlc->special_fixrate && (PKTPRIO(p) == wlc->special_traffic)) {
...
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
    if ((((parms.pkttag)->xmesh_low_rate) == TRUE) && (parms.type == FC_TYPE_DATA)) {
        rate_idx = WLC_RLM_SPECIAL_RATE_IDX;
        special_rate_block = WLC_RLM_SPECIAL_RATE_BASIC;
    }
#endif
    ...
    if (SCB_WME(scb) && parms.qos) {
        ...
        if (wme->edcf_txop[ac] &&
            !ETHER_ISMULTI(&parms.h->a1) && !(parms.pkttag->flags & WLF_8021X)
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
            && (FALSE == (parms.pkttag)->xmesh_low_rate)
#endif
            ) {
	...
	if(start_game_is_on){
        if((scb!=NULL) && (memcmp(&(start_sta_info_cur->ea), &(scb->ea), sizeof(struct ether_addr)) == 0)){
            ...
            musched_info_qq_cur->wlc_fifo_is_dlofdma = wlc_fifo_is_dlofdma_qq(wlc->fifo, scb, ac);
...
wlc_get_txh_info(wlc_info_t* wlc, void* p, wlc_txh_info_t* tx_info)
{
	...
	/* dump_flag_qqdx */
    if(FALSE&&start_game_is_on&&((CHSPEC_BAND(wlc->chanspec) == WL_CHANSPEC_BAND_5G))){
