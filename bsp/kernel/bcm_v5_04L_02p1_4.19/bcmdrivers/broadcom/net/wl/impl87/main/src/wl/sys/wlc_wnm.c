 ...
 *
 * $Id: wlc_wnm.c 810463 2022-04-07 12:15:53Z $
 */
...
#else
#endif /*defined(DONGLEBUILD) || (!defined(CONFIG_UGW_BCM_DONGLE))*/
...
static const size_t wnm_pref_score_num_entries = ARRAYSIZE(wnm_pref_score_entries);
...
typedef struct wnm_btm_disassoc_tmr {
    struct wnm_btm_disassoc_tmr *next;
    struct wl_timer *timer;
    wlc_bsscfg_t *bsscfg;
    uint32 timer_state;
    struct ether_addr sta_mac;
} wnm_btm_disassoc_tmr_t;
...
typedef struct {
    ...
    uint8        flags;
    wnm_btm_disassoc_tmr_t *disassoc_timer;
#endif /* WLWNM_AP */
} wnm_bsscfg_cubby_t;

#ifdef WLWNM_AP
static wnm_btm_disassoc_tmr_t * wlc_wnm_add_btm_disassoc_tmr(wlc_wnm_info_t *wnm,
    wlc_bsscfg_t *bsscfg, struct ether_addr *ea);
static wnm_btm_disassoc_tmr_t * wlc_wnm_get_btm_disassoc_tmr(wlc_wnm_info_t *wnm,
    wlc_bsscfg_t *bsscfg, struct ether_addr *ea);
static int wlc_wnm_del_btm_disassoc_tmr(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg,
    struct ether_addr *ea);
static int wlc_wnm_del_all_btm_disassoc_tmr(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg);
#endif /* WLWNM_AP */
...
#ifndef DONGLEBUILD
    wlc_hrt_to_t    *hrt_timer;    /* Delayed scheduler timer */
    uint8        use_hrt_timer_for_disassoc;
#endif /* DONGLEBUILD */
};
...
enum wlc_wnm_iov {
    ...
    IOV_WNM_USE_HRT_TIMER_FOR_BSS_DISASSOC = 39,
    IOV_LAST
};
...
#endif /* WLWNM_AP */
    {"wnm_use_hrt_timer", IOV_WNM_USE_HRT_TIMER_FOR_BSS_DISASSOC, IOVF_BSSCFG_AP_ONLY, 0,
    IOVT_UINT32, 0},
    {NULL, 0, 0, 0, 0, 0}
};
...
static int wlc_wnm_parse_tfs_notify_resp(wlc_info_t *wlc, struct scb *scb, uint8 *body,
    int body_len);
#ifdef WL11AX
static int wlc_wnm_parse_event_rep(wlc_info_t *wlc, struct scb *scb, uint8 *body, int body_len);
#endif /* WL11AX */
...
static int wlc_wnm_get_sleep_intv(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
    wlc_wnm_info_t *wnm, wnm_bsscfg_cubby_t *wnm_cfg, int *ret_int_val);
#ifdef WL11AX
static int wlc_wnm_parse_event_req(wlc_info_t *wlc, struct scb *scb, uint8 *body, int body_len);
#endif /* WL11AX */
...
#if defined(STA) && defined(WNM_BSSTRANS_EXT)
static void wnm_bsstrans_set_values(uint8 band, void *ptr_5g, void *ptr_2g, void *ptr_6g,
    void *data, uint8 size);
#endif /* STA && WNM_BSSTRANS_EXT */
...
#ifndef DONGLEBUILD
    wnm->hrt_timer = wlc_hrt_alloc_timeout(wlc->hrti);
#endif /* !DONGLEBUILD */
    return wnm;
...
#ifndef DONGLEBUILD
    if (wnm->hrt_timer) {
        wlc_hrt_free_timeout(wnm->hrt_timer);
    }
#endif
...
#ifndef DONGLEBUILD
    if (wnm->hrt_timer) {
        wlc_hrt_del_timeout(wlc->wnm_info->hrt_timer);
        /* reset timer state */
        FOREACH_AP(wlc, i, bsscfg) {
            wnm_cfg = WNM_BSSCFG_CUBBY(wnm, bsscfg);
            wl_del_timer(wlc->wl, wnm_cfg->bsstrans_req_info.timer);
            wnm_cfg->bsstrans_req_info.timer_state = BSSTRANS_INIT;
            wlc_wnm_del_all_btm_disassoc_tmr(wnm, bsscfg);
        }
    }
#endif /* DONGLEBUILD */
#endif /* WLWNM_AP */
    ...
    while (*body_len > 0) {
        ...
        if (pref_chspec_found && ngb->channel != 0) {
            ...
            reassoc->chanspec_list[0] = CH20MHZ_CHSPEC(update_channel,
                    WL_CHANNEL_2G5G_BAND(update_channel));
        ...
        chanspec_t *list, uint32 maxsize)
{
    ...
    while (nbr_rep) {
        if (nbr_rep->nbr_elt.channel != 0 && idx < maxsize) {
            ...
            chspec = CH20MHZ_CHSPEC(nbr_rep->nbr_elt.channel, band);
    ...
    while (*body_len > 0) {
        ...
        if (ngb->channel != 0 && *n_channels < PARTIAL_SCAN_MAX_CHANNELS) {
            ...
            list[*n_channels] = CH20MHZ_CHSPEC(ngb->channel,
                    WL_CHANNEL_2G5G_BAND(ngb->channel));
            *n_channels += 1;
        }
        ...
        uint8 **bufptr, struct scb *scb, uint8 reqmode)
{
    ...
    nbr_rep->len = DOT11_NEIGHBOR_REP_IE_FIXED_LEN;
    bcopy(&btq_nbr_elem->bssid, &nbr_rep->bssid, ETHER_ADDR_LEN);
    bcopy(&btq_nbr_elem->bssid_info, &nbr_rep->bssid_info, sizeof(nbr_rep->bssid_info));
    nbr_rep->reg = btq_nbr_elem->reg;
    nbr_rep->channel = btq_nbr_elem->channel;
    ...
    case DOT11_WNM_ACTION_BSSTRANS_QUERY: {
        ...
        if (CEVENT_STATE(wlc->pub)) {
            wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, transquery->reason,
                0, NULL, 0, CEVENT_D2C_MT_BTM_QUERY, CEVENT_FRAME_DIR_RX);
        }
#endif /* BCM_CEVENT */
...
#ifdef WL11AX
    case DOT11_WNM_ACTION_EVENT_REP:
        ret = wlc_wnm_parse_event_rep(wlc, scb, body, body_len);
        break;
#endif /* WL11AX */
#endif /* WLWNM_AP */
#ifdef STA
#ifdef WL11AX
    case DOT11_WNM_ACTION_EVENT_REQ:
        ret = wlc_wnm_parse_event_req(wlc, scb, body, body_len);
        break;
#endif /* WL11AX */
...
#if defined(STA) && defined(WNM_BSSTRANS_EXT)
static void
wnm_bsstrans_set_values(uint8 band, void *ptr_5g, void *ptr_2g, void *ptr_6g, void *data,
    uint8 size)
{
    if (!ptr_5g || !ptr_2g || !ptr_6g) {
        WL_ERROR(("null pointer\n"));
        return;
    }

    if ((band == BAND_5G_INDEX) || (band == MAXBANDS)) {
        memcpy(ptr_5g, data, size);
    }
    if ((band == BAND_2G_INDEX) || (band == MAXBANDS)) {
        memcpy(ptr_2g, data, size);
    }
    if ((band == BAND_6G_INDEX) || (band == MAXBANDS)) {
        memcpy(ptr_6g, data, size);
    }
}
    ...
    void *params, uint len, wnm_bsscfg_cubby_t *wnm_cfg)
{
    ...
    if (btcfg->band == WLC_BAND_2G) {
        bandunit = BAND_2G_INDEX;
    } else if (btcfg->band == WLC_BAND_5G) {
        bandunit = BAND_5G_INDEX;
    } else if (btcfg->band == WLC_BAND_6G) {
        bandunit = BAND_6G_INDEX;
    } else
        return BCME_RANGE;
    }
    ...
    if (btcfg->type == WNM_BSS_SELECT_TYPE_RSSI) {
        btcfg->count = wnm_cfg->bss_select_table[bandunit]->rssi_table_size;
        memcpy(btcfg->params, wnm_cfg->bss_select_table[bandunit]->rssi_table,
            btcfg->count * sizeof(wnm_bss_select_factor_cfg_t));
    }
    else {
        btcfg->count = wnm_cfg->bss_select_table[bandunit]->cu_table_size;
        memcpy(btcfg->params, wnm_cfg->bss_select_table[bandunit]->cu_table,
            btcfg->count * sizeof(wnm_bss_select_factor_cfg_t));
    }
    ...
    void *params, wnm_bsscfg_cubby_t *wnm_cfg)
{
    ...
    if (btcfg->band == WLC_BAND_2G) {
        ...
        bandunit = BAND_6G_INDEX;
    } else if (btcfg->band == WLC_BAND_AUTO) {
        bandunit = MAXBANDS;
    } else
        return BCME_RANGE;
    }
    ...
    /* update count */
    if (btcfg->type == WNM_BSS_SELECT_TYPE_RSSI) {
        ptr_5g = &wnm_cfg->bss_select_table[BAND_5G_INDEX]->rssi_table_size;
        ptr_2g = &wnm_cfg->bss_select_table[BAND_2G_INDEX]->rssi_table_size;
        ptr_6g = &wnm_cfg->bss_select_table[BAND_6G_INDEX]->rssi_table_size;
    } else {
        ptr_5g = &wnm_cfg->bss_select_table[BAND_5G_INDEX]->cu_table_size;
        ptr_2g = &wnm_cfg->bss_select_table[BAND_2G_INDEX]->cu_table_size;
        ptr_6g = &wnm_cfg->bss_select_table[BAND_6G_INDEX]->cu_table_size;
    }

    wnm_bsstrans_set_values(bandunit, ptr_5g, ptr_2g, ptr_6g, &btcfg->count,
            sizeof(btcfg->count));

    /* update table */
    if (btcfg->type == WNM_BSS_SELECT_TYPE_RSSI) {
        ptr_5g = wnm_cfg->bss_select_table[BAND_5G_INDEX]->rssi_table;
        ptr_2g = wnm_cfg->bss_select_table[BAND_2G_INDEX]->rssi_table;
        ptr_6g = wnm_cfg->bss_select_table[BAND_6G_INDEX]->rssi_table;
    }
    else {
        ptr_5g = wnm_cfg->bss_select_table[BAND_5G_INDEX]->cu_table;
        ptr_2g = wnm_cfg->bss_select_table[BAND_2G_INDEX]->cu_table;
        ptr_6g = wnm_cfg->bss_select_table[BAND_6G_INDEX]->cu_table;
    }

    wnm_bsstrans_set_values(bandunit, ptr_5g, ptr_2g, ptr_6g, btcfg->params,
    ...
    void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
#ifndef DONGLEBUILD
    case IOV_GVAL(IOV_WNM_USE_HRT_TIMER_FOR_BSS_DISASSOC): {
        *ret_int_ptr = wnm->use_hrt_timer_for_disassoc;
        break;
    }
    case IOV_SVAL(IOV_WNM_USE_HRT_TIMER_FOR_BSS_DISASSOC): {
        wnm->use_hrt_timer_for_disassoc = (bool)int_val;
        break;
    }
#endif /* !DONGLEBUILD */
    case IOV_SVAL(IOV_WNM_BSSTRANS_REQ): {
        ...
        if (!ETHER_ISNULLADDR(&wl_bsstrans_req->sta_mac)) {
...
#ifdef BCMDBG
                WL_ERROR(("wl%d: %s: STA "MACF" is not  BSS transition capable\n",
                    wlc->pub->unit, __FUNCTION__,
            ...
            if (bsstrans->reqmode & DOT11_BSSTRANS_REQMODE_DISASSOC_IMMINENT) {
                ...
                timer = wlc_wnm_get_btm_disassoc_tmr(wnm, bsscfg,
                    &wl_bsstrans_req->sta_mac);

                if (timer == NULL) {
                    timer = wlc_wnm_add_btm_disassoc_tmr(wnm, bsscfg,
                        &wl_bsstrans_req->sta_mac);
                } else if (timer->timer_state != BSSTRANS_INIT) {
                    WL_WNM(("wl%d.%d Disassoc timer is already running "
                        "for STA "MACF" Free timer and set new\n",
                        wlc->pub->unit,    WLC_BSSCFG_IDX(bsscfg),
                        ETHER_TO_MACF(wl_bsstrans_req->sta_mac)));
                    wl_free_timer(wlc->wl, timer->timer);
                    timer->timer = NULL;
                    /* Initialize timer */
                    if ((timer->timer = wl_init_timer(wlc->wl,
                            wlc_wnm_bsstrans_disassoc_timer, timer,
                            "wnm_bsstrans_disassoc_timer")) == NULL) {
                        WL_ERROR(("wl%d.%d: %s: wl_init_timer "
                            "for bsstrans_disassoc_timer failed\n",
                            wlc->pub->unit,    WLC_BSSCFG_IDX(bsscfg),
                            __FUNCTION__));
                        wlc_wnm_del_btm_disassoc_tmr(wnm, bsscfg,
                            &wl_bsstrans_req->sta_mac);
                        return BCME_EPERM;
                    }
                }
                ...
                if (timer && timer->timer) {
                    WL_WNM(("wl%d.%d: "MACF" BTM disassoc timeout[%d ms] \n",
                        wlc->pub->unit,    WLC_BSSCFG_IDX(bsscfg),
                        ETHER_TO_MACF(wl_bsstrans_req->sta_mac), msec));
                    /* Schedule the timer to wait for the resp frame */
                    wl_add_timer(wlc->wl, timer->timer, msec, 0);
                    timer->timer_state = BSSTRANS_WAIT_FOR_DISASSOC;
                }
...
#else
            if (wnm->use_hrt_timer_for_disassoc) {
                wlc_hrt_add_timeout(wnm->hrt_timer, (msec * MSEC_TO_USEC),
                    wlc_wnm_bsstrans_req_timer, bsscfg);
            } else {
                wl_add_timer(wlc->wl, bsstrans->timer, msec, 0);
            }
#endif /* DONGLEBUILD */
    ...
    case IOV_SVAL(IOV_WNM_BSSTRANS_QUERY):    {
        ...
        if (btm_query->ssid.SSID_len != 0) {
            ...
            wlc_scan_request(wlc, DOT11_BSSTYPE_INFRASTRUCTURE, &ether_bcast, 1, &ssid,
                0, NULL, DOT11_SCANTYPE_ACTIVE, -1, -1, -1, -1, NULL, 0, 0, FALSE,
    ...
    case IOV_GVAL(IOV_WNM_BSS_SELECT_WEIGHT): {
        ...
        if (bwcfg->band == WLC_BAND_2G) {
            bandunit = BAND_2G_INDEX;
        } else if (bwcfg->band == WLC_BAND_5G) {
            bandunit = BAND_5G_INDEX;
        } else if (bwcfg->band == WLC_BAND_6G) {
            bandunit = BAND_6G_INDEX;
        } else
            return BCME_RANGE;
        }
        ...
        if (bwcfg->type == WNM_BSS_SELECT_TYPE_RSSI) {
            bwcfg->weight = wnm_cfg->bss_select_table[bandunit]->rssi_weightage;
        }
        else {
            bwcfg->weight = wnm_cfg->bss_select_table[bandunit]->cu_weightage;
        }
        break;
    }
    case IOV_SVAL(IOV_WNM_BSS_SELECT_WEIGHT): {
        ...
        if (bwcfg->band == WLC_BAND_2G) {
            ...
            bandunit = BAND_6G_INDEX;
        } else if (bwcfg->band == WLC_BAND_AUTO) {
            bandunit = MAXBANDS;
        } else
            return BCME_RANGE;
        }
        if (bwcfg->type == WNM_BSS_SELECT_TYPE_RSSI) {
            ptr_5g = &wnm_cfg->bss_select_table[BAND_5G_INDEX]->rssi_weightage;
            ptr_2g = &wnm_cfg->bss_select_table[BAND_2G_INDEX]->rssi_weightage;
            ptr_6g = &wnm_cfg->bss_select_table[BAND_6G_INDEX]->rssi_weightage;
        }
        else {
            ptr_5g = &wnm_cfg->bss_select_table[BAND_5G_INDEX]->cu_weightage;
            ptr_2g = &wnm_cfg->bss_select_table[BAND_2G_INDEX]->cu_weightage;
            ptr_6g = &wnm_cfg->bss_select_table[BAND_6G_INDEX]->cu_weightage;
        }
        wnm_bsstrans_set_values(bandunit, ptr_5g, ptr_2g, ptr_6g, &bwcfg->weight,
                sizeof(bwcfg->weight));
        break;
    }
#endif /* WNM_BSSTRANS_EXT */
...
BCMRAMFN(wnm_wbtext_bsscfg_bsi_init)(wlc_bsscfg_t *cfg)
{
    ...
    for (i = 0; i < MAXBANDS; i++) {
        ...
        wnm_cfg->bss_select_table[i]->rssi_table_size = ARRAYSIZE(rssi_table);
        memcpy(wnm_cfg->bss_select_table[i]->rssi_table, rssi_table, sizeof(rssi_table));
        wnm_cfg->bss_select_table[i]->cu_table_size = ARRAYSIZE(cu_table);
...
wlc_wnm_bsscfg_deinit(void *context, wlc_bsscfg_t *cfg)
{
    ...
    if (wnm_cfg->disassoc_timer) {
        wlc_wnm_del_all_btm_disassoc_tmr(wnm, cfg);
    }
    wlc_wnm_timbc_clrframe(wlc, wnm_cfg);
...
#ifdef WLWNM_AP
    if (BSSCFG_AP(bsscfg) && WNM_BSSTRANS_ENABLED(wnm_cfg->cap)) {
        wlc_wnm_del_btm_disassoc_tmr(wnm, bsscfg, &scb->ea);
    }
#endif /* WLWNM_AP */
}
...
#ifdef WL11AX
/** AP specific functionality. Event handling, only event supported at the moment is the 11ax
 *  BSS color collision event. This routine will extract the color collision event data and pass
 *  it on to the color collision handling code.
 */
static int
wlc_wnm_parse_event_rep(wlc_info_t *wlc, struct scb *scb, uint8 *body, int body_len)
{
    dot11_event_rep_t *event_rep = (dot11_event_rep_t *)body;
    dot11_event_rep_element_t *rep_element;
    uint8 *colors_ptr;

    if (event_rep->dialog_token != 0) {
        WL_WNM(("%s: wrong dialog token %d\n", __FUNCTION__, event_rep->dialog_token));
        return BCME_ERROR;
    }
    rep_element = (dot11_event_rep_element_t *)(event_rep + 1);

    if (rep_element->id != DOT11_MNG_EVENT_REP_ID) {
        WL_WNM(("%s: incorrect element id %d\n", __FUNCTION__, rep_element->id));
        return BCME_ERROR;
    }
    if (rep_element->event_type != DOT11_EVENT_TYPE_BSS_COLOR_COLLISION) {
        WL_WNM(("%s: unhandled event type %d\n", __FUNCTION__, rep_element->event_type));
        return BCME_OK;
    }
    if (rep_element->event_report_status != DOT11_EVENT_REPORT_STATUS_SUCCESS) {
        WL_WNM(("%s: unhandled status %d\n", __FUNCTION__,
            rep_element->event_report_status));
        return BCME_OK;
    }
    if (rep_element->event_token != 0) {
        WL_WNM(("%s: incorrect event token %d\n", __FUNCTION__, rep_element->event_token));
        return BCME_ERROR;
    }
    if (rep_element->len < (DOT11_EVENT_REP_ELEMENT_LEN - TLV_HDR_LEN +
        DOT11_EVENT_REPORT_TSF + DOT11_EVENT_COLOR_COLLISION_REPORT_LEN)) {
        WL_WNM(("%s: invalid length field %d\n", __FUNCTION__, rep_element->len));
        return BCME_ERROR;
    }

    colors_ptr = (uint8 *)(rep_element + 1);
    /* now skip the tsf */
    colors_ptr += DOT11_EVENT_REPORT_TSF;

    wlc_he_color_collision_process_report(wlc->hei, colors_ptr);
    ...
    while (body < body_end) {
        ...
        if (dms_desc == NULL) {
            ...
            if (dms_resp_st->type == DOT11_DMS_RESP_TYPE_ACCEPT) {
                ...
                scb = wlc_scbfind(wlc, bsscfg, &bsscfg->BSSID);
                wlc_sendmgmt(wlc, p, wlc->primary_bsscfg->wlcif->qi, scb);
...
#ifndef DONGLEBUILD
    if (wlc->wnm_info->use_hrt_timer_for_disassoc) {
        /* stop the timer */
        wlc_hrt_del_timeout(wlc->wnm_info->hrt_timer);
    }
#endif /* DONGLEBUILD */
...
{
    wnm_btm_disassoc_tmr_t *timer = (wnm_btm_disassoc_tmr_t *)arg;
    wlc_bsscfg_t *bsscfg = timer->bsscfg;
    wlc_info_t *wlc = bsscfg->wlc;
    struct scb *scb;

    if (!(scb = wlc_scbfind(wlc, bsscfg, &timer->sta_mac)) ||
        (!SCB_ASSOCIATED(scb))) {
        timer->timer_state = BSSTRANS_INIT;
        wlc_wnm_del_btm_disassoc_tmr(wlc->wnm_info, bsscfg, &timer->sta_mac);
        return;
    }
    wlc_senddisassoc(wlc, bsscfg, scb, &scb->ea, &bsscfg->BSSID,
            &bsscfg->cur_etheraddr, DOT11_RC_BUSY);
    ...
    uint8 token, uint8 reqmode, uint8 retry_delay, uint8 transition_reason, bool add_self)
{
    ...
    if (CEVENT_STATE(wlc->pub)) {
        wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, transition_reason, 0, NULL, 0,
            CEVENT_D2C_MT_BTM_REQ, CEVENT_FRAME_DIR_TX);
    }
#endif /* BCM_CEVENT */
    ...
    uint8 *body, int body_len)
{
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    maps_info("wl%d recv BTM response from %pM with status %d.", wnm->wlc->pub->unit, &scb->ea, bsstrans_resp->status);
    TDCORE_FUNC_IF(maps_bss_tran_res_handler) {
        TDCORE_FUNC(maps_bss_tran_res_handler)(scb, bsscfg, bsstrans_resp->status);
    }
#endif

#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
    td_em_sta_tran_res_handler(scb->ea.octet, 
        bsscfg, bsstrans_resp->status, bsstrans_resp->data);
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    if(!TDCORE_FUNC_IS_NULL(td_em_sta_tran_res_handler)) {
        TDCORE_FUNC(td_em_sta_tran_res_handler)(scb->ea.octet, bsscfg, bsstrans_resp->status, bsstrans_resp->data);
    }
#endif
#endif
    if (!SCB_BSSTRANS(wnm_scb->cap)) {
...
#ifdef CONFIG_TENDA_PRIVATE_KM
#ifdef CONFIG_UGW_BCM_DONGLE
    wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_BSS_TRANS_RES, 0, 0, &bsstrans_resp->status, sizeof(bsstrans_resp->status));
#else
    if (km_eventscenter_wifiev_bss_trans_res_handler) {
        km_eventscenter_wifiev_bss_trans_res_handler(scb->ea.octet, bsstrans_resp->status);
    }
#endif
#endif
    ...
    if (bsstrans->timer_state == BSSTRANS_WAIT_FOR_BSS_TERM &&
         ...
         */
        WL_WNM(("wl%d: %s: bss transition resp token 0x%02x, status 0x%02x from "MACF"\n",
            wnm->wlc->pub->unit, __FUNCTION__, bsstrans_resp->token,
            bsstrans_resp->status, ETHER_TO_MACF(scb->ea)));
    ...
    if (CEVENT_STATE(wlc->pub)) {
        wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, bsstrans_resp->status, 0, NULL, 0,
            CEVENT_D2C_MT_BTM_RESP, CEVENT_FRAME_DIR_RX);
    }
#endif /* BCM_CEVENT */
    ...
    uint8 *reply_to_bss, bool istx)
{
...
#ifdef BCMDBG
        WL_ERROR(("wl%d.%d: Invalid Ether addr("MACF")\n", wlc->pub->unit,
            WLC_BSSCFG_IDX(bsscfg),
            ETHERP_TO_MACF(arp->src_eth)));
#endif /* BCMDBG */
        return FRAME_NOP;
    }
    ...
    if (!IPV4_ADDR_NULL(arp->src_ip) && !IPV4_ADDR_BCAST(arp->src_ip)) {
        ...
        if (entry == NULL) {
#ifdef BCMDBG
            WL_ERROR(("wl%d.%d: Add parp_entry by ARP "MACF" %s\n",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                ETHERP_TO_MACF(arp->src_eth),
    ...
    /* only learning ARP-Probe(DAD) in receiving path */
        ...
        if (!istx && op == ARP_OPC_REQUEST) {
            ...
            if (entry == NULL) {
#ifdef BCMDBG
                WL_ERROR(("wl%d.%d: create candidate parp_entry by ARP "MACF
                    " %s\n", wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                    ETHERP_TO_MACF(arp->src_eth),
...
#ifdef BCMDBG
            WL_ERROR(("wl%d.%d: withdraw candidate parp_entry IPv4 "MACF" %s\n",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                ETHER_TO_MACF(ea),
                bcm_ip_ntoa((struct ipv4_addr *)arp->src_ip, ipbuf)));
#endif /* BCMDBG */
...
wnm_proxyarp_dhcp4_handle(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, frame_proto_t *fp, bool istx)
{
    ...
    if (istx) {
...
#ifdef BCMDBG
            WL_ERROR(("wl%d.%d: Unmatch DHCP Req Client MAC ("MACF")",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                ETHERP_TO_MACF(&cmac_addr)));
            WL_ERROR(("to DHCP Ack Client MAC("MACF")\n",
                ETHERP_TO_MACF(&dhcp[DHCP_CHADDR_OFFSET])));
#endif /* BCMDBG */
            return FRAME_NOP;
        }
        ...
        if ((scb = wlc_scbfind(wlc, bsscfg, (struct ether_addr*)smac_addr))
            ...
            if (entry == NULL) {
#ifdef BCMDBG
                WL_ERROR(("wl%d.%d: Add parp_entry by DHCP4 "MACF" %s\n",
                    wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                    ETHER_TO_MACF(scb->ea),
        ...
        if (ETHER_ISNULLADDR(&dhcp[DHCP_CHADDR_OFFSET]) ||
            ...
            WL_WNM(("wl%d.%d: Invalid Ether addr("MACF")",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                ETHERP_TO_MACF(eh->ether_shost)));
            WL_WNM(("("MACF") of DHCP Req pkt\n",
                ETHERP_TO_MACF(&dhcp[DHCP_CHADDR_OFFSET])));
            return FRAME_NOP;
        }
        /*
    ...
    uint8 *reply_to_bss, bool istx)
{
    ...
    if (ETHER_ISNULLADDR(eh->ether_shost) || ETHER_ISBCAST(eh->ether_shost) ||
        ETHER_ISMULTI(eh->ether_shost)) {
#ifdef BCMDBG
        WL_ERROR(("wl%d.%d: Invalid Ether addr("MACF") of icmp6 pkt\n", wlc->pub->unit,
            WLC_BSSCFG_IDX(bsscfg),
            ETHERP_TO_MACF(eh->ether_shost)));
#endif /* BCMDBG */
        return FRAME_NOP;
    ...
    if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA ||
        ...
        if (entry == NULL) {
#ifdef BCMDBG
            WL_ERROR(("wl%d.%d: Add new parp_entry by ICMP6 "MACF" %s\n",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                ETHERP_TO_MACF(entry_ea),
    ...
    if (nd_msg->icmph.icmp6_type == ICMPV6_PKT_TYPE_NA) {
        ...
        if (entry) {
            ...
            bcopy(&entry->ea, &ea, ETHER_ADDR_LEN);
#ifdef BCMDBG
            WL_ERROR(("wl%d.%d: withdraw candidate parp_entry IPv6 "MACF" %s\n",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
                ETHER_TO_MACF(ea),
                bcm_ipv6_ntoa((void *)entry_ip, ipbuf)));
#endif /* BCMDBG */
...
#ifdef STA
#ifdef WL11AX
/** STA specific functionality. Event request handling. This is not supported in any way, but as
 *  the device support autonomous event color collision report, the extended capabilities
 *  indicate event support. As a result the event request has to be parsed and proper response is
 *  DOT11_EVENT_REPORT_STATUS_INCAPABLE. Not all fields have to be parsed, just the tokens.
 */
static int
wlc_wnm_parse_event_req(wlc_info_t *wlc, struct scb *scb, uint8 *body, int body_len)
{
    wlc_bsscfg_t *bsscfg = scb->bsscfg;
    int maxlen;
    void *p;
    uint8 *pbody;
    dot11_event_rep_t *rep;
    dot11_event_rep_element_t *rep_element;
    dot11_event_req_t *req;
    dot11_event_req_element_t *req_element;

    maxlen = DOT11_EVENT_REP_LEN + DOT11_EVENT_REP_ELEMENT_LEN;
    ...
    /* Prepare Request incapable response frame */
    rep = (dot11_event_rep_t *)pbody;
    rep->category = DOT11_ACTION_CAT_WNM;
    rep->action = DOT11_WNM_ACTION_EVENT_REP;

    req = (dot11_event_req_t *)body;
    rep->dialog_token = req->dialog_token;

    rep_element = (dot11_event_rep_element_t *)(rep + 1);

    rep_element->id = DOT11_MNG_EVENT_REP_ID;
    rep_element->len = maxlen - DOT11_EVENT_REP_LEN - TLV_HDR_LEN;

    req_element = (dot11_event_req_element_t *)(req + 1);
    rep_element->event_token = req_element->event_token;
    rep_element->event_type = req_element->event_type;
    rep_element->event_report_status = DOT11_EVENT_REPORT_STATUS_INCAPABLE;
...
/** STA specific functionality. Send Color Collision Event Report */
int
wlc_wnm_send_color_collision_frame(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, scb_t *scb,
    uint8 colors[DOT11_EVENT_COLOR_COLLISION_REPORT_LEN])
{
    ...
    rep->category = DOT11_ACTION_CAT_WNM;
    rep->action = DOT11_WNM_ACTION_EVENT_REP;
    /* If the Event Report frame is being transmitted other than in response to an
     * Event Request frame, then the dialog token is 0.
     */
    rep->dialog_token = 0;
    ...
    rep_element->id = DOT11_MNG_EVENT_REP_ID;
    rep_element->len = maxlen - DOT11_EVENT_REP_LEN - TLV_HDR_LEN;
    /* If the Event Report element is being sent autonomously then the Event Token is 0. */
    rep_element->event_token = 0;
    rep_element->event_type = DOT11_EVENT_TYPE_BSS_COLOR_COLLISION;
    rep_element->event_report_status = DOT11_EVENT_REPORT_STATUS_SUCCESS;

    /* now TSF, then the color collision report */
    wlc_read_tsf(wlc, &tsf_l, &tsf_h);
    tsf = (uint8 *)(rep_element + 1);
    htol32_ua_store(tsf_l, &tsf[0]);
    htol32_ua_store(tsf_h, &tsf[4]);

    /* Copy the colors */
    colors_ptr = tsf + DOT11_EVENT_REPORT_TSF;
    bcopy(&colors[0], colors_ptr, DOT11_EVENT_COLOR_COLLISION_REPORT_LEN);
    ...
    if (ignore_conditions) {
        if (ignore_conditions & PM_IGNORE_BCMC_PROXY_ARP) {
            ...
            if (!scb_ap) {
                ...
                wnm_scb = SCB_WNM_CUBBY(wnm, scb_ap);
                ASSERT(wnm_scb);
                if (!SCB_PROXYARP(wnm_scb->cap)) {
                    ignore_bcmc = FALSE;
                }
            }
        }
    ...
    uint8 band, uint8 bcn_rssi)
{
    ...
    int idx;
    wnm_cfg = WNM_BSSCFG_CUBBY(wnm, bsscfg);
    bss_select_table = wnm_cfg->bss_select_table[band];
    ...
    uint8 band, uint8 qbss_cu)
{
    ...
    wnm_cfg = WNM_BSSCFG_CUBBY(wnm, bsscfg);
    bss_select_table = (wnm_bss_select_table_t *)wnm_cfg->bss_select_table[band];
...
{
    uint32 score;
    uint8 bandunit = CHSPEC_BANDUNIT(bsscfg->current_bss->chanspec);

    score = wlc_wnm_bsstrans_get_rssi_score(wnm, bsscfg, bandunit, WNM_DEFAULT_RSSI) +
        wlc_wnm_bsstrans_get_cu_score(wnm, bsscfg, bandunit, WNM_DEFAULT_CU);
    return (score);
}
    ...
    int8 bcn_rssi, uint32 *score)
{
    ...
    band = CHSPEC_BANDUNIT(bi->chanspec);
...
#ifdef WL11AX
    if (bsscfg->current_bss->flags3 & WLC_BSS3_HE) {
        nbrrep->phytype = DOT11_PHYTYPE_HE;
    } else
#endif /* WL11AX */
#ifdef WL11AC
    if (bsscfg->current_bss->flags2 & WLC_BSS2_VHT) {
        nbrrep->phytype = DOT11_PHYTYPE_VHT;
    } else
#endif /* WL11AC */
    if (bsscfg->current_bss->flags & WLC_BSS_HT) {
        nbrrep->phytype = DOT11_PHYTYPE_HT;
    } else
    {
        nbrrep->phytype = 0;
    }
...
#ifdef WLWNM_AP
static wnm_btm_disassoc_tmr_t *
wlc_wnm_get_btm_disassoc_tmr(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg, struct ether_addr *ea)
{
    wnm_btm_disassoc_tmr_t *timer;
    wnm_bsscfg_cubby_t *wnm_cfg;
    ...
    /* Find the timer for this STA */
    timer = wnm_cfg->disassoc_timer;
    while (timer) {
        if (memcmp(&timer->sta_mac, ea, ETHER_ADDR_LEN) == 0)
            break;
        timer = timer->next;
    }
    return timer;
...
static wnm_btm_disassoc_tmr_t *
wlc_wnm_add_btm_disassoc_tmr(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg, struct ether_addr *ea)
{
    wnm_btm_disassoc_tmr_t *timer;
    wlc_info_t *wlc;
    wnm_bsscfg_cubby_t *wnm_cfg;
    ...
    /* Find the disassoc timer from list */
    timer = wlc_wnm_get_btm_disassoc_tmr(wnm, bsscfg, ea);

    if (timer) {
        return timer;
    }

    /* Allocate memory for disassoc timer */
    timer = (wnm_btm_disassoc_tmr_t *)MALLOC(wlc->osh, sizeof(wnm_btm_disassoc_tmr_t));
    if (timer == NULL) {
        WL_ERROR(("wl%d.%d: %s: MALLOC(%d) failed, malloced %d bytes\n",
            WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
            (int)sizeof(wnm_btm_disassoc_tmr_t), MALLOCED(wlc->osh)));
        return NULL;
    }

    /* Initialize timer */
    if ((timer->timer = wl_init_timer(wlc->wl,
        wlc_wnm_bsstrans_disassoc_timer, timer, "wnm_bsstrans_disassoc_timer")) == NULL) {
        WL_ERROR(("wl%d.%d: %s: wl_init_timer for bsstrans_disassoc_timer failed\n",
            wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
        MFREE(wnm->wlc->osh, timer, sizeof(wnm_btm_disassoc_tmr_t));
        return NULL;
    }

    WL_WNM(("wl%d.%d: Adding BTM disassoc timer for "MACF"\n", wlc->pub->unit,
        WLC_BSSCFG_IDX(bsscfg), ETHERP_TO_MACF(ea)));
    bcopy(ea, &timer->sta_mac, ETHER_ADDR_LEN);
    timer->bsscfg = bsscfg;
    timer->timer_state = BSSTRANS_INIT;
    timer->next = wnm_cfg->disassoc_timer;
    wnm_cfg->disassoc_timer = timer;

    return timer;
}

static int
wlc_wnm_del_btm_disassoc_tmr(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg,
    struct ether_addr *ea)
{
    wnm_btm_disassoc_tmr_t *timer = NULL;
    wnm_btm_disassoc_tmr_t *timer_prev;
    wnm_bsscfg_cubby_t *wnm_cfg;
    int ret;

    wnm_cfg = WNM_BSSCFG_CUBBY(wnm, bsscfg);
    timer = timer_prev = wnm_cfg->disassoc_timer;

    while (timer) {
        if (memcmp(&timer->sta_mac, ea, ETHER_ADDR_LEN) == 0) {
            if (timer == wnm_cfg->disassoc_timer) {
                wnm_cfg->disassoc_timer = timer->next;
            } else {
    ...
    if (timer) {
        WL_WNM(("wl%d.%d Deleting BTM disassoc timer for "MACF"\n", wnm->wlc->pub->unit,
            WLC_BSSCFG_IDX(bsscfg), ETHERP_TO_MACF(ea)));
        wl_free_timer(wnm->wlc->wl, timer->timer);
        timer->timer = NULL;
        MFREE(wnm->wlc->osh, timer, sizeof(wnm_btm_disassoc_tmr_t));
        timer = NULL;
        ret = BCME_OK;
    } else {
        WL_WNM(("wl%d.%d BTM Disassoc timer for deletion not found "MACF"\n",
            wnm->wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), ETHERP_TO_MACF(ea)));
        ret = BCME_NOTFOUND;
    }
...
static int
wlc_wnm_del_all_btm_disassoc_tmr(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg)
{
    wnm_btm_disassoc_tmr_t *timer, *timer_next;
    wnm_bsscfg_cubby_t *wnm_cfg;

    wnm_cfg = WNM_BSSCFG_CUBBY(wnm, bsscfg);
    timer = wnm_cfg->disassoc_timer;

    WL_WNM(("wl%d.%d Deleting all BTM disassoc timers \n", wnm->wlc->pub->unit,
        WLC_BSSCFG_IDX(bsscfg)));
    while (timer) {
