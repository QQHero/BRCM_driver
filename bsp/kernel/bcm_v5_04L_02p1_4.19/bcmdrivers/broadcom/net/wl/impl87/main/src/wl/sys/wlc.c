 ...
 *
 * $Id: wlc.c 810769 2022-04-13 17:27:14Z $
 */
...
#if (!defined(CONFIG_UGW_BCM_DONGLE)) || defined(DONGLEBUILD) //没有适配dongle的才需要include <td_multiap_steer.h>
#include <td_multiap_steer.h>
#endif
#if defined(DONGLEBUILD)
#include "td_bcm_rte_os.h"
#ifndef IFNAMSIZ
#define IFNAMSIZ 16
#endif  // IFNAMSIZ
#endif  // defined(DONGLEBUILD)
#endif  //defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
...
#define APCLIENT_STATUS_WLC_STATE_OFFSET 8  /*bit offset of APClient status flags*/
#endif

#if defined(TD_PRIVATE_IE) && defined(CONFIG_TENDA_PRIVATE_WLAN)
#if (!defined(CONFIG_UGW_BCM_DONGLE)) //dongle编译或者没有适配dongle的其他方案才需include
...
typedef struct td_sta_info {
    uint8 ether_addr_octet[ETHER_ADDR_LEN];
    char ifname[16];
    int rssi;
    int sup11k;
    int sup11v;
...
#ifndef CONFIG_TENDA_WLAN_ZIP_IMAGE
    WL_ERROR_VAL | WL_ASSOC_VAL;
#else
    WL_ERROR_VAL;
#endif /* CONFIG_TENDA_WLAN_ZIP_IMAGE */
#else
    0;
#endif /* BCMDBG */
...
#if defined(BCMDBG)
/* Msg macfilter */
int wl_msg_macfltr_idx = 0;
struct ether_addr wl_msg_macfltr[MSG_MACFLTR_MAX];
#endif /* BCMDBG */
...
enum wlc_iov {
    ...
    IOV_WSERVER_PID = 182,
#endif
#ifdef WL_DIAG_CAPTURE_TXRX_FRAMES
    IOV_DIAG_CAPTURE = 183,
#endif
    IOV_LPI_MODE = 184,
    IOV_MSG_MACFLTR = 185,
    ...
    IOV_TD_MCAST_LIMIT_THESH  = 190,
    IOV_TD_MCAST_LIMIT_ENABLE = 191,
#endif
#ifdef GAME_SPEEDUP
    IOV_SPECIAL_STA = 192,
    IOV_SPECIAL_FIXRATE = 193,
    IOV_SPECIAL_TRAFFIC = 194,
#endif
...
#endif /* WL_CFG80211_NIC */
#ifdef WL_MBSSID
    {"mbssid", IOV_MBSSID, IOVF_SET_DOWN,
    0, IOVT_BUFFER, sizeof(wl_mbssid_t)},       /* enable/disable mbssid */
#endif /* WL_MBSSID */
#ifdef WL_DIAG_CAPTURE_TXRX_FRAMES
    {"diag_capture", IOV_DIAG_CAPTURE,
    (IOVF_SET_UP), 0, IOVT_UINT32, 0
    },
#endif
    {"lpi_mode", IOV_LPI_MODE,
    (IOVF_SET_DOWN), 0, IOVT_UINT32, 0
    },
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    {"em_maxpower", IOV_EM_MAXPOWER,
    (IOVF_OPEN_ALLOW), 0, IOVT_UINT32 ,0
    },
#endif
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    {"td_sta_info", IOV_TD_STA_INFO,
    (IOVF_SET_UP|IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, sizeof(td_sta_info_t)
    },
    {"td_bssmode", IOV_TD_BSSMODE,
    (0), 0, IOVT_INT32, 0
    },
#endif
#ifdef BCMDBG
    {"msg_macfilter", IOV_MSG_MACFLTR,
    (0), 0, IOVT_BUFFER, sizeof(struct  ether_addr)
    },
#endif /* BCMDBG */
#ifdef CONFIG_TENDA_WLAN_REPEATER
    {"wserver_pid", IOV_WSERVER_PID, IOVF_OPEN_ALLOW, 0, IOVT_UINT32, 0},
#endif /* BCMDBG */
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
    {"mcast_limit_thresh", IOV_TD_MCAST_LIMIT_THESH, IOVF_OPEN_ALLOW, 0, IOVT_UINT32, 0},
    {"mcast_limit_en", IOV_TD_MCAST_LIMIT_ENABLE, IOVF_OPEN_ALLOW, 0, IOVT_UINT32, 0},
#endif
#ifdef GAME_SPEEDUP
   {"special_sta", IOV_SPECIAL_STA,
   (0), 0, IOVT_BUFFER, sizeof(special_sta_t)
   },
    {"special_fixrate", IOV_SPECIAL_FIXRATE,
    (0), 0, IOVT_UINT32, 0
    },
    {"special_traffic", IOV_SPECIAL_TRAFFIC,
    (0), 0, IOVT_UINT8, 0
    },
#endif
    {NULL, 0, 0, 0, 0, 0}
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
static int wlc_td_sta_info(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, const struct ether_addr *ea,
                        td_sta_info_t *td_sta);
#endif
/* local prototypes */
...
#ifdef GAME_SPEEDUP
static void wlc_special_timer(void *arg);
#endif
#ifdef STA
...
#ifdef BCMDBG
static void wlc_dump_msgfltr(wlc_info_t *wlc, struct bcmstrbuf *b);
#endif /* BCMDBG */
...
static void _wlc_bss_update_beacon(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool check_mbssid);
...
static void
wlc_sta_info_upd_scb_median_rate_stats(const wlc_info_t *wlc, const wlc_bsscfg_t *bsscfg,
    wlc_scb_stats_t *scb_stats, wlc_scb_rate_stats_t *scb_rate_stats, sta_info_t *sta);
...
static void wlc_hwrs_deauth_callback(wlc_info_t *wlc, uint txstatus, void *pkt);
...
static void wlc_out(wlc_info_t *wlc, bool psmwd_debug);
#endif /* BCMNODOWN */
...
{
#ifdef WLTAF
    /* Defer taf schedule to prevent packet enqueue during fifo flush */
...
#ifdef BCMDBG
bool
msg_mac_filter_ex(int msglvl, scb_t *scb)
{
    if (wl_msg_level & msglvl) {
        if ((wl_msg_macfltr_idx == 0) || (scb && scb->msg_macfltr)) {
            return TRUE;
        }
    }
    return FALSE;
...
bool
msg_mac_filter_ex_pfx(int msglvl, scb_t *scb, msg_macfilter_pfx_t *msg_mac_filter_prefix,
    const char* function_name)
{
    bool ret;

    ret = msg_mac_filter_ex(msglvl, scb);

    if (ret && (msg_mac_filter_prefix->initialized == FALSE)) {
        if (scb) {
            snprintf(msg_mac_filter_prefix->prefix, WL_MSG_MACFILTER_PFX_LEN,
                WL_DBG_MSG_MAC_PFX_STRING,
                SCB_BSSCFG(scb)->wlc->pub->unit,
                SCB_BSSCFG(scb)->_idx,
        ...
        msg_mac_filter_prefix->initialized = TRUE;
    }
...
static void
wlc_dump_msgfltr(wlc_info_t *wlc, struct bcmstrbuf *b)
{
    ...
    bcm_bprintf(b, "msg mac filter enabled %d used %d max %d\n",
        (wl_msg_macfltr_idx > 0), wl_msg_macfltr_idx, MSG_MACFLTR_MAX);

    FOREACHSCB(wlc->scbstate, &scbiter, scb) {
        bcm_bprintf(b, " %3d.\t"MACF" %s\n",
            i, ETHER_TO_MACF(scb->ea), (scb->msg_macfltr) ? "*":" ");
        i++;
    }
    bcm_bprintf(b, "msg mac filters:\n");
    for (i = 0; i < MSG_MACFLTR_MAX; i++) {
        bcm_bprintf(b, " %3d.\t"MACF"\n", i, ETHER_TO_MACF(wl_msg_macfltr[i]));
    }
}
#endif /* BCMDBG */
...
void BCMINITFN(wlc_init)(wlc_info_t *wlc)
{
    ...
    /* CSIMON only supports first three radios */
    if (D11REV_GE(wlc->pub->corerev, 129) && (wlc->pub->unit < MAX_CSIMON_NETLINK)) {
        wlc_csimon_init(wlc);
    }
#endif

#if defined(BME_PKTFETCH)
    /* Initialize BME based Packet Fetch module info */
    wlc_bme_pktfetch_init(wlc);
#endif /* BME_PKTFETCH */
    ...
    if (D11REV_GE(wlc->pub->corerev, 128) && wlc->clk) {
        wlc_write_shm(wlc, M_RTS_MINLEN_L(wlc), wlc->RTSThresh);
    }
#if BAND6G
    if ((BAND_6G(band->bandtype)) && (wlc_is_cbp_fcc(wlc))) {
        /* set CBPWAR flag */
        wlc_mhf(wlc, MHF6, MHF6_CBPWAR, MHF6_CBPWAR, WLC_BAND_ALL);
    } else {
        /* clear CBPWAR flag */
        wlc_mhf(wlc, MHF6, MHF6_CBPWAR, 0, WLC_BAND_ALL);
    }
#endif
...
wlc_clear_bssid(wlc_bsscfg_t *cfg)
{
    ...
    if (BSSCFG_IS_PRIMARY(cfg)) {
        ...
        if (MULTIBSSID_ENAB(wlc->pub) && BSSCFG_STA(cfg)) {
            if (cfg->mbssid_transbss_amtidx >= 0) {
                (void) wlc_keymgmt_free_amt(wlc->keymgmt,
                    &cfg->mbssid_transbss_amtidx);
            }
        }
#endif /* WL_MBSSID */
    }
...
wlc_set_bssid(wlc_bsscfg_t *cfg)
{
    ...
    if (BSSCFG_IS_PRIMARY(cfg)) {
        ...
        if (MULTIBSSID_ENAB(wlc->pub) && BSSCFG_STA(cfg)) {
            ...
            if (cfg->mbssid_transbss_amtidx >= 0) {
                /* first release old value if still set */
                (void) wlc_keymgmt_free_amt(wlc->keymgmt,
                    &cfg->mbssid_transbss_amtidx);
            }
            cfg->mbssid_transbss_amtidx = wlc_keymgmt_alloc_amt(wlc->keymgmt);
            if (cfg->mbssid_transbss_amtidx < 0) {
                WL_ERROR(("wl%d: %s: Error mbssid_transbss_amtidx [%d]\n",
                    wlc->pub->unit, __FUNCTION__, cfg->mbssid_transbss_amtidx));
            }
...
wlc_scb_disassoc_cleanup(wlc_info_t *wlc, struct scb *scb)
{
    ...
    /* Return if SCB is already being cleaned up else mark it */
    if (SCB_CLEANUP_IN_PROGRESS(scb)) {
        WL_INFORM(("wl%d.%d: %s: Cleanup of "MACF" already in progess.\n",
            wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
            __FUNCTION__, ETHER_TO_MACF(scb->ea)));
        return;
    }

    scb->mark |= SCB_CLEANUP_IN_PROG;

#ifdef WL_ULMU
    /* Notify driver and ucode to remove scb from ulmu list */
    wlc_ulmu_del_usr(wlc->ulmu, scb, TRUE);
#endif /* WL_ULMU */
    ...
    if (SCB_DWDS(scb)) {
        ...
        if (wlc->ul_ofdma_repeater_ac_param_mod) {
            edcf_acparam_t *acp = &bsscfg->wme->wme_param_ie.acparam[AC_BK];
            wlc->ul_ofdma_repeater_ac_param_mod = FALSE;
            /* Undo AC_BK paramater modifcation.
            * It was modified to param of AC_BE at the time of FIFO allocation
            * in wlc_ulmu_alloc_fifo(). Now write real AC_BK param.
            */
...
#if defined(STA) && defined(WL_PWRSTATS)
...
wlc_macctrl_init(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
    ...
    if (!MBSS_ENAB(wlc->pub)) {
        ...
        if (cfg != NULL) {
            WL_INFORM(("wl%d: BSS "MACF" is configured as a Closed Network, "
...
wlc_BSSinit(wlc_info_t *wlc, wlc_bss_info_t *bi, wlc_bsscfg_t *cfg, int type)
{
    ...
    if (BSSCFG_AP(cfg) || !cfg->BSS) {
        ...
        _wlc_bss_update_beacon(wlc, cfg, TRUE);
...
#ifdef GAME_SPEEDUP
   if (!(wlc->special_timer = wl_init_timer(wlc->wl, wlc_special_timer, wlc, "special"))) {
       WL_ERROR(("wl%d: special_timer failed\n", unit));
       goto fail;
   }
#endif
...
BCMATTACHFN(wlc_info_init)(wlc_info_t *wlc, int unit)
{
    ...
    if ((wlc->phy_cap & PHY_PREATTACH_CAP_SUP_6G) &&
        ...
        wlc->chanspec = CH20MHZ_CHSPEC(1, WL_CHANSPEC_BAND_6G);
    } else if ((wlc->phy_cap & PHY_PREATTACH_CAP_SUP_5G) &&
        !(wlc->phy_cap & PHY_PREATTACH_CAP_SUP_2G)) {
        /* Assuming cores are bandlocked/band specific */
        wlc->chanspec = CH20MHZ_CHSPEC(36, WL_CHANSPEC_BAND_5G);
    } else {
        wlc->chanspec = CH20MHZ_CHSPEC(1, WL_CHANSPEC_BAND_2G);
    }
    ...
    */
    ...
    wlc->probe_rtx_limit = MAX_PRS_RTX_DEF;
    ...
    /* Default max new key duration allowed */
    wlc->pub->newkey_pktdrop_timeout = WL_MAX_KEY_DUR;
    ...
    wlc->pub->_pref_transmit_bss = WLC_INVALID_BSS_IDX;
    wlc->lpi_mode = AUTO;
} /* wlc_info_init */
...
#if defined(TD_PRIVATE_IE) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    TDCORE_FUNC_IF(td_ie_init) {
        TDCORE_FUNC(td_ie_init)(wlc);
    }
#endif
#if defined(AI_MESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    ai_mesh_init();
...
#if defined(CONFIG_UGW_BCM_DONGLE) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    MODULE_ATTACH(td_extend, wlc_td_extend_attach, 220);
#endif
    return BCME_OK;
...
wlc_dump_ratesets(void *ctx, struct bcmstrbuf *b)
{
    ...
    FOREACH_BSS(wlc, idx, cfg) {
        ...
        FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
            bcm_bprintf(b, "scb["MACF"]->rateset:\n", ETHER_TO_MACF(scb->ea));
...
#endif
#if defined(BCMDBG)
    wlc_dump_register(pub, "msg_macfilter", (dump_fn_t)wlc_dump_msgfltr, (void *)wlc);
#endif /* BCMDBG */

    /* low level attach steps(all hw accesses go inside, no more in rest of the attach) */
    err = wlc_bmac_attach(wlc, vendor, device, unit, osh, regsva, bustype, btparam);
    if (err != 0) {
...
/* for EAP builds do not add additional default VHT features beyond the defaults */
    ...
    */
    ...
    if (wlc_proxdscbs_alloc(wlc) != BCME_OK) {
        WL_ERROR(("wl%d: wlc_proxdscbs_alloc failed\n", unit));
        err = 1103;
        goto fail;
    }
#endif
...
#ifdef GAME_SPEEDUP
   if((wlc->special_scb = MALLOCZ(wlc->osh, MAX_SPECIAL_SCB * sizeof(special_scb_t))) == NULL) {
       err = 2001;
...
#define NUM_ANT      3
#define MAXNUM_ANT_ENTRY    ARRAYSIZE(ant_default)
...
#ifdef GAME_SPEEDUP
   if (wlc->special_timer) {
       wl_free_timer(wlc->wl, wlc->special_timer);
       wlc->special_timer = NULL;
   }
#endif
...
#ifdef BCM_CSIMON
    /* CSIMON only supports first three radios */
    if (wlc->pub->unit < MAX_CSIMON_NETLINK) {
...
#endif
#if defined(CONFIG_UGW_BCM_DONGLE) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    MODULE_DETACH(wlc->td_extend, wlc_td_extend_detach);
#endif
} /* wlc_detach_module */
...
wlc_check_assert_type(wlc_info_t *wlc, uint32 reinit_reason)
{
    bool is_out = FALSE;
#ifndef BCMNODOWN
    if (g_assert_type == 3 || g_assert_type == 4) {
...
/** bring the driver down, but don't reset hardware */
static void
wlc_out(wlc_info_t *wlc, bool psmwd_debug)
{
    ...
    if (psmwd_debug) { /* only if actual PSM watchdog and not wl out iovar */
        /* halt the PSM */
...
wlc_watchdog(void *arg)
{
...
#ifdef TD_STEER
    static unsigned char s_steer_flag = 0;
#endif
...
#ifdef TD_STEER
    /* s_steer_flag 0
    * 2.4g:wlc-<pub->unit:0;5g:wl->pub->unit:1
    * Shift 1 unit bit to expand maps_timer-hander from once every s to once every 2s
    */
    s_steer_flag ^= (1 << wlc->pub->unit);
    if (s_steer_flag & (1 << wlc->pub->unit)) {
        TDCORE_FUNC_IF(maps_timer_handler) {
            TDCORE_FUNC(maps_timer_handler)((void*)wlc);
        }
    }
#endif
    ...
    */
    ...
    if (AP_ENAB(wlc->pub) && APSTA_ENAB(wlc->pub) && wlc_apup_allowed(wlc)) {
        ...
        if (BSSCFG_STA(wlc->primary_bsscfg) &&
#ifdef CONFIG_TENDA_PRIVATE_WLAN
            /* 上级为不加密(ws    ec=0)和WEP模式(wsec=1)时，才启用驱动自主关联扫描 */
            ((0 == wlc->primary_bsscfg->wsec) || (1 == wlc->primary_bsscfg->wsec)) &&
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
...
#ifdef CONFIG_TENDA_SCAN_PRIORITY
                else if (AS_SCAN_ABORT == wlc_assoc_get_as_state(wlc->primary_bsscfg)){
                    /* 等待主动触发的扫描完成 */
                    ;
                } 
#endif /* CONFIG_TENDA_SCAN_PRIORITY */
                else {
    ...
    FOREACH_AS_STA(wlc, i, cfg) {
        ...
        if (cfg->BSS) {
            ...
            if (scb && SCB_MARKED_DEL(scb)) {
                WL_ASSOC(("wl%d.%d: %s Free scb:"MACF" \n", wlc->pub->unit,
                    WLC_BSSCFG_IDX(cfg), __FUNCTION__,
                    ETHER_TO_MACF(scb->ea)));
                wlc_disassociate_client(cfg, FALSE);
                /* below function will cause the scb to be freed
                 * using notification callback handler wlc_scb_bss_state_upd
                 */
    ...
    FOREACH_UP_AP(wlc, i, cfg) {
        ...
        FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
            if (!SCB_INTERNAL(scb) &&
                ...
                scb->sched_staperc = (scb->bsscfg->sched_bssperc *
                    scb->sched_staperc)/100;
            }
        }
    }
}
...
BCMINITFN(wlc_up)(wlc_info_t *wlc)
{
    ...
    if (wlc->toe_capable) {
        ...
        /* AMPDU needs TOE header for the Epoch bit. Header cache also need it (?) */
    ...
    */
    ...
    if (wlc->lpi_mode != AUTO) {
        wlc_mhf(wlc, MHF3, MHF3_6G_LPI_MODE,
            wlc->lpi_mode ? MHF3_6G_LPI_MODE : 0, WLC_BAND_ALL);
    }
    WL_MPC(("wl%d: UP\n", wlc->pub->unit));
...
#ifdef GAME_SPEEDUP
   if(wlc_special_scb_exist(wlc) && !wlc->special_run)
       wlc_special_timer(wlc);
#endif
...
BCMUNINITFN(wlc_down)(wlc_info_t *wlc)
{
    ...
    if (!dev_gone) {
        ...
        if (wlc_dfs_cac_in_progress(wlc->dfs)) {
            WL_DFS(("wl%d %s: CAC is in progress...Aborting CAC\n",
                WLCWLUNIT(wlc), __FUNCTION__));
            wlc_dfs_abort_cac(wlc->dfs);
        }
#endif /* WLDFS */
...
#ifdef GAME_SPEEDUP
   if(wlc->special_run) {
       wl_del_timer(wlc->wl, wlc->special_timer);
       wlc->special_run = FALSE;
   }
#endif
...
wlc_rateset_show(wlc_info_t *wlc, wlc_rateset_t *rs, struct ether_addr *ea)
{
    ...
    if (WL_RATE_ON()) {
        if (ea != NULL) {
            WL_RATE(("wl%d: %s: "MACF": ", WLCWLUNIT(wlc), __FUNCTION__,
                ETHERP_TO_MACF(ea)));
        }
        WL_RATE(("[ "));
...
wlc_bandlock(wlc_info_t *wlc, int val)
{
    ...
    wlc_mac_event(wlc, WLC_E_BAND_CHANGE, NULL, 0, 0, 0, &val, sizeof(uint));
...
wlc_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
    ...
    case WLC_SET_SSID: {
        ...
        if ((ssid->SSID_len > DOT11_MAX_SSID_LEN) &&
            (ssid->SSID_len != WL_SHORT_SSID_MAGIC_LEN)) {
...
#ifdef CONFIG_UGW_BCM_DONGLE
        {
            struct km_wifiev_ssid_param param;
            memcpy(param.ifname, wl_ifname(bsscfg->wlc->wl, bsscfg->wlcif->wlif), IFNAMSIZ);
            param.unit = bsscfg->wlc->pub->unit;
            param.ssid_len = ssid->SSID_len;
            memcpy(param.ssid, ssid->SSID, DOT11_MAX_SSID_LEN);

            wlc_mac_event(wlc, WLC_E_TENDA_KM, NULL, KM_WIFIEV_SET_SSID, 0, 0, &param, sizeof(param));
        }
#else
        if (km_eventscenter_wifiev_set_ssid_handler) {
            km_eventscenter_wifiev_set_ssid_handler(bsscfg->wlc->pub->unit, wl_ifname(bsscfg->wlc->wl, bsscfg->wlcif->wlif), ssid->SSID, ssid->SSID_len);
        }
#endif
#endif
        ...
        if (BSSCFG_AP(bsscfg)) {
            ...
            if (WLC_BSSCFG_IDX(bsscfg) ==
                    wlc_get_apcfg_idx_used_for_upr_fd_frame(wlc)) {
        ...
        if (BSSCFG_STA(bsscfg)) {
            ...
            if (((wl_join_params_t *)arg)->ssid.SSID_len == WL_SHORT_SSID_MAGIC_LEN) {
                /* update the SSID length to 4 bytes incase of short SSID */
                ssid->SSID_len = DOT11_SHORT_SSID_LEN;
                wlc->scan->is_host_short_ssid = TRUE;
            }
    ...
    case WLC_SCB_DEAUTHENTICATE_FOR_REASON: {
        ...
        arg = eaptr;
        BCM_REFERENCE(eaptr);
        WL_ASSOC_LT(("iov:%s "MACF"\n", (cmd == WLC_SCB_DEAUTHENTICATE_FOR_REASON) ?
            "SCB_DEAUTHENTICATE_FOR_REASON" : "SCB_DEAUTHENTICATE",
            ETHERP_TO_MACF(eaptr)));
    }
        /* fall thru */
    ...
    case WLC_SCB_DEAUTHORIZE: {
        ...
        if ((cmd == WLC_SCB_AUTHORIZE) || (cmd == WLC_SCB_DEAUTHORIZE)) {
            WL_ASSOC_LT(("iov:%s "MACF"\n", (cmd == WLC_SCB_AUTHORIZE) ?
                "SCB_AUTHORIZE" : "SCB_DEAUTHORIZE", ETHERP_TO_MACF(arg)));
        }
...
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
                td_em_wds_addif_br_notify(scb->wds->wlif->name);
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
                if(!TDCORE_FUNC_IS_NULL(td_em_wds_addif_br_notify)){
#ifdef DONGLEBUILD
                    TDCORE_FUNC(td_em_wds_addif_br_notify)((char *)scb->wds->wlif->dev->name, bsscfg);
#else
                    TDCORE_FUNC(td_em_wds_addif_br_notify)((char *)scb->wds->wlif->name, bsscfg);
#endif
                }
#endif
#endif
    ...
    case WLC_GET_VALID_CHANNELS: {
        ...
        uint count = 0;
        bool channel_array_is_full = FALSE;
        ...
        FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
            ...
            FOREACH_WLC_BAND_CHANNEL20(band, i) {
                if (channel_array_is_full == TRUE) {
                    break;
                }

                if (wlc_valid_chanspec_db(wlc->cmi,
                    CH20MHZ_CHSPEC(i, BANDTYPE_CHSPEC(band->bandtype)))) {
                    channels[count++] = (uint8)i;
                    if (count == MAXCHANNEL) {
                        channel_array_is_full = TRUE;
                    }
                }
            }
        }
    ...
    case WLC_GET_SH_SSID: {
        wlc_ssid_t *ssid = (wlc_ssid_t *)arg;
        WL_PRINT(("wl%d:  short_ssid: 0x%x \n", wlc->pub->unit,
            wlc_calc_short_ssid(ssid->SSID, ssid->SSID_len)));
        break;
    }
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    case WLC_TD_STEER_MAPS:
        TDCORE_FUNC_IF(maps_ioctl_handler) {
            bcmerror = TDCORE_FUNC(maps_ioctl_handler)(arg, len, bsscfg);
        }
        break;
#endif
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
    case WLC_TD_EASYMESH:
        bcmerror = td_easymesh_ioctl((void *)bsscfg, (char *)arg, len);
        break;
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    case WLC_TD_EASYMESH: {
        em_io_data_t *emdata = NULL;
        em_io_data_t *argin = (em_io_data_t *)arg;
        char *datain = NULL;
        
        if(TDCORE_FUNC_IS_NULL(easymesh_ioctl)){
            break;
        }
        datain = (char *)MALLOC(wlc->osh, EM_MSG_MAX_BUFF_SIZE + sizeof(em_io_data_t));
        bzero((char *)datain, EM_MSG_MAX_BUFF_SIZE + sizeof(em_io_data_t));
        
        bcopy((char *)argin, datain, EM_MSG_MAX_BUFF_SIZE + sizeof(em_io_data_t));
        emdata = (em_io_data_t *)datain;
        
        bcmerror = TDCORE_FUNC(easymesh_ioctl)((void *)bsscfg, (char *)datain, len);

        bzero((char *)argin + sizeof(em_io_data_t), EM_MSG_MAX_BUFF_SIZE);
        bcopy(datain + sizeof(em_io_data_t), (char *)argin + sizeof(em_io_data_t), EM_MSG_MAX_BUFF_SIZE);

        argin->output_len = emdata->output_len;

        MFREE(wlc->osh, datain, EM_MSG_MAX_BUFF_SIZE + sizeof(em_io_data_t));
        break;
    }
#endif
#endif
...
int
wlc_validate_set_chanspec(wlc_info_t *wlc, chanspec_t chspec)
{
    if (wf_chspec_malformed(chspec)) {
...
#ifdef CONFIG_TENDA_PRIVATE_KM
#ifdef CONFIG_UGW_BCM_DONGLE
        struct km_wifiev_set_channel_param param;
        param.unit = wlc->wl->pub->unit;
        param.channel = wf_chspec_primary20_chan(chspec);

        wlc_mac_event(wlc, WLC_E_TENDA_KM, NULL, KM_WIFIEV_SET_CHANNEL, 0, 0, &param, sizeof(param));
#else
        if (km_eventscenter_wifiev_set_channel_handler) {
            km_eventscenter_wifiev_set_channel_handler(wlc->wl->pub->unit, wf_chspec_primary20_chan(chspec));
        }
#endif
#endif
...
#ifdef GAME_SPEEDUP
static struct special_scb *
wlc_special_scb_find(wlc_info_t *wlc, special_sta_t *special_sta, struct scb *scb)
{
    uint8 i;
    uint8 last_unused = MAX_SPECIAL_SCB;
    special_scb_t *special_scb;

    for (i = 0; i < MAX_SPECIAL_SCB; i++) {
        special_scb = &wlc->special_scb[i];
        if (special_scb->used == FALSE)
            last_unused = i;
        if ((special_scb->scb) && (scb == special_scb->scb)) {
            break;
        }
    }

    if ((i == MAX_SPECIAL_SCB) && (last_unused == MAX_SPECIAL_SCB)) {
        WL_ERROR(("%s, special scb full\n",__FUNCTION__));
        return (NULL);
    }
    if((i == MAX_SPECIAL_SCB) && (last_unused < MAX_SPECIAL_SCB)) {
        special_scb = &wlc->special_scb[last_unused];
    }


    if (special_sta->action == 2) {
        //delete
        if(special_scb->used == TRUE) {
            WL_ERROR(("delete special_scb index i=%u, special_scb=%px\n",i,special_scb));
            memset(special_scb, 0, sizeof(special_scb_t));
        } else {
            WL_ERROR(("no scb found to be delete\n"));
            return (NULL);
        }
    } else if (special_sta->action == 1) {
        //add
        WL_ERROR(("add tencetn_scb index i =%u, last_unused=%u, special_scb=%px\n",i, last_unused,special_scb));
        if(special_scb->used == TRUE) {
            memset(special_scb, 0, sizeof(special_scb_t));
        }
        special_scb->used = TRUE;
        special_scb->scb = scb;
        scb->special_scb = special_scb;
        memcpy(&special_scb->idlerate_thresh, &special_sta->idlerate_thresh, 2 * sizeof(uint8));
        memcpy(&special_scb->rssi_thresh, &special_sta->rssi_thresh, 2 * sizeof(int8));
        memcpy(&special_scb->utils_thresh, &special_sta->utils_thresh, 2 * sizeof(uint32));

    } else {
        WL_ERROR(("%s, action error\n",__FUNCTION__));
        return (NULL);
    }
...
bool
wlc_special_scb_exist(wlc_info_t * wlc)
{
    uint i = 0;
    bool exist = FALSE;
    special_scb_t *temp_scb;
    for(i=0; i<MAX_SPECIAL_SCB; i++){
        temp_scb = &wlc->special_scb[i];
        if (temp_scb->used) {
            exist = TRUE;
            break;
        }
...
uint8
wlc_chanim_special(wlc_info_t *wlc)
{
    special_chanim_t *special_chanim = &wlc->special_chanim;
    uint32 txop_now, txop_delta;
    uint slottime = APHY_SLOT_TIME;
    uint32 time_delta, current_time;
    uint8 txop_percent;
    ...
    txop_now = wlc_bmac_cca_read_counter(wlc->hw,
            M_CCA_TXOP_L_OFFSET(wlc), M_CCA_TXOP_H_OFFSET(wlc));
    if(current_time >= special_chanim->last_uptime) {
        time_delta = current_time - special_chanim->last_uptime;
    } else {
        time_delta = current_time + (0xFFFFFFFF - special_chanim->last_uptime);
    }

    special_chanim->last_uptime = current_time;

    if(txop_now >= special_chanim->last_txop) {
        txop_delta = txop_now - special_chanim->last_txop;
    } else {
        txop_delta = txop_now + (0xFFFFFFFF - special_chanim->last_txop);
    }

    special_chanim->last_txop = txop_now;

    txop_delta = txop_delta * slottime;

    txop_percent = (uint8)CEIL(txop_delta, time_delta * 10);
...
static void
wlc_special_watchdog(void *arg)
{
    wlc_info_t *wlc = (wlc_info_t *)arg;
    special_scb_t *special_scb;
    uint8   txop_percent;
    uint8   i = 0;
    uint8   j;//info record cnt
    ...
    //uint32 tx_rate;
    special_sta_event_t special_sta_event;
    special_all_event_t special_all_event;
    special_event_info_t *special_event_info;

    memset(&special_sta_event, 0, sizeof(special_sta_event_t));
    memset(&special_all_event, 0, sizeof(special_all_event_t));

    uint8 sta_num = 0;

    j = wlc->current_event_cnt % SPECIAL_EVENT_CNT;
    event_all = (j == 9);

    txop_percent = wlc_chanim_special(wlc);


    //scb_stats = &scb->scb_stats;
    //tx_rate = RSPEC2KBPS(scb_stats->rx_rate)/500;
    for(i=0; i<MAX_SPECIAL_SCB; i++) {
        special_scb = &wlc->special_scb[i];
        if(special_scb->used) {
            special_scb->need_event = 0;
            scb = special_scb->scb;
            if (j==0) {
                special_scb->idlerate_level = special_scb->rssi_level = special_scb->utils_level = 0;
                memset(&special_scb->idlerate_percent, 0, SPECIAL_EVENT_CNT * sizeof(uint8));
                memset(&special_scb->rssi, 0, SPECIAL_EVENT_CNT * sizeof(int8));
                memset(&special_scb->queue_utils, 0, SPECIAL_EVENT_CNT * sizeof(uint32));
            }
            wlc_txq_special(wlc, special_scb, txop_percent);
            rssi = wlc_lq_rssi_ma(wlc, SCB_BSSCFG(scb), scb);
            special_scb->rssi[j] = rssi;
            if (special_scb->rssi_level == 0) {
                if (rssi < special_scb->rssi_thresh[1]) {
                    special_scb->rssi_level = 2;
                    special_scb->need_event |= RSSI_EVENT_FLAG;
                } else if (rssi < special_scb->rssi_thresh[0]) {
                    special_scb->rssi_level = 1;
                    special_scb->need_event |= RSSI_EVENT_FLAG;
                }
            } else if (special_scb->rssi_level == 1) {
                if (rssi < special_scb->rssi_thresh[1]) {
                    special_scb->rssi_level = 2;
                    special_scb->need_event |= RSSI_EVENT_FLAG;
                }
            ...
            if(event_all){
                special_event_info = &(special_all_event.special_event_info[sta_num]);
                memcpy(&special_event_info->ea, &scb->ea, ETHER_ADDR_LEN);
                memcpy(&special_event_info->idlerate_percent, &special_scb->idlerate_percent,SPECIAL_EVENT_CNT * sizeof(uint8));
                memcpy(&special_event_info->rssi, &special_scb->rssi,SPECIAL_EVENT_CNT * sizeof(int8));
                memcpy(&special_event_info->queue_utils, &special_scb->queue_utils,SPECIAL_EVENT_CNT * sizeof(uint32));
                sta_num++;
            }
        }
    }

    //special over threshold event
    for(i=0; i<MAX_SPECIAL_SCB; i++) {
        special_scb = &wlc->special_scb[i];
        scb = special_scb->scb;
        if((special_scb->used) && special_scb->need_event) {
            memcpy(&special_sta_event.ea, &scb->ea, ETHER_ADDR_LEN);
            special_sta_event.need_event = special_scb->need_event;
            special_sta_event.idlerate_percent = special_scb->idlerate_percent[j];
            special_sta_event.rssi = special_scb->rssi[j];
            special_sta_event.queue_utils = special_scb->queue_utils[j];
            special_sta_event.idlerate_level = special_scb->idlerate_level;
            special_sta_event.rssi_level = special_scb->rssi_level;
            special_sta_event.utils_level = special_scb->utils_level;
            
#ifdef CONFIG_TENDA_PRIVATE_KM
            if (km_eventscenter_wifiev_game_speed_flowcontrol_handler) {
                km_eventscenter_wifiev_game_speed_flowcontrol_handler( SCB_BSSCFG(scb)->wlcif->wlif->dev->name, WLC_E_SPECIAL_STA_EVENT,
                    (void *)&special_sta_event,sizeof(special_sta_event_t));
            }
#else
            wlc_bss_mac_event(wlc, SCB_BSSCFG(scb), WLC_E_SPECIAL_STA_EVENT,&scb->ea, WLC_E_STATUS_ERROR, 0, 0, &special_sta_event, sizeof(special_sta_event_t));
#endif
    ...
    //one cycle event
    if (event_all) {
        special_all_event.sta_num = sta_num;
        special_all_event.datalen = sizeof(uint32) + sizeof(uint32) + sta_num * sizeof(special_event_info_t);
#ifdef CONFIG_TENDA_PRIVATE_KM
        if (km_eventscenter_wifiev_game_speed_flowcontrol_handler) {
            km_eventscenter_wifiev_game_speed_flowcontrol_handler( SCB_BSSCFG(scb)->wlcif->wlif->dev->name, WLC_E_SPECIAL_STA_ALL_EVENT,
                (void *)&special_all_event,sizeof(special_all_event.datalen));
        }
#else
            wlc_mac_event(wlc, WLC_E_SPECIAL_STA_ALL_EVENT, NULL, 0, 0, 0, &special_all_event, special_all_event.datalen);
#endif
    }

    wlc->current_event_cnt++;
}

static void
wlc_special_timer(void *arg)
{
    wlc_info_t *wlc = (wlc_info_t *)arg;
    ...
    if (wlc_special_scb_exist(wlc)) {
        wlc_special_watchdog(arg);

        wl_add_timer(wlc->wl, wlc->special_timer, wlc->timer_windows, TRUE);
        wlc->special_run = TRUE;
    }
}
#endif
    ...
    void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
    ...
    */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
            sta_info_all->states = wlc->state & 0xff; /* wlc->state value as high byte */
            sta_info_all->states <<= APCLIENT_STATUS_WLC_STATE_OFFSET;
            sta_info_all->states |= (bsscfg->assoc->state & 0xff); /* assoc->state value as low byte */
#endif
...
#endif /* AP */
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    case IOV_GVAL(IOV_TD_STA_INFO):
    {
        err = wlc_td_sta_info(wlc, bsscfg, (struct ether_addr *)params, (td_sta_info_t *)arg);
        break;
    }
    case IOV_GVAL(IOV_TD_BSSMODE):
    {
        if (BSSCFG_AP(bsscfg) && !BSSCFG_STA(bsscfg)) {
            bcopy("AP", arg, sizeof("AP"));
            break;
        } else if (!BSSCFG_AP(bsscfg) && BSSCFG_STA(bsscfg)) {
            bcopy("STA", arg, sizeof("STA"));
            break;
        } else {
    ...
    case IOV_SVAL(IOV_WPA_AUTH):
        if ((int_val & WPA3_AUTH_SAE_PSK) || (int_val & WPA3_AUTH_SAE_FBT)) {
#ifndef WL_SAE
            err = BCME_UNSUPPORTED;
            break;
#endif
        }
        if (int_val & WPA3_AUTH_DPP) {
#ifndef WL_DPP
            err = BCME_UNSUPPORTED;
            break;
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    case IOV_GVAL(IOV_COUNTERS_TD):
        if (WLC_UPDATE_STATS(wlc)) {
            wlc_statsupd(wlc);
        }
        err = wlc_get_cnts_td(wlc, arg, len);
        break;
#endif /* CONFIG_TENDA_PRIVATE_WLAN */

#endif /* WLCNT */
#ifdef WL_DIAG_CAPTURE_TXRX_FRAMES
    case IOV_GVAL(IOV_DIAG_CAPTURE):
        *ret_int_ptr = (int32)wlc->pub->_evt_diag_capture;
        break;
    case IOV_SVAL(IOV_DIAG_CAPTURE):
        wlc->pub->_evt_diag_capture = (uint32)int_val;
        break;
#endif
#if defined(DELTASTATS)
    ...
    case IOV_SVAL(IOV_CHANSPEC): {
        ...
        err = wlc_validate_set_chanspec(wlc, chspec);
        break;
    }
    ...
    case IOV_GVAL(IOV_LIFETIME):
    {
        ...
        if (int_val == WL_LIFETIME_TXFIFO && D11REV_GE(wlc->pub->corerev, 128)) {
            ...
            lifetime->lifetime = (wlc->lifetime_txfifo + 500) / 1000;
    ...
    case IOV_SVAL(IOV_LIFETIME):
    {
        ...
        if (int_val == WL_LIFETIME_TXFIFO && D11REV_GE(wlc->pub->corerev, 128)) {
            ...
            if (lifetime.lifetime <= lifetime_max) {
                ...
                wlc->lifetime_txfifo = lifetime.lifetime * 1000;
                wlc_rfaware_lifetime_set(wlc, wlc->rfaware_lifetime);
            } else {
                err = BCME_BADARG;
            }
    ...
    case IOV_GVAL(IOV_CURR_RATESET): {
        ...
        ret_rs->count = rs->count;
        bcopy(&rs->rates, &ret_rs->rates, rs->count);
        op_txstreams = wlc->stf->op_txstreams;

        if (N_ENAB(wlc->pub)) {
            int i;
            bcopy(rs->mcs, ret_rs->mcs, MCSSET_LEN);
            /* MCS to be limited based on op_txstreams */
            for (i = op_txstreams; i <= WLC_HT_MCS_MAX_NSS; i++) {
                ret_rs->mcs[i] = 0;
            }
        }
#ifdef WL11AC
        if (VHT_ENAB_BAND(wlc->pub, wlc->band->bandtype)) {
            ...
            for (nss = 1; nss <= op_txstreams; nss++) {
    ...
    case IOV_SVAL(IOV_EXTENDED_OPS_SUPPORT):
        ...
        break;
          ...
          case IOV_GVAL(IOV_MBSSID):
            {
                wl_mbssid_t cfg;
                cfg.version = WL_MBSSID_VER;
                cfg.length = sizeof(wl_mbssid_t);
                cfg.config = wlc->pub->_mbssid_enab;
                cfg.status = MBSSID_ENAB(wlc->pub, wlc->band->bandtype);
                memcpy(arg, &cfg, sizeof(wl_mbssid_t));
            }
            break;
          case IOV_SVAL(IOV_MBSSID):
            {
                ...
                if (p_len < sizeof(wl_mbssid_t)) {
                    err = BCME_BUFTOOSHORT;
                    break;
                }
                cfg = (wl_mbssid_t *)arg;
                if (cfg->version != WL_MBSSID_VER) {
                    err = BCME_VERSION;
                    break;
                }
                if (cfg->config > 1 || cfg->config < -1) {
                    err = BCME_BADARG;
                    break;
                }
                wlc->pub->_mbssid_enab = cfg->config;
            }
            break;
#endif /* WL_MBSSID */
    case IOV_GVAL(IOV_LPI_MODE): {
        *ret_int_ptr = wlc->lpi_mode;
        break;
    }
    case IOV_SVAL(IOV_LPI_MODE): {
        if (int_val >= AUTO && int_val <= ON) {
            wlc->lpi_mode = int_val;
        }
        break;
    }

#if defined(BCMDBG)
    case IOV_SVAL(IOV_MSG_MACFLTR): {
        struct scb *scb;
        struct scb_iter scbiter;
        uint8 mac_zero[] = {0, 0, 0, 0, 0, 0}; /* zero mac addr */
        ...
        mac_p = (uint8 *)arg;
        mac_addr = (struct ether_addr *)arg;
        if (!eacmp(&mac_zero, mac_p)) {
            disable = 1;
        } else {
            if (wl_msg_macfltr_idx >= MSG_MACFLTR_MAX) {
                err = BCME_RANGE;
                break;
            }
            WL_PRINT((" Enabling Msg MacFilter : "MACF"\n",
                ETHERP_TO_MACF(mac_addr)));
            eacopy((char*)mac_addr, &wl_msg_macfltr[wl_msg_macfltr_idx]);
            wl_msg_macfltr_idx++;
        }

        FOREACHSCB(wlc->scbstate, &scbiter, scb) {
            if (disable && scb->msg_macfltr) {
                WL_PRINT((" Removing Msg MacFilter : "MACF"\n",
                    ETHER_TO_MACF(scb->ea)));
                scb->msg_macfltr = 0;
            } else if (!eacmp(mac_addr, &scb->ea)) {
                scb->msg_macfltr = 1;
                break;
            }
        }
        if (disable) {
            wl_msg_macfltr_idx = 0;     /* Set invalid index */
            for (i = 0; i < MSG_MACFLTR_MAX; i++) {
                eacopy((char*)&mac_zero, &wl_msg_macfltr[i]);
            }
        }
        break;
    }
...
#ifdef CONFIG_TENDA_WLAN_REPEATER
    case IOV_GVAL(IOV_WSERVER_PID):
        *ret_int_ptr = (int32)(wlc->pub->wserver_pid);
        break;
    case IOV_SVAL(IOV_WSERVER_PID):
        wlc->pub->wserver_pid = int_val;
        break;
#endif
#ifdef TD_EASYMESH_SUPPORT
    case IOV_GVAL(IOV_EM_MAXPOWER):
        *ret_int_ptr = wlc->em_maxpwr;
        break;
    case IOV_SVAL(IOV_EM_MAXPOWER):
        wlc->em_maxpwr = int_val;
        break;
#endif

#if defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
    case IOV_GVAL(IOV_TD_MCAST_LIMIT_THESH):
        *ret_int_ptr = wlc->pub->mcast_limit_thresh;
        break;
    case IOV_SVAL(IOV_TD_MCAST_LIMIT_THESH):
        /* 上层单位为kbps，这里转换成 Bps，便于比较 */
        wlc->pub->mcast_limit_thresh = int_val * 1024 / 8;
        break;

    case IOV_GVAL(IOV_TD_MCAST_LIMIT_ENABLE):
        *ret_int_ptr = wlc->pub->mcast_limit_enable;
        break;
    case IOV_SVAL(IOV_TD_MCAST_LIMIT_ENABLE):
        wlc->pub->mcast_limit_enable = int_val;
        wlc->pub->mcast_stats_timeout = jiffies + HZ;
        break; 
#endif

#ifdef GAME_SPEEDUP
   case IOV_SVAL(IOV_SPECIAL_STA): {
       special_sta_t *special_sta;
       special_scb_t *special_scb;
       struct scb *scb;
       special_sta = (special_sta_t *)params;
       if (!(scb = wlc_scbfind(wlc, bsscfg, &special_sta->ea))) {
           err = BCME_NOTFOUND;
       } else {
           special_scb = wlc_special_scb_find (wlc, special_sta, scb);
           if (!special_scb) {
               err = BCME_ERROR;
           } else {

               if (special_sta->action == 1) {
                   if(special_sta->timer_windows && (special_sta->timer_windows != wlc->timer_windows)) {
                       wlc->timer_windows = special_sta->timer_windows;
                   }
                   wlc->current_event_cnt = 0;
                   wlc_special_timer(wlc);
       
               } else if (special_sta->action == 2) {
               
                   if(!wlc_special_scb_exist(wlc)) {
                       WL_ERROR(("no special scb, del timer\n"));
                       wl_del_timer(wlc->wl, wlc->special_timer);
                       wlc->special_run = FALSE;
                   }
   ...
   case IOV_GVAL(IOV_SPECIAL_STA): {
       ...
       special_sta = (special_sta_t *)arg;
       if (!(scb = wlc_scbfind(wlc, bsscfg, ea))) {
           err = BCME_NOTFOUND;
       } else {
           for(i=0; i<MAX_SPECIAL_SCB; i++){
               special_scb = &wlc->special_scb[i];
               if (scb == special_scb->scb)
                   break;
           }
           if (i >= MAX_SPECIAL_SCB) {
               err = BCME_NOTFOUND;
           } else if (!special_scb->used) {
               err = BCME_NOTFOUND;
           } else {
               memcpy(&special_sta->ea,ea,ETHER_ADDR_LEN);
               special_sta->action = 0;
               memcpy(&special_sta->idlerate_thresh, &special_scb->idlerate_thresh, 2 * sizeof(uint8));
               memcpy(&special_sta->rssi_thresh, &special_scb->rssi_thresh, 2 * sizeof(int8));
               memcpy(&special_sta->utils_thresh, &special_scb->utils_thresh, 2 * sizeof(uint32));
               special_sta->timer_windows = wlc->timer_windows;
           }
       }

   }
       break;
    case IOV_SVAL(IOV_SPECIAL_FIXRATE): {
        wlc->special_fixrate =  int_val;
        err = wlc_set_special_ratespec_override(wlc, wlc->special_fixrate);
    }
        break;
    case IOV_GVAL(IOV_SPECIAL_FIXRATE): {
        *ret_int_ptr = wlc->special_fixrate;
    }
        break;
    case IOV_SVAL(IOV_SPECIAL_TRAFFIC): {
        wlc->special_traffic = (uint8)int_val;
    }
        break;
    case IOV_GVAL(IOV_SPECIAL_TRAFFIC): {
        *ret_int_ptr = wlc->special_traffic;
    }
        break;
#endif
...
wlc_dump_wlc(wlc_info_t *wlc, struct bcmstrbuf *b)
{
    ...
    bcm_bprintf(b, "\n");
    bcm_bprintf(b, "promisc %d monitor %d gmode %d\n",
            wlc->pub->promisc, wlc->monitor, wlc->band->gmode);
#ifdef BCM_CSIMON
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    bcm_bprintf(b, "mcast_limit_enable %u mcast_pkts_len %u  mcast_limit_thresh %u mcast_drop_cnt %u\n", 
                    wlc->pub->mcast_limit_enable, wlc->pub->mcast_pkts_len, wlc->pub->mcast_limit_thresh,  wlc->pub->mcast_drop_cnt);
#endif
...
wlc_get_all_cnts(wlc_info_t *wlc, void *buf, int buflen)
{
    ...
    for (i = 0; i < ARRAYSIZE(wlc->pub->_cnt->reinitreason); i++) {
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
int
wlc_get_cnts_td(wlc_info_t *wlc, void *buf, int buflen)
{
    wireless_info_t *outinfo = (wireless_info_t *)buf;

    outinfo->unit = wlc->pub->unit;
    outinfo->txerr = wlc->pub->_cnt->txerror;
    outinfo->rxerr = wlc->pub->_cnt->rxerror;
...
static const struct devid_bandmask BCMATTACHDATA(wlc_bandmask_table)[] = {
    ...
    { BCM43602_D11AC_ID,        (1 << BAND_2G_INDEX) }, /* only 2G allowed in KUDU */
    { BCM4352_D11AC_ID,     ((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
    ...
    { BCM43602_CHIP_ID,     (1 << BAND_2G_INDEX) },  /* only 2G allowed in KUDU */
...
{
    return wlc_prec_enq_head(wlc, q, pkt, prec, FALSE, NULL);
}

bool BCMFASTPATH
wlc_prec_enq_head(wlc_info_t *wlc, struct pktq *q, void *pkt,
...
wlc_pkttag_info_move(wlc_info_t *wlc, void *pkt_from, void *pkt_to)
{
    ...
    WLPKTTAG(pkt_to)->taf = WLPKTTAG(pkt_from)->taf;
#endif
#ifdef WLATF_BARE
    WLPKTTAG(pkt_to)->pktinfo.atf = WLPKTTAG(pkt_from)->pktinfo.atf;
#endif /* WLATF_BARE */
#ifdef CONFIG_TENDA_XMESH_HB_ENABLE
    WLPKTTAG(pkt_to)->xmesh_low_rate = WLPKTTAG(pkt_from)->xmesh_low_rate;
#endif
}
...
wlc_pkt_abs_supr_enq(wlc_info_t *wlc, struct scb *scb, void *pkt)
{
    ...
    if (pkttag->flags3 & WLF3_TXQ_SHORT_LIFETIME) {
        ...
        if (!(pkttag->flags & WLF_PSDONTQ)) {
            ...
            WL_INFORM(("wl%d.%d: %s: requeue packet %p for "MACF" %s\n",
            ...
            WL_INFORM(("wl%d.%d: %s: drop NULL packet %p for "MACF"\n",
                WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), __FUNCTION__,
                OSL_OBFUSCATE_BUF(pkt),
                ETHER_TO_MACF(scb->ea)));
            free_pkt = TRUE;
        ...
        if (!psp) {
            ...
            WL_INFORM(("wl%d.%d: %s: drop PSPoll packet %p for "MACF"\n",
                WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), __FUNCTION__,
                OSL_OBFUSCATE_BUF(pkt),
                ETHER_TO_MACF(scb->ea)));
            free_pkt = TRUE;
    ...
    uint16 fc, uint8 *plcp)
{
    ...
    /**
     * [b26-b32] TxOP
     * If not 6GHz or bsscolor_disable is true, set to 0x7f (unspecified)
     * Otherwise, leave it as default 0
     * XXX: The lower mac will adjust it based on actual txop. Can remove the conditions later
     */
    if (!CHSPEC_IS6G(wlc->chanspec) || wlc_he_get_bsscolor_disable(wlc, cfg)) {
...
wlc_cck_plcp_set(int rate_500, uint length, uint8 *plcp)
{
    ...
    default:
        WL_ERROR(("%s: invalid/unsupported rate %d\n", __FUNCTION__, rate_500));
        ASSERT(0);
        break;
    }
    /* PLCP signal byte */
...
wlc_mgmt_ctl_d11hdrs(wlc_info_t *wlc, void *pkt, struct scb *scb,
    uint fifo, ratespec_t rspec_override)
{
    return wlc_d11hdrs(wlc, pkt, scb, 0, 0, 0, fifo, 0, NULL, NULL, rspec_override);
}
...
/* Need AP defined here to print out capability only if enabled */
    ...
    bcm_bprintf(b, "assert ");
#endif
#if defined(BCMDBG)
    bcm_bprintf(b, "dbg ");
#endif
#if defined(WLC_MEDIAN_RATE_REPORT)
    bcm_bprintf(b, "reportmedianrate ");
#endif /* WLC_MEDIAN_RATE_REPORT */
} /* wlc_cap_bcmstrbuf */
...
wlc_pkt_get_d11_hdr(wlc_info_t* wlc, void *p)
{
    ...
    if (D11REV_LT(wlc->pub->corerev, 40)) {
...
#if defined(BCMDBG_ASSERT)
        uint8 dot11_hdrsz = DOT11_A3_HDR_LEN;
#endif /* BCMDBG_ASSERT */
        ...
        if ((WLPKTTAG(p)->flags & WLF_TXHDR) != 0) {
            ...
            offset += sizeof(wlc_pdu_tx_params_t);
        }
        h = (struct dot11_header*)(pktHdr + offset);
#if defined(BCMDBG_ASSERT)
        if (FC_TYPE(ltoh16(h->fc)) == FC_TYPE_CTL) {
            dot11_hdrsz = sizeof(struct dot11_ctl_header);
        }
    ...
    dot11_management_header_t *mgmt, uint32 len)
{
    ...
    if ((scb->WPA_auth == WPA_AUTH_OWE) || (scb->WPA_auth == WPA3_AUTH_DPP)) {
        ...
        } else if (scb->WPA_auth == WPA_AUTH_OWE) {
            bcm_tlv_t *rsn_ie;
    ...
    dot11_management_header_t *mgmt, uint32 len)
{
    ...
    WL_ASSOC(("wl%d: %s: auth frame for MAC "MACF"\n", WLCWLUNIT(wlc), __FUNCTION__,
        ETHER_TO_MACF(mgmt->da)));
    ...
    if (scb == NULL) {
        WL_ASSOC(("wl%d: %s: auth frame for unknown MAC "MACF"\n", WLCWLUNIT(wlc),
            __FUNCTION__, ETHER_TO_MACF(mgmt->da)));
        return BCME_BADADDR;
    }
    ...
    struct dot11_management_header *hdr, uint8 *body, int body_len)
{
    ...
    if (ETHER_ISMULTI(&hdr->da)) {
        WL_INFORM(("wl%d: %s: suppressing error reply to "MACF
            " due to multicast address "MACF"\n",
            WLCWLUNIT(wlc), __FUNCTION__, ETHER_TO_MACF(hdr->sa),
            ETHER_TO_MACF(hdr->da)));
        return BCME_ERROR;
    }
    ...
    if (body_len < 2) {
        WL_INFORM(("wl%d: %s: sending error reply to "MACF" for Category %d "
            " with missing Action field\n", WLCWLUNIT(wlc), __FUNCTION__,
            ETHER_TO_MACF(hdr->sa), action_category));
    } else {
        WL_INFORM(("wl%d: %s: sending error reply to "MACF" for Category/Action %d/%d\n",
            WLCWLUNIT(wlc), __FUNCTION__,
            ETHER_TO_MACF(hdr->sa), action_category, (int)body[1]));
    }
...
static wlc_bsscfg_t *wlc_get_pref_transmit_bss(wlc_info_t *wlc)
{
    wlc_bsscfg_t *cfg;
    uint8 pref_tx_bss = wlc->pub->_pref_transmit_bss;
    int i;

    if (pref_tx_bss == WLC_INVALID_BSS_IDX) {
        pref_tx_bss = 0;
    }

    FOREACH_AP(wlc, i, cfg) {
        if (cfg->_idx == pref_tx_bss) {
            return cfg;
        }
    }

    return (wlc_bsscfg_t *)NULL;
}

int
wlc_calc_beacon_len(wlc_info_t *wlc, int *bcn_len)
{
    uint8 *body, *bcn_buf;
    int hdr_len = DOT11_MAC_HDR_LEN, body_len;
    wlc_bsscfg_t *cfg;
    uint16 max_bss_count;

    if (!MBSSID_ENAB(wlc->pub, wlc->band->bandtype))
    {
        WL_ERROR(("%s: MBSSID must be enabled\n", __FUNCTION__));
        return BCME_UNSUPPORTED;
    }

    cfg = wlc_get_pref_transmit_bss(wlc);
    if (cfg == NULL) {
        WL_ERROR(("%s: Could not get pref transmit BSS config\n", __FUNCTION__));
        return BCME_ERROR;
    }

    bcn_buf = MALLOCZ(wlc->osh, MAX_BEACON_LEN);
    if (bcn_buf == NULL) {
        WL_ERROR(("%s: Could not allocate memory for beacon buffer\n", __FUNCTION__));
        return BCME_NOMEM;
    }

    body_len = MAX_BEACON_LEN - hdr_len; /* calc buffer size provided for frame body */
    body = bcn_buf + hdr_len;

    wlc_bcn_prb_body(wlc, FC_BEACON, cfg, body, &body_len, NULL); /* mpdu without phy header */
    MFREE(wlc->osh, bcn_buf, MAX_BEACON_LEN);

    /* Add some for minimum TIM length */
    max_bss_count = wlc_ap_get_maxbss_count(wlc->ap);
    body_len += TLV_HDR_LEN + DOT11_MNG_TIM_FIXED_LEN + 1 + (max_bss_count/8);

    *bcn_len = hdr_len + body_len; /* return actual size */
    return BCME_OK;
}
#endif /* WL_MBSSID */
                     ...
                     uint16 *buf, int *len)
{
    ...
    wlc_bcn_prb_body(wlc, type, cfg, body, &body_len, NULL); /* mpdu without phy header */
 ...
 *
 *
 * check_mbssid: Set to true when Only specific bsscfg beacon update is called. As Only the
 * transmitted bss is beaconing in MBSSID. Need to update the transmited bss beacon.
 * case where wlc_bss_update_beacon is called in a FOREACHUPAP loop,
 * this need to be set to FALSE as transmitted beacon update is also called in the loop.
 */
static void
_wlc_bss_update_beacon(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool check_mbssid)
{
    /* Clear the soft intmask */

    wlc_bmac_set_defmacintmask(wlc->hw, MI_BCNTPL, ~MI_BCNTPL);
    /* MBSSID: only te transmitted bss beacons */
    if (check_mbssid) {
        if (MBSSID_ENAB(wlc->pub, wlc->band->bandtype)) {
            if (wlc->main_ap_bsscfg) {
                cfg = wlc->main_ap_bsscfg;
            }
        }
    }
...
void
wlc_bss_update_beacon(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool check_mbssid)
{
    if (!cfg->up)
        return;
    _wlc_bss_update_beacon(wlc, cfg, check_mbssid);
}
...
wlc_bss_update_probe_resp(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool suspend)
{
    ...
    if (BAND_6G(wlc->band->bandtype)) {
        if (WL_UPR_FD_SW_ENAB(wlc)) {
            ...
            wlc_ap_bcast_probe_rsp_update_required(wlc, wlc->main_ap_bsscfg);
    ...
    void *param)
{
    ...
    bool ht_cap = TRUE;
    wlc_prb_rqst_param_t *req_param = (wlc_prb_rqst_param_t*)param;
    ...
    if (req_param) {
        ftcbparm.bcn.ssid = &req_param->ssid;
        ftcbparm.bcn.short_ssid_len = req_param->short_ssid_len;
        if (ftcbparm.bcn.short_ssid_len) {
            if ((ftcbparm.bcn.short_ssid = (uint32*)MALLOCZ(wlc->osh,
                req_param->short_ssid_len)) != NULL) {
                    memcpy(ftcbparm.bcn.short_ssid, req_param->short_ssid,
                    req_param->short_ssid_len);
            } else {
    ...
    cbparm.ht = (BSS_N_ENAB(wlc, bsscfg) && ht_cap && (cbparm.bandunit != BAND_6G_INDEX)) ?
        TRUE : FALSE;
#ifdef WL11AC
    cbparm.vht = (BSS_VHT_ENAB(wlc, bsscfg) && ht_cap && (cbparm.bandunit != BAND_6G_INDEX)) ?
        TRUE : FALSE;
#endif /* WL11AC */
#ifdef WL11AX
    ...
    *len = body_len;
exit:
    if (req_param && ftcbparm.bcn.short_ssid) {
        MFREE(bsscfg->wlc->osh, ftcbparm.bcn.short_ssid, req_param->short_ssid_len);
    }
    return;
} /* wlc_bcn_prb_body */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
uint16
wlc_backhaul_bss_assocscb_getcnt(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
    struct scb *scb = NULL;
    uint16 assoc = 0;
    struct scb_iter scbiter;
    ...
    FOREACH_BSS(wlc, idx, bsscfg) {
        FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
            if ((bsscfg->map_attr & MAP_EXT_ATTR_BACKHAUL_BSS) && scb->bsscfg == cfg
                && !SCB_DWDS(scb) && SCB_ASSOCIATED(scb)) {
                assoc++;
            }
        }
    }
    ...
    uint8 **pbody, uint iv_len, uint tail_len)
{
    ...
    *pbody = (uint8*)&hdr[1] + iv_len;
    ...
    if (fc == FC_ACTION || fc == FC_PROBE_REQ || fc == FC_PROBE_RESP) {
        ...
        WL_P2P(("wl%d: prep %s frame tx to "MACF", pkt %p\n",
            WLCWLUNIT(wlc), frmtype_str,
            ETHER_TO_MACF(hdr->da), OSL_OBFUSCATE_BUF(p)));
    }
    WL_WSEC(("wl%d: %s: allocated %u bytes; iv %u and tail %u bytes\n",
        WLCWLUNIT(wlc), __FUNCTION__, (wlc->txhroff + len), iv_len, tail_len));
#endif /* WLP2P && BCMDBG */
...
*/
static uint8
wlc_ctrlmgmt_pkt_prec(struct dot11_header *hdr)
{
    uint16 fk;
    ...
    if (fk == FC_ACTION) {
        uchar *ba_data;
        struct dot11_management_header *m_hdr = (struct dot11_management_header *)hdr;
        ba_data = ((uint8 *)&m_hdr[1]);

        WL_INFORM(("action frame: pbody %p, m_hdr[1] %p, category %d, action %d\n",
            hdr, &m_hdr[1], ba_data[0], ba_data[1]));
...

{
    ...
    if (!wlc_ctrlmgmt_pkt_prec(wlc_pkt_get_d11_hdr(wlc, p))) {
     ...
     */
    if (BSSCFG_AP(bsscfg) &&
        ((SCB_TXMOD_ACTIVE(scb, TXMOD_APPS) && !(WLPKTTAG(p)->flags & WLF_PSDONTQ)) ||
        SCB_TWTPS(scb))) {
    ...
    if (cpktq_prec_enq(wlc, &qi->cpktq, p, prec, FALSE)) {
        if (!enq_only) {
            wlc_send_q(wlc, qi);
        }
        return TRUE;
...
#ifdef WL_DIAG_CAPTURE_TXRX_FRAMES
void
wlc_diag_mgmt_ctl(wlc_info_t *wlc, int evt, uint16 fc, wlc_bsscfg_t *bsscfg, void *hdr, int len)
{
    if (DIAG_CAPTURE_NONE(wlc)) {
        return;
    }
    else {
        bool diag_evt = FALSE;
        if (DIAG_CAPTURE_MGMT(wlc) && (FC_TYPE(fc) == FC_TYPE_MNG)) {
            diag_evt = TRUE;
        } else if (DIAG_CAPTURE_ALL(wlc) &&
            (FC_TYPE(fc) == FC_TYPE_MNG || FC_TYPE(fc) == FC_TYPE_CTL)) {
            diag_evt = TRUE;
        }
        if (diag_evt) {
            wlc_bss_mac_event(wlc, bsscfg, evt,
                &((struct dot11_management_header *)hdr)->da,
                    fc&FC_KIND_MASK, 0, 0, (uint8 *)hdr, len);
        }
    }
}
#endif /* WL_DIAG_CAPTURE_TXRX_FRAMES */
    ...
    wlc_bsscfg_t *bsscfg, bool short_preamble, wlc_key_t *key, ratespec_t rate_override)
{
...
#ifdef MFP
    /* setup key for MFP */
    if (WLC_MFP_ENAB(wlc->pub) && WLPKTFLAG_MFP(WLPKTTAG(p)) && key == NULL) {
    ...
    if (FC_TYPE(ltoh16(hdr->fc)) == FC_TYPE_MNG) {
        ...
        /* Reset pairwise key pending time to allow deauth/disassoc frames to go out */
        if ((scb->dropblock_dur > 0) && (fk == FC_DISASSOC || fk == FC_DEAUTH)) {
            WL_ASSOC((" wl%d %s "MACF" fk 0x%x reseting scb dropblock_dur %d\n",
                    wlc->pub->unit, __func__, ETHER_TO_MACF(scb->ea), fk,
                    scb->dropblock_dur));
            scb->dropblock_dur = 0;
        }
    }
...
#endif /* MFP */
    wlc_diag_mgmt_ctl(wlc, WLC_E_CAPTURE_TXFRAME, ltoh16(hdr->fc),
        bsscfg, hdr, PKTLEN(wlc->osh, p));
...
void
wlc_sendprobe_with_sh_ssid(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
    const uint8 ssid[], int ssid_len, const struct ether_addr *sa,
    const struct ether_addr *da, const struct ether_addr *bssid,
    ratespec_t ratespec_override, uint8 *extra_ie, int extra_ie_len)
{
    uint8 dummy_ssid[11] = "DUMMY_SSID";
    uint8 sh_ssid[SHORT_SSID_LEN] = {0};
    uint32 *short_ssid = (uint32 *)sh_ssid;
    uint8 n_short_ssid = SHORT_SSID_LEN/sizeof(uint32);
    memcpy(short_ssid, ssid, SHORT_SSID_LEN);
    ssid_len = (sizeof(dummy_ssid) - 1);

    wlc_sendprobe(wlc, bsscfg, dummy_ssid,
        ssid_len, n_short_ssid, short_ssid,
        sa, da, bssid, ratespec_override, NULL, 0);
    ...
    ratespec_t ratespec_override, uint8 *extra_ie, int extra_ie_len)
{
    ...
    cbparm.ht = BSS_N_ENAB(wlc, bsscfg) && (BSSCFG_BANDUNIT(bsscfg) != BAND_6G_INDEX);
#ifdef WL11AC
    cbparm.vht = BSS_VHT_ENAB(wlc, bsscfg) && (BSSCFG_BANDUNIT(bsscfg) != BAND_6G_INDEX);
#endif /* WL11AC */
#ifdef WL11AX
    cbparm.he = BSS_HE_ENAB_BAND(wlc, wlc->bandstate[cbparm.bandunit]->bandtype, bsscfg) ?
            TRUE : FALSE;
...
#ifdef BCM_CSIMON_AP
    if (CSIMON_ENAB(wlc->pub)&& CSIMON_ENAB_AP(wlc->pub)) {
        if (wlc_csimon_ap_ssid_len_check(wlc->band->hwrs_scb) != 0) {
            WLPKTTAG(pkt)->flags  |= WLF_CSI_NULL_PKT;
        }
    }
#endif
...
#endif
#ifdef WL_OCE
        if (OCE_ENAB(wlc->pub) && BSS_OCE_ENAB(wlc, bsscfg))
    ...
    void *tx_cplt_ctx)
{
    ...
    if (CEVENT_STATE(wlc->pub)) {
        wlc_send_cevent(wlc, cfg, SCB_EA(scb), (uint16)auth_status, 0, 0, NULL, 0,
            CEVENT_D2C_MT_AUTH_TX, CEVENT_D2C_FLAG_QUEUED | CEVENT_FRAME_DIR_TX);
    }
#endif /* BCM_CEVENT */
    ...
    void *arg, void *extra_arg)
{
    ...
    if (CEVENT_STATE(wlc->pub)) {
        wlc_send_cevent(wlc, cfg, SCB_EA(scb), 0,
                reason_code, 0, NULL, 0, CEVENT_D2C_MT_DISASSOC_TX,
                CEVENT_D2C_FLAG_QUEUED | CEVENT_FRAME_DIR_TX);
    }
#endif /* BCM_CEVENT */

#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
    td_em_client_event(__FUNCTION__, __LINE__, (void *)cfg,cfg->cur_etheraddr.octet, 
                    scb->ea.octet, TD_EASYMESH_CLIENT_LEAVE , reason_code, 0);
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    if(!TDCORE_FUNC_IS_NULL(td_em_client_event)){
        TDCORE_FUNC(td_em_client_event)((void *)cfg, (char *)cfg->cur_etheraddr.octet, (char *)scb->ea.octet, TD_EASYMESH_CLIENT_LEAVE , reason_code, 0);
    }
#endif
#endif
    ...
    int (*pre_send_fn)(wlc_info_t*, wlc_bsscfg_t*, void*, void *), void *data)
{
    ...
    WL_TRACE(("wl%d: %s: DA "MACF" rspec 0x%08X pktflags 0x%08X prio %d bsscfg %p\n",
        wlc->pub->unit, __FUNCTION__,
        ETHERP_TO_MACF(da), rate_override,
    ...
    if ((wlc->lifetime[WME_PRIO2AC(PKTPRIO(p))])) {
    ...
    WL_PS_EX(scb, ("sending null frame\n"));
...
{
    WL_TRACE(("wl%d: %s: DA "MACF" rspec 0x%08X prio %d cfg %p\n", wlc->pub->unit, __FUNCTION__,
        ETHERP_TO_MACF(da), rate_override, prio, OSL_OBFUSCATE_BUF(cfg)));
...
wlc_sendpspoll(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
    ...
    if (!(scb = wlc_scbfind(wlc, bsscfg, bssid))) {
        WL_ERROR(("wl%d: %s: wlc_scbfind failed, BSSID "MACF" bandunit %d\n",
            WLCWLUNIT(wlc), __FUNCTION__, ETHERP_TO_MACF(bssid),
            wlc->band->bandunit));
        return FALSE;
    }
...
wlc_sendpspoll_complete(wlc_info_t *wlc, void *pkt, uint txstatus)
{
    ...
    if ((txstatus & TX_STATUS_MASK) == TX_STATUS_NO_ACK) {
        ...
        WL_ERROR(("wl%d: %s: no ACK from "MACF" for PS-Poll\n",
...
#ifdef WLDEAUTH_INTRANSIT_FILTER
static void
wlc_hwrs_deauth_callback(wlc_info_t *wlc, uint txstatus, void *pkt)
{
    struct dot11_management_header *hdr;
    wlc_bsscfg_t *bsscfg;
    int err = 0;
    ...
    bsscfg = wlc_bsscfg_find(wlc, WLPKTTAGBSSCFGGET(pkt), &err);

    hdr = (struct dot11_management_header*)wlc_pkt_get_d11_hdr(wlc, pkt);

    /* in case bsscfg is freed before this callback is invoked */
    if (bsscfg == NULL) {
        WL_ERROR(("wl%d: %s: missing bsscfg for "MACF"\n",
            wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(hdr->da)));
        ASSERT(bsscfg != NULL);
        return;
    }

    wlc_ap_intransit_filter_rem_da(wlc->ap, bsscfg, &hdr->da);
}
#endif /* WLDEAUTH_INTRANSIT_FILTER */
    ...
    bool use_cb_fn)
{
...
#ifdef WLDEAUTH_INTRANSIT_FILTER
    if (BSSCFG_AP(cfg)) {
        if (wlc_ap_intransit_filter_check_da(wlc->ap, cfg, da)) {
            return NULL;
        }
    }
#endif /* WLDEAUTH_INTRANSIT_FILTER */
    ...
    if (CEVENT_STATE(wlc->pub) && scb && !SCB_INTERNAL(scb)) {
        wlc_send_cevent(wlc, cfg, SCB_EA(scb), 0,
                reason_code, 0, NULL, 0, CEVENT_D2C_MT_DEAUTH_TX,
                CEVENT_D2C_FLAG_QUEUED | CEVENT_FRAME_DIR_TX);
    }
#endif /* BCM_CEVENT */
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
...
#ifdef MFP
        /* setup key for MFP */
        if (scb && WLC_MFP_ENAB(wlc->pub) && WLPKTFLAG_MFP(WLPKTTAG(pkt))) {
...
#ifdef WLDEAUTH_INTRANSIT_FILTER
    /* In case hwrs scb is used, add the DA to the intransit filter so that there
     * will only be one deauth frame pending at a time.
     */
    if (BSSCFG_AP(cfg) && (scb == wlc->band->hwrs_scb)) {
        if (!wlc_ap_intransit_filter_add_da(wlc->ap, cfg, da)) {
            WL_ERROR(("wl%d: %s: wlc_ap_intransit_filter_add_da failed\n",
                wlc->pub->unit, __FUNCTION__));
        } else {
#ifdef CONFIG_TENDA_PRIVATE_WLAN
            hwrs_scb_flag = 1;
            ...
            wlc_pcb_fn_register(wlc->pcb, wlc_hwrs_deauth_callback,
                (void *)pkt, pkt);
        }
    }
#endif /* WLDEAUTH_INTRANSIT_FILTER */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
        if (1 == hwrs_scb_flag) {
            WL_ASSOC(("[%s:%d] wl%d.%d pkt_idx %d\n", __func__, __LINE__, wlc->pub->unit, WLC_BSSCFG_IDX(cfg), WLPKTTAGBSSCFGGET(pkt)));
            WLPKTTAGBSSCFGSET(pkt, WLC_BSSCFG_IDX(cfg));
        }
#endif
        WL_ASSOC(("wl%d.%d: %s: station "MACF" has been notified of deauthentication %s\n",
            wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__,
            ETHERP_TO_MACF(da), scb != wlc->band->hwrs_scb?"":"using hwrs_scb"));
        return pkt;
    }
...
wlc_sendassocreq(wlc_info_t *wlc, wlc_bss_info_t *bi, struct scb *scb, bool reassoc)
{
    ...
    cbparm.ht = N_ENAB(wlc->pub) && SCB_HT_CAP(scb) &&
        (BSSCFG_BANDUNIT(bsscfg) != BAND_6G_INDEX);
#ifdef WL11AC
...
#ifdef WL11AC
    bi_orig = wlc_assoc_get_join_target(wlc, 0);
    if (cbparm.vht && bi_orig != NULL &&
            (wf_bw_chspec_to_half_mhz(bi_orig->chanspec) >
             wf_bw_chspec_to_half_mhz(wlc->chanspec))) {
...
wlc_sta_info_upd_scb_median_rate_stats(const wlc_info_t *wlc, const wlc_bsscfg_t *bsscfg,
    wlc_scb_stats_t *scb_stats, wlc_scb_rate_stats_t *scb_rate_stats, sta_info_t *sta)
{
    /* use up median rate to update scb rate specs */
    scb_stats->rx_rate = wlc_scb_get_median_rate(scb_rate_stats->rx_rate_log, NRATELOG);
    scb_stats->tx_rate = wlc_scb_get_median_rate(scb_rate_stats->tx_rate_log, NRATELOG);
}
#endif /* WLC_MEDIAN_RATE_REPORT */
    ...
    const wlc_scb_stats_t *scb_stats, sta_info_t *sta)
{
    ...
    rate = RSPEC2KBPS(scb_stats->tx_rate);
    rate = (rate >= 0)? rate : 0;
    sta->tx_rate = rate;
    sta->tx_rate_fallback = RSPEC2KBPS(scb_stats->tx_rate_fallback);

    rate = RSPEC2KBPS(scb_stats->rx_rate);
    rate = (rate >= 0)? rate : 0; /* RSPEC2KBPS can return -1 */
    sta->rx_rate = rate;
...
#ifdef GAME_SPEEDUP
    sta->rts_tx_cnt = scb_stats->rts_tx_cnt;
    sta->rts_failed_cnt = scb_stats->rts_failed_cnt;
    sta->ps_cnt = scb_stats->ps_cnt;
#endif
}
...
#ifdef WLCNTSCB
#ifdef WLC_MEDIAN_RATE_REPORT
    scb->scb_stats.tx_rate = wlc_scb_get_median_rate(scb->scb_rate_stats.tx_rate_log, NRATELOG);
    scb->scb_stats.rx_rate = wlc_scb_get_median_rate(scb->scb_rate_stats.rx_rate_log, NRATELOG);
#else /* WLC_MEDIAN_RATE_REPORT */
                        ...
                        td_sta_info_t *td_sta)
{
    ...
    bcopy(ea, td_sta->ether_addr_octet, ETHER_ADDR_LEN);
    for (i = WL_ANT_IDX_1; i < WL_RSSI_ANT_MAX; i++) {
        rssi[i] = wlc_lq_ant_rssi_get(wlc, SCB_BSSCFG(scb), scb, i);
        if ((rssi[i] < 0) && (rssi[i] > max_rssi)) {
            max_rssi = rssi[i];
        }
    }
    td_sta->rssi = (int)max_rssi;
    td_sta->sup11k = SCB_RRM(scb);
    td_sta->sup11v = wlc_wnm_get_scbcap(wlc, scb);
    td_sta->last_time = scb->last_time;
    strncpy(td_sta->ifname, wl_ifname(scb->bsscfg->wlc->wl, scb->bsscfg->wlcif->wlif), sizeof(td_sta->ifname) - 1);
    td_sta->ifname[sizeof(td_sta->ifname) - 1] = '\0';
    return BCME_OK;
}
#endif
    ...
    void *buf, int len)
{
    ...
    if (SCB_WRONG_KEY(scb)) {
        sta.flags |= WL_STA_WRONG_KEY;
    }
#endif
    ...
    if (SCB_SPP_AMSDU(scb)) {
        sta.flags |= WL_SPP_AMSDU_CAP;
    }
#endif /* WL_SPP_AMSDU */
#ifdef WLC_DTPC
    if (SCB_DTPC_CAP(scb)) {
        sta.flags |= WL_STA_DTPC_CAP;
    }
#endif /* WLC_DTPC */
...
#ifdef WLCNTSCB
#ifdef  WLC_MEDIAN_RATE_REPORT
        /* update sta last packet rates with median rate stats */
        wlc_sta_info_upd_scb_median_rate_stats(wlc, bsscfg, &scb->scb_stats,
                &scb->scb_rate_stats, &sta);
#endif /* WLC_MEDIAN_RATE_SUPPORT */
...
#endif /* WL11K_AP */
    bands = wlc_ap_scb_supported_bands(wlc, scb);
    /* Add band of current Radio to supported bands */
    bands |= (1 << wlc->band->bandunit);
    for (bandunit = 0; bandunit < MAXBANDS; bandunit++) {
        if (bands & (1 << bandunit)) {
            nband++;
            sta.bands[nband] = wlc_bandunit2bandtype(bandunit);
        }
    }
    sta.bands[0] = nband;
...
#ifdef GAME_SPEEDUP
   scb->scb_stats.rts_tx_cnt = scb->scb_stats.rts_failed_cnt = scb->scb_stats.ps_cnt = 0;
#endif
...
{
    if (wlc_bmac_detect_and_handle_fifoerrors(wlc->hw)) {
        WL_HEALTH_LOG(wlc, DESCRIPTOR_ERROR);
        wlc_fatal_error(wlc);   /* big hammer */
    }
}
...
wlc_iov_get_wlc_ver(wl_wlc_version_t *ver)
{
    ...
    ver->tx_pwr_rpt_version = TX_POWER_T_VERSION;
}
...
wlc_8023_etherhdr(wlc_info_t *wlc, osl_t *osh, void *p)
{
    ...
    if (PKTLEN(osh, p) < (DOT11_LLC_SNAP_HDR_LEN + ETHER_HDR_LEN))
        return;
    ...
    if (enable) {
        ...
        if (flag == AUTHORIZED) {
            ...
            if (wlc->pub->up && (SCB_AUTHENTICATED(scb) || SCB_LEGACY_WDS(scb)) &&
                !scb->sent_deauth) {
                if (BSSCFG_AP(bsscfg) && (bsscfg->WPA_auth != WPA_AUTH_DISABLED) &&
                        (bsscfg->WPA_auth != WPA_AUTH_NONE) &&
                        (!SCB_AUTHORIZED(scb))) {
                 ...
                 */
                if (BSSCFG_AP(bsscfg) && (rc == DOT11_RC_STALE_DETECTION)) {
                     ...
                     */
                    wlc_bss_mac_event(wlc, bsscfg, WLC_E_DEAUTH,
                        (struct ether_addr *)&scb->ea,
                        WLC_E_STATUS_SUCCESS, rc, 0, 0, 0);
...
wlc_process_multiap_ie(wlc_info_t *wlc, struct scb *scb, multiap_ie_t *multiap_ie, uint16 ft)
{
    ...
    /* Get the BSS's map_profile */
    bsscfg_map_profile = scb->bsscfg ? scb->bsscfg->map_profile : 0;
    ...
    for (tlv = (bcm_tlv_t *)multiap_ie->attr; tlv != NULL;
        ...
        if (tlv->id  == MAP_EXT_ATTR) {
            ...
            if (map_profile && (map_profile->map_profile == MAP_PROFILE_2) &&
                (bsscfg_map_profile >= MAP_PROFILE_2)) {
                scb->flags3 |= SCB3_MAP_P2_CAP;
            }
...
enum wlc_bandunit
wlc_bandtype2bandunit_noassert(int bandtype)
{
    int i;
...
{
    enum wlc_bandunit bu = wlc_bandtype2bandunit_noassert(bandtype);
...
enum wlc_bandunit
wlc_chspecband2bandunit_noassert(uint16 chspec_band)
{
    int i;

    for (i = 0; i < MAXBANDS; i++) {
        if (band_unit_conv[i].chspec_band == chspec_band)
            break;
    }
    return i;
}
...
enum wlc_bandunit
wlc_chspecband2bandunit(uint16 chspec_band)
{
    enum wlc_bandunit bu = wlc_chspecband2bandunit_noassert(chspec_band);
...
#endif /* WLATM_PERC) */
#ifdef BCMPCIEDEV
/* Used by BUS layer when TAF is OFF.
 * Part of Fairfetch scheduling where BUS layer
 * does a fair split of available TX buffers.
 */
static int16
wlc_flow_ring_get_maxpkts_for_link(wlc_info_t *wlc, struct scb *scb)
{
    uint16  rate = 0xff;
    ratespec_t rspec;
    ...
    /* return raw datarate in units of 500 Kbit/s */
 ...
 */
int
wlc_bus_scb_link_update(wlc_info_t *wlc, struct wlc_if *wlcif,
...
#if BAND6G || defined(WL_OCE_AP)
uint8
wlc_get_apcfg_idx_used_for_upr_fd_frame(wlc_info_t *wlc)
{
    wlc_bsscfg_t *apcfg = NULL;
    int i = 0, idx = -1;

    if (wlc->upr_fd_info->cfg) {
        /* already configured cfg in wlc_ap.c for UCODE or SW UPR/FD. */
        return WLC_BSSCFG_IDX(wlc->upr_fd_info->cfg);
    }
    /* if not, return first AP up index */
...
wlc_update_upr_fd_template(wlc_info_t *wlc, bool suspend)
{
    ...
    if (!wlc->main_ap_bsscfg) {
        WL_INFORM(("wl%d TX BSS not set, return\n", wlc->pub->unit));
        return;
    }

    if (!wlc->main_ap_bsscfg->up) {
        WL_INFORM(("wl%d:%d TX BSS not UP, return\n", wlc->pub->unit,
            WLC_BSSCFG_IDX(wlc->main_ap_bsscfg)));
        return;
    }

    if (isset(&(wlc->nbr_discovery_cap), WLC_NBR_DISC_CAP_20TU_BCAST_PRB_RSP)) {
        psc_chan = wf_chspec_primary20_chspec(wlc->main_ap_bsscfg->current_bss->chanspec);
        if (!WF_IS_6G_PSC_CHAN(psc_chan)) {
            WL_ERROR(("wl%d:%d UPR not enabled on NON PSC channel\n", wlc->pub->unit,
            WLC_BSSCFG_IDX(wlc->main_ap_bsscfg)));
    ...
    wlc_bss_update_upr_fd_template(wlc, wlc->main_ap_bsscfg, suspend);
}

/* should be called with wrapper wlc_update_upr_fd_template */
void
wlc_bss_update_upr_fd_template(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool suspend)
{
    ...
    if (send_bcast_prb_rsp) {
        ...
        if (OCE_ENAB(wlc->pub)) {
            wlc_prepare_fd_template(wlc, FC_ACTION, wlc->bcn_rspec, cfg,
                prb_resp, &len);
        }
    }
#endif
    if (MBSS_ENAB(wlc->pub)) {
        ...
        if (D11REV_GE(wlc->pub->corerev, 40)) {
            ...
            /*  rounding of the offset to next buffer block unit
             *  which is multiple of 512
             *  Each buffer is of 512 bytes in length.
             *  But three blocks are combined to form a unit.
             *  bcn/upr should start and end in the same unit.
             *  there is a overlap of buffers in units
             *  unit 0: 0   - 1536
             *  unit 1: 512 - 2047
             *  unit 2: 1024- 2559
             *  unit 3: 1536- 3071
             ...
             *  |
             *  |
             *  uint n: 512 *n - ((512*n) + 1536)
             */
            blk_cnt = (uint16)(offset / 512);
            blk_offset = (uint16)(offset % 512);
            if (blk_offset) {
                offset = 512 * (blk_cnt + 1);
            }
        ...
        uint16 *buf, int *len)
{
    ...
    frame_len = wlc_oce_get_fd_frame_len(wlc->oce, cfg);
