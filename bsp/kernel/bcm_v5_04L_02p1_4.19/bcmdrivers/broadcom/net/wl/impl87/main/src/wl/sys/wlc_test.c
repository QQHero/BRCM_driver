 ...
 *
 * $Id: wlc_test.c 808078 2022-02-09 01:06:52Z $
 */
...
wlc_nvram_dump(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	int page_len = 0;
	int err = BCME_OK;
	bool dump_nvram = (wlc->pub->vars == NULL); /* avoid reading unnecessary (host) data */
#ifdef DONGLEBUILD
	dump_nvram = TRUE; /* FD doesn't access host data so always dump as much as possible */
#endif /* DONGLEBUILD */
	...
	if (dump_nvram) {
		...
		if ((nvram_vars = MALLOCZ(wlc->osh, MAXSZ_NVRAM_VARS)) == NULL) {
			err = BCME_NOMEM;
			goto exit;
		}
...
#define WAIT_COUNT_XI	100	/* wait count for tx interrupt */
	...
	if (STS_XFER_PHYRXS_ENAB(wlc->pub)) {
		wlc_sts_xfer_bmac_recv(wlc, RX_FIFO, &rx_list, 0);
	}
#endif /* STS_XFER_PHYRXS */
...
wlc_test_pkteng_addsta(wlc_test_info_t *test, int usridx)
{
	...
	/* Clear the auto-allocated aid, avoid aid leak */
	wlc_test_clear_aid(wlc, scb);
	scb->aid = usr->aid;
	...
	wl_pkteng_cmd_params_t* params)
{
	...
	} else if (!strncmp(keystr, "rxwithack", strlen("rxwithack"))) {
		val16 = bcm_strtoul(valstr, NULL, 0);
		if (val16 != 0) {
			uint16 mfgtest_flag = 0;
			mfgtest_flag = wlc_read_shm(wlc, M_MFGTEST_NUM(wlc));
			wlc_write_shm(wlc, M_MFGTEST_NUM(wlc), (mfgtest_flag |
				MFGTEST_RXMODE_ACK));
		}
