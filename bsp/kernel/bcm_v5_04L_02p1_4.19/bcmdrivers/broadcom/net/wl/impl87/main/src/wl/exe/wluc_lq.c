 ...
 *
 * $Id: wluc_lq.c 807758 2022-01-31 09:38:36Z $
 */
	...
	if ((count == 1) && ((req_count == WL_CHANIM_COUNT_US_ONE) ||
		...
		if (version < WL_CHANIM_STATS_VERSION_3) {
			...
			if (raw_chanspec) {
				...
				wf_chspec_ntoa(dtoh16(stats_us->chanspec), chanspec_string);
			...
			if (us_ver == WL_CHANIM_STATS_US_VERSION_1) {
				...
				if (raw_chanspec) {
					...
					wf_chspec_ntoa(dtoh16(stats_us_v1->chanspec),
						chanspec_string);
					printf("%-9s ", chanspec_string);
				}
				...
				if (raw_chanspec) {
					...
					wf_chspec_ntoa(dtoh16(stats_us_v2->chanspec),
						chanspec_string);
					printf("%-9s ", chanspec_string);
				}
				...
				printf("Sorry, your driver has wl_chanim_stats us version %d "
		...
		for (i = 0; i < count; i++) {
			if (us_ver == WL_CHANIM_STATS_US_VERSION_1) {
				...
				if (raw_chanspec) {
					printf("0x%-8x  ",
						dtoh16(list_v2->stats_us_v2[i].chanspec));
				} else {
					wf_chspec_ntoa(dtoh16(list_v2->stats_us_v2[i].chanspec),
						chanspec_string);
					printf("%-11s ", chanspec_string);
				}
...
static void wl_chanim_stats_print_all(wl_chanim_stats_t *list)
{
	...
	if (list->version == WL_CHANIM_STATS_VERSION_4) {
		...
		elmt_size = sizeof(chanim_stats_v2_t);
	} else {
		return;
	}
	...
	nstats = (uint8 *)list->stats;
	stats = list->stats;

	for (i = 0; i < list->count; i++) {
		printf(" chanspec: 0x%x crsglitch cnt: %d bad plcp: %d noise: %d\n",
			dtoh16(stats->chanspec), dtoh32(stats->glitchcnt),
			dtoh32(stats->badplcp), stats->bgnoise);
		...
		printf("\t cca_txdur: %d cca_inbss: %d cca_obss:"
			...
			stats->ccastats[CCASTATS_OBSS],
			stats->ccastats[CCASTATS_NOCTG],
			stats->ccastats[CCASTATS_NOPKT]);
...
static void
wl_chanim_stats_print(void *ptr, uint32 req_count, uint decimal, bool raw_chanspec)
{
	...
	if (list->version == WL_CHANIM_STATS_VERSION_4) {
		if (list->count == 1 && req_count == WL_CHANIM_COUNT_ONE) {
			...
			printf("chanspec tx    inbss    obss    nocat   nopkt   doze     txop   "
				...
				"timestamp\n");
			if (raw_chanspec) {
				printf("0x%4x\t ", dtoh16(stats_v4->chanspec));
			} else {
			...
			for (i = 0; i < CCASTATS_MAX; i++) {
				if (stats_v4->acc_ms) {
					percentage = (float)dtoh32(stats_v4->acc_ccastats[i])
						/ (float)(dtoh32(stats_v4->acc_ms)*10);
				...
				printf("%.*f\t", decimal, percentage);
			}
			if (stats_v4->acc_ms) {
				unsigned int busy_time = 0;

				for (i = CCASTATS_TXDUR; i <= CCASTATS_NOPKT; ++i) {
					busy_time += dtoh32(stats_v4->acc_ccastats[i]);
				}

				percentage = (float)busy_time /
						(float)(dtoh32(stats_v4->acc_ms)*10);
			} else {
				percentage = 0.0;
			}

			printf("%d\t%d\t%d\t%d\t%.*f\t%d", dtoh32(stats_v4->glitchcnt),
				dtoh32(stats_v4->badplcp), stats_v4->bgnoise,
				stats_v4->chan_idle, decimal, percentage,
				dtoh32(stats_v4->timestamp));
		} else if (list->count >= 1 && req_count == WL_CHANIM_COUNT_ALL) {
			wl_chanim_stats_print_all(list);
		}
	} else if (list->version == WL_CHANIM_STATS_VERSION_3) {
		if (list->count == 1 && req_count == WL_CHANIM_COUNT_ONE) {
			chanim_stats_v3_t *stats_v3 = (chanim_stats_v3_t *)list->stats;
...
static int wl_chanim_stats_us_dur(void *wl, cmd_t *cmd, char **argv, int version, bool raw_chanspec)
{
	int err;
	...
	wl_chanim_stats_us_print(ptr, param_us.count, version, raw_chanspec);
	return err;
...
wl_chanim_stats(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv != NULL) {
		if (!strcmp(*argv, "us")) {
			...
			wl_chanim_stats_us_print(ptr, param.count, version, raw_chanspec);
			return 0;
		} else {
			param.buflen = htod32(sizeof(wl_chanim_stats_t));
	...
	if (!((stats->version >= WL_CHANIM_STATS_VERSION_2) &&
		(stats->version <= WL_CHANIM_STATS_VERSION_4))) {
		printf("Sorry, your driver has wl_chanim_stats version %d "
			"but this program supports only version %d, %d and %d.\n",
			stats->version, WL_CHANIM_STATS_VERSION_2,
			WL_CHANIM_STATS_VERSION_3, WL_CHANIM_STATS_VERSION_4);
		free(stats);
		return 0;
	}
	...
	wl_chanim_stats_print(ptr, param.count, decimal, raw_chanspec);
	free(stats);
	return (err);
}
