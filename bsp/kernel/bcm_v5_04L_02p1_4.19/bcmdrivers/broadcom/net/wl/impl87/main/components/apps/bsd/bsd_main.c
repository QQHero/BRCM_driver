...
/* condition met: return BSD_OK	(0) otherwise BSD_FAIL (-1) */
#ifndef STRING_LENGTH_32
#define STRING_LENGTH_32 32
#endif /* STRING_LENGTH_32 */
static int bsd_check_condition(void)
{
	char *ifname, *p_akm, *p_psk, *p_radiuskey, *p_radius2key;
	char nv_name[STRING_LENGTH_32] = {0}, *nv_value = NULL, bsd_ifnames[STRING_LENGTH_32];
	int num_tokens = 0;

	if ((nv_value = nvram_get("bsd_ifnames")) == NULL) {
		BSD_ERROR("%s BandSteering bsd_ifnames not defined\n", __FUNCTION__);
		return BSD_FAIL;
	}

	/* format like "wl0 wl1" */
	strncpy(bsd_ifnames, nv_value, sizeof(bsd_ifnames) - 1);
	bsd_ifnames[sizeof(bsd_ifnames) - 1] = 0;
	ifname = strtok(bsd_ifnames, " ");
	if (ifname) {
		++ num_tokens;
		/* Store the values for first ifname */
		snprintf(nv_name, sizeof(nv_name), "%s_akm", ifname);
		p_akm = nvram_safe_get(nv_name);

		snprintf(nv_name, sizeof(nv_name), "%s_wpa_psk", ifname);
		p_psk = nvram_safe_get(nv_name);

		snprintf(nv_name, sizeof(nv_name), "%s_radius_key", ifname);
		p_radiuskey = nvram_safe_get(nv_name);

		snprintf(nv_name, sizeof(nv_name), "%s_radius2_key", ifname);
		p_radius2key = nvram_safe_get(nv_name);
	} else {
		BSD_ERROR("%s %d, nvram for bsd_ifnames not set\n",
			__FUNCTION__, __LINE__);
		return BSD_FAIL;
	}

	while (1) {
		/* Get the next ifname from bsd_ifnames */
		ifname = strtok(NULL, " ");
		if (!ifname) {
			if (num_tokens < 2) {
				BSD_ERROR("%s %d, bsd_ifnames should have 2 or more interfaces\n",
					__FUNCTION__, __LINE__);
				return BSD_FAIL;
			} else {
		...
		snprintf(nv_name, sizeof(nv_name), "%s_akm", ifname);
		if (strcmp(nvram_safe_get(nv_name), p_akm)) {
			BSD_ERROR("%s %d, Authentication Mismatch\n", __FUNCTION__, __LINE__);
			return BSD_FAIL;
		}

		if (strstr(p_akm, "psk")) {
			snprintf(nv_name, sizeof(nv_name), "%s_wpa_psk", ifname);
			if (strcmp(nvram_safe_get(nv_name), p_psk)) {
				BSD_ERROR("%s %d, Key Mismatch\n", __FUNCTION__, __LINE__);
				return BSD_FAIL;
			}
		}
		...
		if (strstr(p_akm, "wpa")) {
			snprintf(nv_name, sizeof(nv_name), "%s_radius_key", ifname);
			if (strcmp(nvram_safe_get(nv_name), p_radiuskey)) {
				BSD_ERROR("%s %d, Radius Key Mismatch\n", __FUNCTION__, __LINE__);
				return BSD_FAIL;
			}

			snprintf(nv_name, sizeof(nv_name), "%s_radius2_key", ifname);
			if (strcmp(nvram_safe_get(nv_name), p_radius2key)) {
				BSD_ERROR("%s %d, Radius2 Key Mismatch\n", __FUNCTION__, __LINE__);
				return BSD_FAIL;
			}
		}
	}
...
/* dump_flag_qqdx */
	...
	/* bsd_init -> bsd_info_init will check radio, bss_enable and ssid etc.
	   bsd_check_condition will check if all bsd interfaces use same credential
	*/
	val = nvram_safe_get(BSD_AKM_KEY_CHECK_NVRAM);
	check_akm_key = strtoul(val, NULL, 0);
	if ((bsd_info->enable_flag & BSD_FLAG_ENABLED) &&
		check_akm_key && (bsd_check_condition() != BSD_OK)) {
		BSD_ERROR("Err: bsd condition is not met\n");
		goto done;
	}
	...
	while (bsd_info->mode != BSD_MODE_DISABLE) {
		...
		if ((tv.tv_sec == 0 && tv.tv_usec == 0) || count == BSD_WATCHDOG_INTERVAL) {
			count = 0;
			bsd_info->ticks ++;
			tv.tv_sec = bsd_info->poll_interval;
			tv.tv_usec = 0;
