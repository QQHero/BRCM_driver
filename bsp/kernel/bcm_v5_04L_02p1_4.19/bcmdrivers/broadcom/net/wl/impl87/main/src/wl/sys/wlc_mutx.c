/*
 * DL MU-MIMO transmit module for Broadcom 802.11 Networking Adapter Device Drivers,
 * also containing a part of the DL MU-OFDMA (the admission logic) functionality.
 *
 ...
 *
 * $Id: wlc_mutx.c 810525 2022-04-08 10:06:06Z $
 */
...
/** MUMIMO_CLIENT_NRX_MAX: max number of rx streams of remote node that this node can support */
#define MUMIMO_CLIENT_NRX_MAX   2
#define MUMIMO_CLIENT_NRX_1 1
#define MUMIMO_CLIENT_NRX_2 2

/* Maximum VHT MCS index. MCS 0 - 11, up to 4 streams */
#define MUTX_MCS_NUM  12
#define MUTX_NSS_NUM   4
#define MUTX_MCSNSS_STORE_ELMTS  (MUTX_MCS_NUM*MUTX_NSS_NUM)
...
/** macro to map from DL-MUMIMO subtype to an index */
#define TP_TO_IDX(mutx_tp) ((mutx_tp == HEMMU) ? 1 : 0)

#define MUTXPKTPENDTOT(wlc) (TXPKTPENDTOT(wlc) - TXPKTPENDGET(wlc, ULMU_TRIG_FIFO))

/* MU-MIMO Transmit Policy */
typedef struct mutx_dl_mumimo_policy {
...
enum wlc_mutx_iov {
	...
	IOV_MU_GROUP_BY_BSS = 14,
	IOV_MUTX_ADMIT_MCS_THRESH = 15,
	IOV_MUTX_LAST
};
...
static const bcm_iovar_t mutx_iovars[] = {
	...
	{"mutx_admit_mcs_thresh", IOV_MUTX_ADMIT_MCS_THRESH,
	0, 0, IOVT_UINT32, 0
	},
	{NULL, 0, 0, 0, 0, 0 }
};
...
struct wlc_mutx_info {
	...
	mutx_scb_list_t *mu_plcy_vldn_list[MUTX_TP_MAX][BW_160MHZ];
	/* Max number of muclients */
	scb_t *mu_clients[MUCLIENT_NUM];
	int max_muclients;
	/* MU TX policy - AC and BW - different from mu_policy_t */
	mutx_dl_mumimo_policy_t *mutx_policy;
	bool client_bw160;
	bool _swap_mufifotech;
};

#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
/** this struct gets instantiated on a per-scb basis */
typedef struct mutx_dl_mumimo_stats {
	...
	/* The following arrays contain per-(MCS+NSS) information. In these arrays, no distinction
	 * is made between VHT/HE/EHT MCS'es.
	 */
	/* Below indexed by mcs + ((nss - 1) * mcs_num) */
	/* Total mu frame transmission attempts */
	uint32 txmu[MUTX_MCSNSS_STORE_ELMTS];
	/* Total mu frame transmissions acked */
	uint32 txmu_succ[MUTX_MCSNSS_STORE_ELMTS];
	/* Total mu frames that end up being txed as SU */
	uint32 txsu[MUTX_MCSNSS_STORE_ELMTS];
	/* Total mu frames that end up being txed as SU and acked */
	uint32 txsu_succ[MUTX_MCSNSS_STORE_ELMTS];
	/* Total mu frames Tx'ed with sgi on */
	uint32 txmu_sgi[MUTX_MCSNSS_STORE_ELMTS];
	/* Total mu frame transmissions acked with sgi on */
	uint32 txmu_sgi_succ[MUTX_MCSNSS_STORE_ELMTS];
	uint32 txassu_reason[C_M2SQ_REASON_MAX];
	uint32 snd_fail_cnt;
	bool   snd_epch_prev;
} mutx_dl_mumimo_stats_t;
#endif
...
typedef struct mutx_pfmon_stats {
	...
	uint32 txmu_primrate[MUTX_MCSNSS_STORE_ELMTS];
	uint32 txassu_reason[C_M2SQ_REASON_MAX];
	uint32 snd_fail_cnt;
	bool   snd_epch_prev;
} mutx_pfmon_stats_t;

typedef struct mutx_dl_mumimo_snr_calib_stats {
	int index[MUMIMO_CLIENT_NRX_MAX];
	/* Total mu frame transmission attempts */
	uint32 txmu[MUTX_MCSNSS_STORE_ELMTS];
	uint32 txmu_succ[MUTX_MCSNSS_STORE_ELMTS];
	uint32 txmu_total[MUMIMO_CLIENT_NRX_MAX];
	uint8 count[MUMIMO_CLIENT_NRX_MAX][17];
	bool change[MUMIMO_CLIENT_NRX_MAX];
} mutx_dl_mumimo_snr_calib_stats_t;
#endif /* WLCNT */
...
enum {
	...
	MUTX_REASON_TWTSTA = 14,
	MUTX_REASON_DSMPS = 15,
	MUTX_REASON_OMI_ULMUDIS = 16,
	MUTX_REASON_PPS_LT_THRESH = 17,
	MUTX_REASON_SINGLE_STA_EVICT = 18,
	MUTX_REASON_UNDEFINED = 19,
	MUTX_REASON_MAX = 20
};
...
enum {
	...
	MUTX_POLICY_EVICT_SCORE = 4,
	MUTX_POLICY_EVICT_NOTREADY = 5,
	MUTX_POLICY_EVICT_MAX = 6
};
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
	mutx_dl_mumimo_stats_t *mutx_stats;	/* counters/stats */
#else /* BCMDBG || BCMDBG_DUMP || BCMDBG_DUMP_MUTX */
	void *_unused;
#endif
#ifdef WLCNT
	mutx_pfmon_stats_t *mutx_pfmon_stats;
	mutx_dl_mumimo_snr_calib_stats_t *mutx_snr_calib_stats;
#endif /* WLCNT */
	uint8 retry_gid;
...
#define MU_STA_MAX_HOLD_DUR 60
	...
	uint8 datastart_upd[MUTX_AC_VO+1];
	uint8 ltthreshdatastart_upd[MUTX_AC_VO+1];
	...
	uint32 m2sq_rr_lmt;
	uint32 txpause_starttime;
	bool swap_mufifotech;
} mutx_scb_t;
...
static int mu_add_dl_mumimo_client_to_set(wlc_mutx_info_t *mu_info, scb_t *scb);
static int mu_rm_dl_mumimo_client_from_set(wlc_mutx_info_t *mu_info, scb_t *scb);
static void mu_clear_dl_mumimo_clientset(wlc_mutx_info_t *mu_info);
static bool is_dl_mumimo_clientset_full(wlc_mutx_info_t *mu_info);
static void mu_client_set_new_sta(wlc_mutx_info_t *mu_info, scb_t *scb);
static bool is_client_in_dl_mumimo_clientset(wlc_mutx_info_t *mu_info, scb_t *scb);
static int mu_clients_select(wlc_mutx_info_t *mu_info);

static bool mu_sta_dl_mumimo_ready(wlc_mutx_info_t *mu_info, scb_t *scb, bool strict_check);
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
static void wlc_muclient_clear_stats(wlc_mutx_info_t *mu_info, mutx_dl_mumimo_stats_t *mutx_stats);
#endif
...
#endif
static int wlc_vht_dl_mumimo_membership_set(wlc_mutx_info_t *mu_info, uint16 client_index,
	uint8 *membership, uint8 *position);
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
static void
wlc_mutx_dl_mumimo_print_txpermcs(wlc_mutx_info_t *mu_info, uint32 tot_tx, uint32 tot_tx_succ,
...
static void
wlc_mutx_dl_mumimo_dump_stats(wlc_mutx_info_t *mu_info,
	mutx_dl_mumimo_stats_t *mutx_stats, uint8 mu_tp, bcmstrbuf_t *b, uint8 option)
{
	...
	for (i = 0; i < MUTX_MCSNSS_STORE_ELMTS; i++) {
	...
	bcm_bprintf(b, "mutx: tot_tx %u tx_as_mu %u (%u%%)"
		" ncons %u nlost %u\n",
		total, tot_txmu, (tot_txmu * 100 / total),
	...
	wlc_mutx_dl_mumimo_print_txpermcs(mu_info, tot_txmu, tot_txmu_succ, last_mcs_idx_mu,
		mutx_stats->txmu, mutx_stats->txmu_succ, b, TRUE, FALSE, mu_tp);

	if (option == MUTX_MUINFO_OPT_DBG) {
		wlc_mutx_dl_mumimo_print_txpermcs(mu_info, tot_txmu_sgi, tot_txmu_sgi_succ,
			last_mcs_idx_mu, mutx_stats->txmu_sgi, mutx_stats->txmu_sgi_succ, b, TRUE,
			TRUE, mu_tp);

		wlc_mutx_dl_mumimo_print_txpermcs(mu_info, tot_txsu, tot_txsu_succ,
			last_mcs_idx_su, mutx_stats->txsu, mutx_stats->txsu_succ, b, FALSE, FALSE,
			mu_tp);
	}
...
} /* wlc_mutx_dl_mumimo_dump_stats */
#endif

/* Dump DL MU-MIMO state information. */
static int
wlc_mutx_dl_mumimo_info(wlc_mutx_info_t *mu_info, bcmstrbuf_t *b, uint8 option)
{
	scb_t *scb;
	mutx_scb_t *mu_scb;
	int i;
	...
	if (MU_TX_ENAB(wlc) || HE_DLOMU_ENAB(wlc->pub)) {
		bcm_bprintf(b, "MU-MIMO BFR capable and configured; ");
	}
	...
	bcm_bprintf(b, "MU-MIMO feature is %s, AC policy is %s\n"
	...
	if (mu_info->muclient_elect_pending)
		bcm_bprintf(b, "MU-MIMO client election pending\n");
	...
	bcm_bprintf(b, "MU-MIMO clients: vhtmu %2d hemmu %2d\n",
		vhtmu_cnt, hemmu_cnt);
	for (i = 0; i < mu_info->max_muclients; i++) {
		if (!(scb = mu_info->mu_clients[i])) {
			continue;
		...
		ASSERT(mu_scb->reason < MUTX_REASON_MAX);
		bcm_bprintf(b, "[%u][%s] "MACF" rssi %d nrx %u bw %d "
			...
			mu_scb->nrx, wlc_scb_ratesel_get_link_bw(wlc, scb),
			ac, RAVG_AVG(&mu_scb->ravg_info[ac]),
			(mu_scb->reason >= MUTX_REASON_MAX) ? "INV" :
			mutx_reason2str[mu_scb->reason]);

#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
		if (option) {
			if (mu_scb->mutx_stats) {
				wlc_mutx_dl_mumimo_dump_stats(mu_info, mu_scb->mutx_stats,
	...
	bcm_bprintf(b, "\nMU-MIMO capable but not admitted STAs:\n");
	FOREACH_LIST_POS(hold_pos, next, mu_info->mu_cap_stas_list) {
		...
		ac = (mu_info->mutx_policy->ac_policy != MUTX_AC_NONE) ?
			...
			mu_info->mutx_policy->effective_ac;
		ASSERT(mu_scb->reason < MUTX_REASON_MAX);
		bcm_bprintf(b, "[%s] "MACF" rssi %d nrx %u bw %d "
			"ac %d avg_mpdu_sz %4u [%s]\n",
			mu_scb->tp == VHTMU ? "vhtmu" : mu_scb->tp == HEMMU ? "hemmu" : "ehtmmu",
			ETHER_TO_MACF(scb->ea),
			mu_info->mutx_client_scheduler ? mu_scb->rssi :
			wlc_lq_rssi_get(wlc, SCB_BSSCFG(scb), scb),
			mu_scb->nrx, wlc_scb_ratesel_get_link_bw(wlc, scb),
		...
		if (option) {
			if (mu_scb->mutx_stats) {
				/* TODO:WL11BE:MUMIMO add EHT below */
				wlc_mutx_dl_mumimo_dump_stats(mu_info, mu_scb->mutx_stats,
					((mu_scb->tp == HEMMU) ?  TX_STATUS_MUTP_HEMM :
						TX_STATUS_MUTP_VHTMU), b, option);
				wlc_muclient_clear_stats(mu_info, mu_scb->mutx_stats);
...
wlc_mutx_dump(void *ctx, bcmstrbuf_t *b)
{
	...
	return wlc_mutx_dl_mumimo_info(mu_info, b, MUTX_MUINFO_OPT_DETAIL);
}
	...
	void *p, uint plen, void *a, uint alen, uint vsize, wlc_if_t *wlcif)
{
	...
	case IOV_SVAL(IOV_MU_POLICY): {
		...
		if (policy->nrx != mu_info->muclient_nrx) {
			...
			if (policy->nrx < MUMIMO_CLIENT_NRX_1 ||
			    policy->nrx > MUMIMO_CLIENT_NRX_MAX) {
				err = BCME_RANGE;
				break;
			}
		...
		if (policy->max_muclients > MUCLIENT_NUM) {
			err = BCME_RANGE;
			break;
		}
	...
	case IOV_GVAL(IOV_MUTX_MUINFO): {
		...
		err = wlc_mutx_dl_mumimo_info(mu_info, &b, option);
		break;
	}
	...
	case IOV_SVAL(IOV_MUTX_ADMIT_MCS_THRESH):
		if (!(int_val >= 0 && int_val < MUTX_MCS_NUM)) {
			err = BCME_UNSUPPORTED;
			break;
		}

		mu_info->mutx_admit_mcs_thresh = int_val;
		break;
	case IOV_GVAL(IOV_MUTX_ADMIT_MCS_THRESH):
		*ret_uint_ptr = (uint8)mu_info->mutx_admit_mcs_thresh;
		break;
	default:
		err = BCME_UNSUPPORTED;
		break;
...
static inline void
wlc_mutx_scb_pause_traffic_all(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	mutx_scb_t *mu_scb;
	uint8 ac;
	mu_scb = MUTX_SCB(mu_info, scb);

	if (mu_scb->txpause_starttime != 0) {
		return;
	}

	for (ac = 0; ac < AC_COUNT; ac++) {
		wlc_scb_pause_traffic(mu_info->wlc, scb, ac);
	}

	mu_scb->txpause_starttime = WLC_GET_CURR_TIME(mu_info->wlc);
	WL_MUTX(("%s pause traffic to tx_type %d------->"MACF
		" swap_mufifotech %d #inflt %d\n",
		__FUNCTION__, mu_scb->tp, ETHER_TO_MACF(scb->ea),
		mu_scb->swap_mufifotech,
...
#define BW_FACTOR 20 /* BW*20 */
	...
	if (mu_info->active &&
		(mu_info->client_samebw && (bw != bw_policy_old))) {
...
uint8 prio2mufifo[NUMPRIO] = {
	MUTX_AC_BE,	/* 0	BE	AC_BE */
	MUTX_AC_BK,	/* 1	BK	AC_BK */
	MUTX_AC_BK,	/* 2	--	AC_BK */
	MUTX_AC_BE,	/* 3	EE	AC_BE */
...
wlc_mutx_ac_update(wlc_mutx_info_t *mu_info)
{
	...
	uint32 score, pkts_num[MUTX_AC_VO + 1], tx_bytes[MUTX_AC_VO + 1];
	mutx_scb_list_t *pos, *next;
	mutx_dl_mumimo_policy_t *mutx_policy;
	ratespec_t rspec;
	...
	FOREACH_LIST_POS(pos, next, mu_info->mu_cap_stas_list) {
		...
		is_mu_cap = TRUE;
		if (!wlc_mutx_sta_nrx_check(mu_info, scb)) {
			ASSERT(!is_client_in_dl_mumimo_clientset(mu_info, scb));
			is_mu_cap = FALSE;
		}

		if (mu_info->muclient_pfmon && is_mu_cap &&
			!is_client_in_dl_mumimo_clientset(mu_info, scb)) {
		...
		bzero(&pkts_num, sizeof(pkts_num));
		bzero(&tx_bytes, sizeof(tx_bytes));
		for (prio = 0; prio < NUMPRIO; prio++) {
			ac = prio2mufifo[prio];
			pkts_num[ac] += ampdu_scb_stats.tx_pkts[prio];
			tx_bytes[ac] += ampdu_scb_stats.tx_bytes[prio];
		}

		for (ac = 0; ac <= MUTX_AC_VO; ac++) {
			if (pkts_num[ac] == 0) {
				if (mu_info->muclient_pfmon &&
					is_client_in_dl_mumimo_clientset(mu_info, scb)) {
					mu_scb->ltthreshdatastart_upd[ac] = 0;
					mu_scb->datastart_upd[ac] = 0;
				}
				RAVG_INIT(&mu_scb->ravg_info[ac], 0, 0);
				continue;
			}
			avg_mpdu_len = tx_bytes[ac] / pkts_num[ac];
			RAVG_ADD(&mu_scb->ravg_info[ac], avg_mpdu_len);

			if (pkts_num[ac] < MUTX_PKTRATE_ADMIT_THRESH) {
				if (mu_info->muclient_pfmon &&
					is_client_in_dl_mumimo_clientset(mu_info, scb)) {
					mu_scb->ltthreshdatastart_upd[ac]++;
					mu_scb->datastart_upd[ac] = 0;
				}
				continue;
			}

			WL_MUTX((MACF" ac=%d rssi=%d nrx=%u tx_pkts %u tx_bytes %u "
				"avg_mpdu_len %u\n",
			...
			score = pkts_num[ac]/100;
			mu_scb->ac_policy_scores[ac] += score;
			if (mu_info->muclient_pfmon &&
				is_client_in_dl_mumimo_clientset(mu_info, scb)) {
				mu_scb->datastart_upd[ac]++;
				mu_scb->ltthreshdatastart_upd[ac] = 0;
			}
			if (is_mu_cap &&
				(pkts_num[ac] >= MUTX_PKTRATE_ADMIT_THRESH) &&
	...
	for (; (mu_info->active) && (bw >= BW_20MHZ); bw--) {
		for (ac = MUTX_AC_BE; ac <= MUTX_AC_VO; ac++) {
	...
	if ((mu_info->active) &&
		((mu_info->client_samebw && (bw_policy != bw_policy_old)) ||
		...
		WL_ERROR(("wl%d: Updating bw_policy_old=%u=> bw_policy=%u\n"
			"ac_policy_old=%u=>ac_policy=%u\n",
			mu_info->pub->unit, bw_policy_old, bw_policy,
			ac_policy_old, ac_policy));
		if (mu_info->active) {
			/* Evict policy mismatch client */
			for (idx = 0; idx < mu_info->max_muclients; idx++) {
				...
				if ((mu_scb->ac_policy_scores[ac_policy] == 0) ||
					(mu_info->client_samebw && (bw != bw_policy))) {
					mu_scb->policy_evict = MUTX_POLICY_EVICT_AC;
					mu_scb->reason = MUTX_REASON_AC_POLICY_MISMATCH;
				}
			}
	...
	FOREACH_LIST_POS(pos, next, mu_info->mu_cap_stas_list) {
		...
		if (ac != MUTX_AC_NONE) {
			...
			WL_MUTX(("wl%d: "MACF" tx_type %d ac %u score = %u "
				...
				mu_scb->tp, ac, mu_scb->ac_policy_scores[ac],
				RAVG_AVG(&mu_scb->ravg_info[ac])));
		} else {
			mu_scb->score = 0;
		}
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
static void
wlc_muclient_clear_stats(wlc_mutx_info_t *mu_info, mutx_dl_mumimo_stats_t *mutx_stats)
{
	memset(mutx_stats, 0, sizeof(mutx_dl_mumimo_stats_t));
}
#endif
...
wlc_muclient_clear_pfmon_stats(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	...
	bzero(&mu_scb->datastart_upd, sizeof(mu_scb->datastart_upd));
...
/**
 * Evicts a particular MU client if the highest MCS that was used to send towards the client falls
 * below a threshold.
 */
static bool
wlc_muclients_pfmon_eval_primrate_mcs(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	...
	/* determines highest*index* on which a transmit took place */
	for (i = 0, last = 0; i < MUTX_MCSNSS_STORE_ELMTS; i++) {
...
wlc_muclient_pfmon(wlc_mutx_info_t *mu_info)
{
	...
	for (i = 0; i < mu_info->max_muclients; i++) {
		...
		if ((ac != MUTX_AC_NONE) &&
			(mu_scb->datastart_upd[ac] > MUTX_STATS_MIN_NUPD)) {
			link_bw = wlc_scb_ratesel_get_link_bw(mu_info->wlc, scb) - 1;
			evict = wlc_muclients_pfmon_eval_primrate_mcs(mu_info, scb);
			if (!evict) {
				evict = wlc_muclients_pfmon_eval_primrate_succ(mu_info, scb);
			}
			if (!evict) {
				evict = wlc_muclients_pfmon_eval_tx_as_mu(mu_info, scb);
			}
			if (!evict && RAVG_AVG(&mu_scb->ravg_info[ac]) <=
				mu_info->mpdusz_mu_admit_thresh[link_bw]) {
				evict = TRUE;
				mu_scb->policy_evict = MUTX_POLICY_EVICT_AC;
				mu_scb->reason = MUTX_REASON_PFMON_AVG_MPDU_SZ_EVICT;
				WL_MUTX(("evict_client(%s):pfmon_avg_mpdu_sz: "MACF""
					" ac %d, %u < thresh(%u)\n", (evict ? "true" : "false"),
					ETHER_TO_MACF(scb->ea), ac,
					RAVG_AVG(&mu_scb->ravg_info[ac]),
					mu_info->mpdusz_mu_admit_thresh[link_bw]));
		...
		if (!evict) {
			...
			if ((ac != MUTX_AC_NONE) &&
				(mu_scb->ltthreshdatastart_upd[ac] >
					...
					evict = TRUE;
					mu_scb->policy_evict = MUTX_POLICY_EVICT_AC;
					mu_scb->reason = MUTX_REASON_PPS_LT_THRESH;
					WL_MUTX(("evict_client(%s):pfmon_pps_lt_thresh: "
						MACF"" " ac %d < threshold(%u)\n", (evict ? "true" :
						"false"), ETHER_TO_MACF(scb->ea), ac,
						MUTX_PKTRATE_THRESH_LOW));
					bzero(&mu_scb->ltthreshdatastart_upd,
						sizeof(mu_scb->ltthreshdatastart_upd));
			}
		}
		if (evict) {
			evict_cnt++;
...
wlc_mutx_fifo_hold_clr(wlc_info_t *wlc, void *fifo_bitmap)
{
	...
	for (i = 0; i < WLC_HW_NFIFO_INUSE(wlc); i++) {
		...
		wlc_hwa_tx_fifo_hpq_hw_fill(wlc, wlc->active_queue->low_txq, i);
#endif
	}
}
...
wlc_mutx_watchdog(void *mi)
{
	...
	if ((!(mu_info->_mu_tx[TP_TO_IDX(VHTMU)] ||
		...
		!LIST_IS_EMPTY(mu_info->evictee_list) ||
		!LIST_IS_EMPTY(mu_info->candidate_ofdma_list) ||
		!LIST_IS_EMPTY(mu_info->admit_ofdma_list)) {
...
wlc_mutx_up(void *mi)
{
	...
	// mu_clear_dl_mumimo_clientset(mu_info);
	...
	if (!wlc->pub->up) {
		wlc_fifo_dlmmu_reset_subtype_max(wlc->fifo);
		if (!mu_info->muclient_pfmon) {
			mu_info->mutx_policy->effective_ac = MUTX_AC_BE;
		}
	}
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
		size += ALIGN_SIZE(sizeof(mutx_dl_mumimo_stats_t), sizeof(uint32));
#endif
#ifdef WLCNT
		size += ALIGN_SIZE(sizeof(mutx_pfmon_stats_t), sizeof(uint32)) +
			ALIGN_SIZE(sizeof(mutx_dl_mumimo_snr_calib_stats_t), sizeof(uint32));
#endif	/* WLCNT */
	}
	return size;
}
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
		mu_scb->mutx_stats = (mutx_dl_mumimo_stats_t *)secptr;
		secptr += ALIGN_SIZE(sizeof(mutx_dl_mumimo_stats_t), sizeof(uint32));
#endif
#ifdef WLCNT
		mu_scb->mutx_pfmon_stats = (mutx_pfmon_stats_t*)secptr;
		secptr += ALIGN_SIZE(sizeof(mutx_pfmon_stats_t), sizeof(uint32));
		mu_scb->mutx_snr_calib_stats = (mutx_dl_mumimo_snr_calib_stats_t*)secptr;
#endif	/* WLCNT */
...
mu_scb_cubby_deinit(void *ctx, scb_t *scb)
{
	...
	if ((mu_scb->tp == VHTMU) || (mu_scb->tp == HEMMU)) {
		...
		if (is_client_in_dl_mumimo_clientset(mu_info, scb)) {
			mu_sta_mu_disable(mu_info, scb);
...
/* Add a STA to the current DL-MU client set.
 * Returns
 ...
 */
static int
mu_add_dl_mumimo_client_to_set(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	wlc_info_t *wlc = mu_info->wlc;
	mutx_scb_t *mu_scb = MUTX_SCB(mu_info, scb);
	uint16 i;
	...
	if (mu_info->muclient_pfmon) {
		...
		fifo_avail_cnt = wlc_fifo_avail_count(wlc->fifo,
			ac - 1, WL_MU_DLMMU);
		WL_ERROR(("wl%d: STA "MACF" ac %d fifo_avail_cnt %d tx_type %d\n",
			wlc->pub->unit, ETHER_TO_MACF(scb->ea), ac, fifo_avail_cnt,
			mu_scb->tp));
		if (!mu_scb->swap_mufifotech) {
			(void) wlc_fifo_alloc(wlc->fifo, scb, WL_MU_DLMMU, (ac - 1));
		} else {
			for (ac_2 = MUTX_AC_BE; ac_2 <= MUTX_AC_VO; ac_2++) {
				if (!wlc_fifo_isMU(wlc->fifo, scb, (ac_2 - 1))) {
					continue;
				}
				wlc_fifo_switch_mu_type(wlc->fifo, scb->fifo_idx[ac_2 - 1],
					ac_2 - 1, WL_MU_DLOFDMA, 0, WL_MU_DLMMU, HEMMU);
			}
		}
	}
	...
	if (mu_scb->tp == VHTMU) {
		...
		if (D11REV_LT(mu_info->pub->corerev, 129)) {
			err = wlc_vht_dl_mumimo_membership_set(mu_info, i,
...
/* Remove STA from current DL-MU client set.
 * Returns
 *   BCME_OK if STA is removed from MU client set
 *   BCME_NOTFOUND if STA is not found
 */
static int
mu_rm_dl_mumimo_client_from_set(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	mutx_scb_t *mu_scb = MUTX_SCB(mu_info, scb);
	wlc_info_t *wlc = mu_info->wlc;
	...
	WL_ERROR(("wl%d: STA "MACF" removed from MU client set with index %d "
		"tx_type %d reason %s\n", mu_info->pub->unit, ETHER_TO_MACF(scb->ea),
		mu_scb->client_index, mu_scb->tp, mutx_reason2str[mu_scb->reason]));

	if (mu_scb->policy_evict == MUTX_POLICY_EVICT_PFMON) {
		heurs_reason = HEURS_MU_PFMON_EVICT;
	} else if (mu_scb->policy_evict == MUTX_POLICY_EVICT_SCORE) {
...
static void
mu_clear_dl_mumimo_clientset(wlc_mutx_info_t *mu_info)
{
	int i;
	scb_t *scb;
	BCM_REFERENCE(mu_info);
	for (i = 0; i < mu_info->max_muclients; i++) {
		...
		if (mu_rm_dl_mumimo_client_from_set(mu_info, scb) != BCME_OK) {
...
static bool
is_client_in_dl_mumimo_clientset(wlc_mutx_info_t *mu_info, scb_t *scb)
{
...
static bool
is_dl_mumimo_clientset_full(wlc_mutx_info_t *mu_info)
{
	int i, clients = 0;
...
mu_client_set_new_sta(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	...
	if (is_dl_mumimo_clientset_full(mu_info)) {
		return;
	}

	(void)mu_add_dl_mumimo_client_to_set(mu_info, scb);
}
	...
	bool sched)
{
	...
	if (!mu_info->active && !HE_DLOMU_ENAB(wlc->pub)) {
		return;
	}

	max_limit = is_dl_mumimo_clientset_full(mu_info);
	...
	FOREACH_LIST_POS(pos, next, mu_info->mu_cap_stas_list) {
		...
		scb = mu_scb->scb;
		ASSERT(!is_client_in_dl_mumimo_clientset(mu_info, scb));
		if (mu_sta_dl_mumimo_ready(mu_info, scb, TRUE)) {
			...
			cnt++;
		} else if (mu_sta_dl_mumimo_ready(mu_info, scb, FALSE)) {
...
/* HE-STA is now a TWT-STA. Evict if it is
 * admitted as HEMMU
 */

void
wlc_mutx_twtsta_evict_from_hemmu(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	mutx_scb_t *mu_scb;
	...
	mu_scb = MUTX_SCB(mu_info, scb);
	if (!mu_scb || !mu_scb->tp || (mu_scb->tp != HEMMU)) {
		return;
	}

	if (ITEM_IN_LIST(mu_info->new_list, mu_scb->item)) {
	...
	if (ITEM_IN_LIST(mu_info->evictee_list, mu_scb->item)) {
		mutx_scb_list_move(mu_info->mu_cap_stas_list, mu_scb->item);
	} else {
		mu_scb->reason = MUTX_REASON_TWTSTA;
		mu_scb->policy_evict = MUTX_POLICY_EVICT_NOTREADY;
	}

	mu_rm_dl_mumimo_client_from_set(mu_info, scb);
	mu_info->muclient_elect_pending = TRUE;
}

void
wlc_mutx_process_dlhemmu_dis_change(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	mutx_scb_t *mu_scb;
	...
	if (wlc_he_get_dlmmu_allow(mu_info->wlc->hei, scb)) {
		return;
	}
	...
	mu_scb->reason = MUTX_REASON_OMI_ULMUDIS;
	mu_scb->policy_evict = MUTX_POLICY_EVICT_NOTREADY;
...
void
wlc_mutx_evict_dlofdma_client(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	mutx_scb_t *mu_scb;
	wlc_info_t *wlc;

	wlc = mu_info->wlc;
	if (!mu_info || !HE_DLOMU_ENAB(wlc->pub)) {
		return;
	}
	...
	if (ITEM_IN_LIST(mu_info->admit_ofdma_list, mu_scb->item)) {
		mutx_scb_list_move(mu_info->mu_cap_stas_list, mu_scb->item);
	}
	...
	if (!SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb)) {
		...
		WL_MUTX(("%s evict DLOFDMA STA------->"MACF"\n", __FUNCTION__,
			ETHER_TO_MACF(scb->ea)));
	}
...
#define DLMMU_MAXFIFOPERAC 16
static void
wlc_mutx_nfifo_pertxtech(wlc_mutx_info_t *mu_info, int nvhtmu,
	int nhemmu, int nfifo, int *nfifopertech)
{
	wlc_info_t *wlc;
	int fifo_max_cnt;
	int i, j, k, idx;
	int count = 0;
	int sum[(DLMMU_MAXFIFOPERAC / 2) + 1][WL_MU_DLMMU_SUBTYPE_MAX] = {{0}};
	BCM_REFERENCE(fifo_max_cnt);
	wlc = mu_info->wlc;
	fifo_max_cnt = wlc_fifo_max_per_ac(wlc->fifo, WL_MU_DLMMU);
	ASSERT(fifo_max_cnt <= DLMMU_MAXFIFOPERAC);

	k = (nvhtmu + nhemmu) < nfifo ? (nvhtmu + nhemmu) : nfifo;
	for (i = 0; i <= k; i++) {
		for (j = i; j <= k; j++) {
			if ((i + j) == k) {
				sum[i][0] = i;
	...
	WL_MUTX(("#nfifo %d nvhtmu %d nhemmu %d\n", nfifo, nvhtmu, nhemmu));
	for (i = 0; i < count; i++) {
		WL_MUTX(("#nfifopertech idx[%d] = %d %d\n", i, sum[i][0], sum[i][1]));
	}

	if ((nvhtmu == 0) || (nhemmu == 0)) {
		idx = 0;
	} else if ((nfifo == 2) || (nfifo == 3)) {
		idx = 0;
	} else if (nfifo >= (nvhtmu + nhemmu)) {
		idx = nvhtmu < nhemmu ? nvhtmu : nhemmu;
	} else {
		idx = nvhtmu < nhemmu ? nvhtmu : nhemmu;
		idx = (count - 1) < idx ? (count - 1) : idx;
	}

	nfifopertech[0] = sum[idx][0];
	nfifopertech[1] = sum[idx][1];
	if ((nvhtmu >= nhemmu) &&
		(nfifopertech[0] < nfifopertech[1])) {
		nfifopertech[0] = sum[idx][1];
		nfifopertech[1] = sum[idx][0];
	}

	WL_MUTX(("#nfifopertech idx[%d] = %d %d\n",
		idx, nfifopertech[0], nfifopertech[1]));
}

static void
wlc_mutx_assign_txfifo(wlc_mutx_info_t *mu_info)
{
	wlc_info_t *wlc;
	mutx_scb_t *mu_scb;
	scb_t *mu_client;
	mutx_scb_list_t *pos, *next;
	uint ac, ac_2, i;
	int fifo_avail_cnt, fifo_max_cnt, fifo_reclaim_cnt = 0;
	int fifo_inuse_vhtcnt[AC_COUNT], fifo_inuse_hecnt[AC_COUNT];
	int admitted_cnt = 0;
	int nfifopertech[2] = { 0 };
	int nvhtmu = 0, nhemmu = 0;
	...
	for (ac_2 = 0; ac_2 < AC_COUNT; ac_2++) {
		fifo_inuse_vhtcnt[ac_2] = wlc_fifo_dlmmu_get_inuse_per_ac(wlc->fifo,
			WL_MU_DLMMU_VHTMU, ac_2);
		fifo_inuse_hecnt[ac_2] = wlc_fifo_dlmmu_get_inuse_per_ac(wlc->fifo,
			WL_MU_DLMMU_HEMMU, ac_2);
		fifo_reclaim_cnt += fifo_inuse_vhtcnt[ac_2] + fifo_inuse_hecnt[ac_2];
	}

	fifo_max_cnt = wlc_fifo_max_per_ac(wlc->fifo, WL_MU_DLMMU);
	fifo_avail_cnt = wlc_fifo_dlmmu_avail_count(wlc->fifo);
	fifo_avail_cnt += fifo_reclaim_cnt;
	fifo_avail_cnt = MIN(fifo_avail_cnt, fifo_max_cnt);

	if (!(wlc->block_datafifo & DATA_BLOCK_MUTX)) {
		wlc_mutx_nfifo_pertxtech(mu_info, nvhtmu, nhemmu,
			fifo_avail_cnt, &nfifopertech[0]);
		if (!(((nvhtmu && (fifo_inuse_vhtcnt[ac - 1] < 2)) ||
			(nhemmu && (fifo_inuse_hecnt[ac - 1] < 2))) ||
			(fifo_avail_cnt &&
			((nfifopertech[0] > fifo_inuse_vhtcnt[ac - 1]) ||
			(nfifopertech[1] > fifo_inuse_hecnt[ac - 1]))))) {
			return;
		}

		wlc_block_datafifo(wlc, DATA_BLOCK_MUTX, DATA_BLOCK_MUTX);
		WL_MUTX(("wl%d: %s:  block data fifo %d\n", wlc->pub->unit,
			__FUNCTION__, (wlc->block_datafifo & DATA_BLOCK_MUTX) != 0));
		WL_MUTX(("%s+=>\n"
			"ac_policy %d fifo_max_cnt %d fifo_avail_cnt %d fifo_reclaim_cnt %d"
			" admitted_cnt %d\nnhemmu %d nfifoperhe %d fifo_inuse_hecnt %d\n"
			"nvhtmu %d nfifopervht %d fifo_inuse_vhtcnt %d\n",
			__FUNCTION__, ac, fifo_max_cnt, fifo_avail_cnt, fifo_reclaim_cnt,
			admitted_cnt, nhemmu, nfifopertech[1], fifo_inuse_hecnt[ac - 1],
			nvhtmu, nfifopertech[0], fifo_inuse_vhtcnt[ac - 1]));
		mu_info->block_datafifo = TRUE;
		return;
	} else {
		FOREACH_LIST_POS(pos, next, mu_info->mu_cap_stas_list) {
			mu_scb = (mutx_scb_t *)pos->data;
			mu_client = mu_scb->scb;
			if (!SCB_DLOFDMA_ADM(mu_client) ||
				wlc_twt_scb_active(wlc->twti, mu_client)) {
				continue;
			}
			wlc_fifo_free_all(wlc->fifo, mu_client);
		}
		...
		for (ac_2 = 0; ac_2 < AC_COUNT; ac_2++) {
			wlc_fifo_dlmmu_set_max_per_ac(wlc->fifo, WL_MU_DLMMU_VHTMU, ac_2,
				nfifopertech[0]);
			wlc_fifo_dlmmu_set_max_per_ac(wlc->fifo, WL_MU_DLMMU_HEMMU, ac_2,
				nfifopertech[1]);
		}
		...
		for (i = 0; i < mu_info->max_muclients; i++) {
			...
			wlc_muclient_clear_pfmon_stats(mu_info, mu_info->mu_clients[i]);
		}

		fifo_inuse_vhtcnt[ac - 1] = wlc_fifo_dlmmu_get_inuse_per_ac(wlc->fifo,
			WL_MU_DLMMU_VHTMU, ac - 1);
		fifo_inuse_hecnt[ac - 1] = wlc_fifo_dlmmu_get_inuse_per_ac(wlc->fifo,
			WL_MU_DLMMU_HEMMU, ac - 1);
		WL_MUTX(("%s-=>\n"
			"ac_policy %d fifo_max_cnt %d fifo_avail_cnt %d fifo_reclaim_cnt %d"
			" admitted_cnt %d\nnhemmu %d nfifoperhe %d fifo_inuse_hecnt %d\n"
			"nvhtmu %d nfifopervht %d fifo_inuse_vhtcnt %d\n",
...
wlc_mutx_eval_dlofdma_admission(wlc_mutx_info_t *mu_info)
{
	...
	uint32 rspec, mcs, avg_mpdusz_thresh, ac;
	int tx_type;

	if (!HE_DLOMU_ENAB(wlc->pub)) {
		return;
	}
	...
	FOREACH_LIST_POS(pos, next, mu_info->mu_cap_stas_list) {
		...
		avg_mpdusz_thresh = mu_info->mpdusz_dlofdma_admit_thresh[link_bw];
		for (ac = MUTX_AC_BE; (mu_info->muclient_pfmon &&
				(ac < (MUTX_AC_VO + 1))); ac++) {
			...
			WL_MUTX(("wl%d: "MACF" tx_type %d ac %u score = %u "
				...
				RAVG_AVG(&mu_scb->ravg_info[ac]), avg_mpdusz_thresh));
		}

		rspec = wlc_scb_ratesel_get_primary(wlc, scb, NULL);
		mcs = wlc_ratespec_mcs(rspec);
		WL_MUTX(("isdlofdma ? %s link_bw %d mcs %d\n",
			SCB_DLOFDMA_ADM(scb) ? "TRUE" : "FALSE", link_bw, mcs));

		tx_type = DLOFDMA;
		if (mu_info->muclient_pfmon &&
			!CHSPEC_IS160(wlc->chanspec) &&
			(mcs >= MUTX_SU_MCS_THRESH)) {
			if (((mu_scb->ac_policy_scores[MUTX_AC_BE] != MUTX_AC_NONE) &&
				(RAVG_AVG(&mu_scb->ravg_info[MUTX_AC_BE]) >= avg_mpdusz_thresh)) ||
				((mu_scb->ac_policy_scores[MUTX_AC_BK] != MUTX_AC_NONE) &&
				(RAVG_AVG(&mu_scb->ravg_info[MUTX_AC_BK]) >= avg_mpdusz_thresh)) ||
				((mu_scb->ac_policy_scores[MUTX_AC_VI] != MUTX_AC_NONE) &&
				(RAVG_AVG(&mu_scb->ravg_info[MUTX_AC_VI]) >= avg_mpdusz_thresh)) ||
				((mu_scb->ac_policy_scores[MUTX_AC_VO] != MUTX_AC_NONE) &&
				(RAVG_AVG(&mu_scb->ravg_info[MUTX_AC_VO]) >= avg_mpdusz_thresh))) {
				tx_type = SU;
			}
		}
		...
		} else if (!SCB_DLOFDMA_ADM(scb) && !SCB_IS_UNUSABLE(scb) &&
	...
	FOREACH_LIST_POS(pos, next, mu_info->admit_ofdma_list) {
		...
		if (!mu_scb->swap_mufifotech &&
			SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb)) {
			wlc_mutx_scb_pause_traffic_all(mu_info, scb);
		}
	}
...
wlc_mutx_admit_dlofdma_clients(wlc_mutx_info_t *mu_info)
{
	...
	/* dump_flag_qqdx */
			...
			ASSERT(!SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb));
			wlc_fifo_free_all(wlc->fifo, scb);
		} else {
			ofdma_on = TRUE;
			WL_MUTX(("%s admit------->"MACF" swap_mufifotech %d #inflt %d\n",
				__FUNCTION__, ETHER_TO_MACF(scb->ea), mu_scb->swap_mufifotech,
				SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb)));
			...
			if (!mu_scb->swap_mufifotech) {
				...
				for (ac = MUTX_AC_BE; ac <= MUTX_AC_VO; ac++) {
					if (!wlc_fifo_isMU(wlc->fifo, scb, (ac - 1))) {
						continue;
					}
					wlc_fifo_switch_mu_type(wlc->fifo, scb->fifo_idx[ac - 1],
						(ac - 1), WL_MU_DLMMU, HEMMU,
						WL_MU_DLOFDMA, DLOFDMA);
				}
			}
		}
...
static void
wlc_mutx_switch_txtech(wlc_mutx_info_t *mu_info)
{
	wlc_info_t *wlc = mu_info->wlc;
	mutx_scb_t *mu_scb;
	mutx_scb_list_t *pos, *next;
	...
	uint ac;
	uint ac_2 = (mu_info->mutx_policy->ac_policy != MUTX_AC_NONE) ?
		mu_info->mutx_policy->ac_policy : mu_info->mutx_policy->effective_ac;
	...
	FOREACH_LIST_POS(pos, next, mu_info->evictee_list) {
		...
		mu_scb->swap_mufifotech = mu_info->_swap_mufifotech;
		/* swap FIFO Tx tech */
		for (ac = 0; ac < AC_COUNT; ac++) {
			if ((mu_scb->tp == VHTMU) ||
				(wlc_fifo_isMU(wlc->fifo, evictee, ac) &&
				(wlc_fifo_user_count(wlc->fifo, evictee, ac) >= 2))) {
				mu_scb->swap_mufifotech = FALSE;
				break;
			}
		}

		if (mu_scb->swap_mufifotech ||
			!SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(evictee)) {
			WL_MUTX(("%s evict tx_type %d ------->"MACF
				" swap_mufifotech %d #inflt %d\n", __FUNCTION__, mu_scb->tp,
				ETHER_TO_MACF(evictee->ea), mu_scb->swap_mufifotech,
				SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(evictee)));

			mutx_scb_list_move(mu_info->mu_cap_stas_list, pos);
			mu_rm_dl_mumimo_client_from_set(mu_info, evictee);
		} else {
			wlc_mutx_scb_pause_traffic_all(mu_info, evictee);
		}
	}
	...
	FOREACH_LIST_POS(pos, next, mu_info->new_list) {
		...
		for (ac = 0; ac < AC_COUNT; ac++) {
			if ((mu_scb->tp == VHTMU) ||
				((wlc_fifo_isMU(wlc->fifo, candidate, ac) &&
				(wlc_fifo_user_count(wlc->fifo, candidate, ac) >= 2)) ||
				wlc_fifo_is_deep_dma(wlc->fifo, candidate, ac) ||
				((ac == ac_2 - 1) && !(wlc_fifo_isMU(wlc->fifo, candidate, ac))))) {
				mu_scb->swap_mufifotech = FALSE;
				break;
			}
		}
		...
		if (mu_scb->swap_mufifotech ||
			!SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(candidate)) {
				mutx_scb_list_move(mu_info->mu_cap_stas_list, pos);
			WL_MUTX(("%s admit tx_type %d------->"MACF
				" swap_mufifotech %d #inflt %d\n",
				__FUNCTION__, mu_scb->tp, ETHER_TO_MACF(candidate->ea),
				mu_scb->swap_mufifotech,
				SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(candidate)));
				mu_sta_mu_enable(mu_info, candidate);
		} else {
			wlc_mutx_scb_pause_traffic_all(mu_info, candidate);
		}
	}
	...
	if (LIST_IS_EMPTY(mu_info->new_list) &&
		...
		mu_info->muclient_elect_pending = FALSE;
		if (!(wlc->block_datafifo & DATA_BLOCK_MUTX)) {
			wlc_mutx_assign_txfifo(mu_info);
		}
	}
	return;
}
...
void
wlc_mutx_scb_drained(wlc_info_t *wlc, scb_t *scb)
{
	mutx_scb_t *mu_scb;
	int i;
	wlc_mutx_info_t *mu_info = wlc->mutx;

	if (!mu_info || (!(mu_info->_mu_tx[TP_TO_IDX(VHTMU)] ||
		mu_info->_mu_tx[TP_TO_IDX(HEMMU)] ||
		HE_DLOMU_ENAB(wlc->pub)))) {
		return;
	}
	...
	mu_scb->txpause_starttime = 0;

	if (ITEM_IN_LIST(mu_info->new_list, mu_scb->item) ||
		ITEM_IN_LIST(mu_info->evictee_list, mu_scb->item)) {
		if (ITEM_IN_LIST(mu_info->new_list, mu_scb->item)) {
			WL_MUTX(("%s admit tx_type %d------->"MACF" swap_mufifotech %d "
				"inflt %d\n", __FUNCTION__, mu_scb->tp, ETHER_TO_MACF(scb->ea),
				mu_scb->swap_mufifotech, SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb)));
			mu_sta_mu_enable(mu_info, scb);
		} else {
			WL_MUTX(("%s evict tx_type %d------->"MACF" swap_mufifotech %d "
				"inflt %d\n", __FUNCTION__, mu_scb->tp, ETHER_TO_MACF(scb->ea),
				mu_scb->swap_mufifotech, SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb)));
			mu_rm_dl_mumimo_client_from_set(mu_info, scb);
		}
		...
		if (LIST_IS_EMPTY(mu_info->new_list) &&
			...
			for (i = 0; i < mu_info->max_muclients; i++) {
				if (mu_info->mu_clients[i]) {
					wlc_muclient_clear_pfmon_stats(mu_info,
						mu_info->mu_clients[i]);
				}
			}
		}
	}
...
{
	if (!HE_DLOMU_ENAB(mu_info->wlc->pub)) {
		return;
	}
	mu_info->muclient_elect_pending = TRUE;
}
	...
	heurs_reason_t reason)
{
	...
	switch (reason)
	{
		...
		case HEURS_MU_PFMON_EVICT:
		{
			...
			musta_heurs->counter0 = MAX(musta_heurs->counter0, 2);
...
/* Admits both DL-MIMO and DL-OFDMA clients to the scheduler */
static void
wlc_mutx_admit_clients(wlc_mutx_info_t *mu_info)
{
	...
	WL_MUMIMO(("%s+ MUTXPKTPENDTOT %d\n", __FUNCTION__, (MUTXPKTPENDTOT(wlc))));
	BCM_REFERENCE(mu_type);
	...
	FOREACH_LIST_POS(pos, next, mu_info->admit_list) {
			...
			WL_MUMIMO((""MACF" mu_type %d evict %d\n", ETHER_TO_MACF(candidate->ea),
				mu_scb->tp, mu_scb->policy_evict));
	}
	...
	if (mu_info->active &&
		...
		FOREACH_LIST_POS(pos, next, mu_info->admit_list) {
			...
			if (is_client_in_dl_mumimo_clientset(mu_info, candidate)) {
	...
	wlc_mutx_switch_txtech(mu_info);
#ifdef BCMDBG
	...
	FOREACH_LIST_POS(pos, next, mu_info->mu_cap_stas_list) {
		...
		WL_MUMIMO((""MACF" mu_type %d\n", ETHER_TO_MACF(candidate->ea),
			mu_scb->tp));
		if ((mu_scb->tp == HEMMU) &&
			(SCB_HEMMU(candidate) || SCB_DLOFDMA_ADM(candidate))) {
...
mu_clients_eviction(wlc_mutx_info_t *mu_info, bool sched)
{
	...
	uint8 i;
	int nclients = 0;
	uint8 bw, tp_idx, link_bw;
	uint8 nitems[MUTX_TP_MAX][BW_160MHZ] = { {0} };
	BCM_REFERENCE(wlc);
	BCM_REFERENCE(candidate);

	if (!LIST_IS_EMPTY(mu_info->candidate_list)) {
		WL_MUTX(("%s-: mu_candidate_stas_list\n", __FUNCTION__));
		FOREACH_LIST_POS(pos, next, mu_info->candidate_list) {
			mu_scb = (mutx_scb_t *)pos->data;
			candidate = mu_scb->scb;
			WL_MUTX((""MACF" mu_type %d score %u\n", ETHER_TO_MACF(candidate->ea),
				mu_scb->tp, mu_scb->score));
		}
	}
	...
	for (i = 0; i < mu_info->max_muclients; i++) {
		...
		ASSERT(mu_scb_evict != NULL);
		ASSERT(mu_scb_evict->item != NULL);
		if (SCB_IS_UNUSABLE(evictee) &&
			mu_scb_evict->policy_evict) {
			mu_scb_evict->reason = MUTX_REASON_UNDEFINED;
	...
	if (!LIST_IS_EMPTY(mu_info->evictee_list)) {
		WL_MUTX(("%s-: evictee_list\n", __FUNCTION__));
		FOREACH_LIST_POS(pos, next, mu_info->evictee_list) {
			mu_scb = (mutx_scb_t *)pos->data;
			candidate = mu_scb->scb;
			WL_MUTX((""MACF" mu_type %d score %u\n", ETHER_TO_MACF(candidate->ea),
	...
	while ((cand_entry = mutx_scb_list_max(mu_info->candidate_list)) != NULL) {
		...
		if (nclients < mu_info->max_muclients) {
			...
			mu_scb_evict->policy_evict = MUTX_POLICY_EVICT_SCORE;
	...
	FOREACH_LIST_POS(pos, next, mu_info->admit_list) {
		mu_scb = (mutx_scb_t *)pos->data;
		candidate = mu_scb->scb;
		link_bw = wlc_scb_ratesel_get_link_bw(mu_info->wlc, candidate) - 1;
		tp_idx = TP_TO_IDX(mu_scb->tp);
		if (mu_scb->policy_evict) {
			continue;
		}

		mutx_scb_list_move(mu_info->mu_plcy_vldn_list[tp_idx][link_bw],
				pos);
		nitems[tp_idx][link_bw]++;
		WL_MUTX(("%s "MACF" tp_idx %d link_bw %d\n", __FUNCTION__,
			ETHER_TO_MACF(candidate->ea), tp_idx, link_bw));
	}

	nclients = 0;
	for (tp_idx = 0; tp_idx < MUTX_TP_MAX; tp_idx++) {
		if (((tp_idx == 0) && !mu_info->_mu_tx[TP_TO_IDX(VHTMU)]) ||
			((tp_idx == 1) && !mu_info->_mu_tx[TP_TO_IDX(HEMMU)])) {
			continue;
		}

		for (bw = 0; bw < BW_160MHZ; bw++) {
			if (nitems[tp_idx][bw] < 2) {
				continue;
			}
			nclients += nitems[tp_idx][bw];
			WL_MUTX(("%s tp_idx %d bw %d nclients %d\n", __FUNCTION__,
				tp_idx, bw, nitems[tp_idx][bw]));
		}
	}
	...
	while (nclients < mu_info->max_muclients) {
		...
		evictee = NULL;
		if (alter_entry) {
			mu_scb_evict = (mutx_scb_t *)alter_entry->data;
			evictee = mu_scb_evict->scb;
		}
		...
		if (!alter_entry) {
			...
			cand_entry = mu_scb_evict->item;
			mu_scb = mu_scb_evict;
			candidate = mu_scb_evict->scb;
		} else {
			if (mu_scb_evict->score >= mu_scb->score) {
				mutx_scb_list_move(mu_info->candidate_list, cand_entry);
				cand_entry = mu_scb_evict->item;
				...
				mutx_scb_list_move(mu_info->alternate_list, alter_entry);
			}
		}
		...
		mutx_scb_list_move(mu_info->mu_plcy_vldn_list[tp_idx][link_bw],
			cand_entry);
		if ((nclients +
			ITEMS_IN_LIST(mu_info->mu_plcy_vldn_list[tp_idx][link_bw])) >
			mu_info->max_muclients) {
			continue;
		}

		if ((nitems[tp_idx][link_bw] +
			ITEMS_IN_LIST(mu_info->mu_plcy_vldn_list[tp_idx][link_bw])) >= 2) {
			if ((nitems[tp_idx][link_bw] +
				ITEMS_IN_LIST(mu_info->mu_plcy_vldn_list[tp_idx][link_bw])) == 2) {
				nclients++;
			}
		...
		nitems[tp_idx][link_bw]++;
		WL_MUTX(("%s tp_idx %d bw %d nclients %d\n", __FUNCTION__,
			tp_idx, link_bw, nitems[tp_idx][link_bw]));
	}
	...
	for (tp_idx = 0; tp_idx < MUTX_TP_MAX; tp_idx++) {
		...
		for (bw = 0; bw < BW_160MHZ; bw++) {
			if (nitems[tp_idx][bw] == 0) {
				continue;
			}

			FOREACH_LIST_POS(pos, next,
				mu_info->mu_plcy_vldn_list[tp_idx][bw]) {
				mu_scb = (mutx_scb_t *)pos->data;
				candidate = mu_scb->scb;
				ASSERT((bw + 1) == wlc_scb_ratesel_get_link_bw(mu_info->wlc,
					candidate));
				/* exclude candidate/client without a partner */
				if (nitems[tp_idx][bw] == 1) {
					if (is_client_in_dl_mumimo_clientset(mu_info, candidate)) {
						mu_scb->policy_evict = MUTX_POLICY_EVICT_BW;
						mu_scb->reason = MUTX_REASON_SINGLE_STA_EVICT;
						mutx_scb_list_move(mu_info->admit_list, pos);
						WL_MUTX(("%s "MACF" prune1 tp_idx %d link_bw %d\n",
							__FUNCTION__, ETHER_TO_MACF(candidate->ea),
							tp_idx, bw));
					} else {
						WL_MUTX(("%s "MACF" prune2 tp_idx %d link_bw %d\n",
							__FUNCTION__, ETHER_TO_MACF(candidate->ea),
							tp_idx, bw));
						mutx_scb_list_move(mu_info->mu_cap_stas_list,
							pos);
					}
				} else {
					if (!is_client_in_dl_mumimo_clientset(mu_info, candidate)) {
...
wlc_mutx_evict_all_muclients(wlc_mutx_info_t *mu_info)
{
	...
	WL_ERROR(("wl%d: %s: Force evict all DLMU clients\n", wlc->pub->unit, __FUNCTION__));
	mu_clear_dl_mumimo_clientset(mu_info);
	if (HE_DLOMU_ENAB(wlc->pub)) {
...
/* Report whether a given STA is ready to be an MU-MIMO receiver.
 * STA must MU capable.
 * STA must also be associated to a BSS where this device is acting as an AP.
 * Returns TRUE if STA is MU capable.
 */
static bool
mu_sta_dl_mumimo_ready(wlc_mutx_info_t *mu_info, scb_t *scb, bool strict_check)
{
	uint32 bw_policy;
	uint8 link_bw;
	mutx_dl_mumimo_policy_t *mutx_policy;
	...
	/* Check traffic loads */
	if (mu_info->muclient_pfmon &&
		!wlc_mutx_sta_ac_check(mu_info, scb, link_bw)) {
	...
	if (((!mu_info->client_bw160) && (link_bw == BW_160MHZ)) ||
		...
		(mu_scb->tp != HEMMU)) ||
		(((!mu_info->muclient_pfmon && strict_check) ||
		mu_info->client_samebw) &&
		(link_bw != bw_policy))) {
...
/** @return number of receive antenna's of remote party for DL-MUMIMO reception */
static bool
...
wlc_mutx_scb_state_upd(void *ctx, scb_state_upd_data_t *notif_data)
{
	...
	if ((oldstate & ASSOCIATED) && !SCB_ASSOCIATED(scb)) {
		...
		if (HE_ENAB(wlc->pub) && SCB_HE_CAP(scb)) {
			if (!HE_DLMMU_ENAB(wlc->pub) && !HE_DLOMU_ENAB(wlc->pub)) {
				return;
			}
		...
		mu_scb->tp = (SCB_HE_CAP(scb) &&
			(HE_DLMMU_ENAB(wlc->pub) || HE_DLOMU_ENAB(wlc->pub))) ?
			HEMMU : VHTMU;
 ...
 * Could be used to get current state, modify membership, and call
 * wlc_vht_dl_mumimo_membership_set() with modified state.
 *
 * Inputs:
 *   mu_info -    MU TX state
 ...
 */
int
wlc_vht_dl_mumimo_membership_set(wlc_mutx_info_t *mu_info, uint16 client_index,
	uint8 *membership, uint8 *position)
{
	mutx_scb_t *mu_scb;
	//uint8 max_vht_usrs = wlc_txcfg_max_clients_get(mu_info->wlc->txcfg, VHTMU);
...
/* VHT specific. Callback invoked when tx status is processed for a group ID mgmt frame.
...
wlc_mupkteng_addsta(wlc_mutx_info_t *mu_info, struct ether_addr *ea, int client_idx, int nrxchain)
{
	...
	SCB_VHTMU_ENABLE(scb); /* admit to scheduler */
...
#ifdef WLCNT
/** applicable to MU-MIMO operation, not to MU-OFDMA */
static void
wlc_mu_snr_calib_reset(mutx_scb_t *mutx_scb, int nss)
{
	int i;
	mutx_dl_mumimo_snr_calib_stats_t *mutx_snr_calib_stats = mutx_scb->mutx_snr_calib_stats;
...
wlc_mutx_mutxs_update(wlc_mutx_info_t *mu_info, scb_t *scb,
	mutx_pfmon_stats_t *mutx_pfmon_stats,
	mutx_dl_mumimo_snr_calib_stats_t *mutx_snr_calib_stats,	tx_status_t *txs)
{
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
	mutx_scb_t *mu_scb = NULL;
	mutx_dl_mumimo_stats_t *mutx_stats = NULL;
#endif

	uint8 gbmp = TX_STATUS64_MU_GBMP(TX_STATUS_MACTXS_S4(txs));
	uint8 gpos = TX_STATUS64_MU_GPOS(TX_STATUS_MACTXS_S4(txs));
	uint8 mutxcnt = TX_STATUS64_MU_TXCNT(TX_STATUS_MACTXS_S4(txs));
	uint8 mupktnum = TX_STATUS40_TXCNT_RT0(TX_STATUS_MACTXS_S3(txs));
	uint8 muacknum = TX_STATUS40_ACKCNT_RT0(TX_STATUS_MACTXS_S3(txs));
	uint8 gid = TX_STATUS64_MU_GID(TX_STATUS_MACTXS_S3(txs));
	ratespec_t rspec = TX_STATUS64_MU_RSPEC(TX_STATUS_MACTXS_S4(txs));
	...
	txs_mutp = TX_STATUS_MUTYP(mu_info->wlc->pub->corerev, TX_STATUS_MACTXS_S5(txs));
	...
	ASSERT(rate_index < MUTX_MCSNSS_STORE_ELMTS);
	...
	if (mutx_stats) {
		bool is_sgi = TX_STATUS64_MU_SGI(TX_STATUS_MACTXS_S3(txs));
...
/**
 * Called after either transmission completion, where the packet was part of an MU-MIMO PPDU (so
 * not an MU-OFDMA PPDU). Updates the counters in the MU scb cubby.
 *
 * @param[in] txs_mu    FALSE if packets intended for MU transmission were tx'ed in SU.
 * @param[in] txs       Transmit status
 * @param[in] rs_txs    An assortement of tallies
 */
void BCMFASTPATH
	...
	bool txs_mu, tx_status_t *txs, ratesel_txs_t *rs_txs, uint8 rnum)
{
...
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
	mutx_dl_mumimo_stats_t *mutx_stats;
#endif
	mutx_pfmon_stats_t *mutx_pfmon_stats;
	bool snd_epch_cng;
	mutx_dl_mumimo_snr_calib_stats_t *mutx_snr_calib_stats;
	...
	if (txs_mu) {
		...
		/* Txed as SU */
		txassu_reason  = TX_STATUS128_M2SQ(TX_STATUS_MACTXS_S5(txs));
		snd_epch_cng = (TX_STATUS128_SNDFL_EPCH(TX_STATUS_MACTXS_S5(txs))
		...
		if (snd_epch_cng) {
			...
			mutx_pfmon_stats->snd_epch_prev =
				TX_STATUS128_SNDFL_EPCH(TX_STATUS_MACTXS_S5(txs));
#if defined(BCMDBG) || defined(BCMDBG_DUMP_MUTX)
...
BCMATTACHFN(wlc_mutx_attach)(wlc_info_t *wlc)
{
	...
	mu_info->_swap_mufifotech = FALSE;
	...
	if (D11REV_GE(mu_info->pub->corerev, 129) && D11REV_LE(mu_info->pub->corerev, 132) &&
		...
		/* only 43684b0/b1, 63178/47622, 6710a0/a1 and 6715b0 for now, default MU on
	...
	mu_info->muclient_nrx = MUMIMO_CLIENT_NRX_2;
	...
	mu_info->active = TRUE;
	if ((D11REV_GE(mu_info->pub->corerev, 132) && !wlc_bmac_is_pcielink_slowspeed(wlc->hw)) ||
		(D11REV_IS(wlc->pub->corerev, 130) && D11MINORREV_GE(wlc->pub->corerev_minor, 2))) {
	...
	for (tp_idx = 0; tp_idx < MUTX_TP_MAX; tp_idx++) {
		for (bw = 0; bw < BW_160MHZ; bw++) {
			...
			if (mu_info->mu_plcy_vldn_list[tp_idx][bw] == NULL) {
				wlc_mutx_detach(mu_info);
				return NULL;
			}
		}
	...
	if ((mu_info->mutx_policy = (mutx_dl_mumimo_policy_t *)
		MALLOCZ(wlc->osh, sizeof(mutx_dl_mumimo_policy_t))) == NULL) {
...
BCMATTACHFN(wlc_mutx_detach)(wlc_mutx_info_t *mu_info)
{
	...
	if (mu_info->mutx_policy != NULL) {
		MFREE(mu_info->osh, mu_info->mutx_policy, sizeof(mutx_dl_mumimo_policy_t));
	...
	for (tp_idx = 0; tp_idx < MUTX_TP_MAX; tp_idx++) {
		for (bw = 0; bw < BW_160MHZ; bw++) {
			...
			mutx_scb_list_del(mu_info, mu_info->mu_plcy_vldn_list[tp_idx][bw]);
		}
	}
...
void wlc_mutx_update_hemmu_dsmps(wlc_mutx_info_t *mu_info, scb_t *scb)
{
	...
	mu_scb->reason = MUTX_REASON_DSMPS;
	mu_scb->policy_evict = MUTX_POLICY_EVICT_NOTREADY;
	if (!SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb)) {
		mu_rm_dl_mumimo_client_from_set(mu_info, scb);
	}
