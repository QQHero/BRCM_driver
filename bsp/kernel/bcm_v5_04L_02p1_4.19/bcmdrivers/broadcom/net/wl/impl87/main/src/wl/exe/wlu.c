 ...
 *
 * $Id: wlu.c 810647 2022-04-12 05:16:17Z $
 */
...
static cmd_func_t wl_scb_rateset;
...
static cmd_func_t wl_get_chanspec_reg_txpwr;
static cmd_func_t wl_get_txpwr_adj_est;
static cmd_func_t wl_chan_info;
static cmd_func_t wl_wme_ac_req, wl_add_ie, wl_del_ie, _wl_list_ie, _wl_clr_ie;
...
static cmd_func_t wl_def_6g_scan_type;
...
#if defined(BCMDBG)
static cmd_func_t wl_msg_macfltr;
#endif /* BCMDBG */
...
#if defined(WLTEST)
static cmd_func_t wl_phy_adcasym;
#endif
...
static int wl_mbssid(void *wl, cmd_t *cmd, char **argv);
...
cmd_t wl_cmds[] = {
	...
	{ "chanspec_reg_txpwr", wl_get_chanspec_reg_txpwr, WLC_GET_VAR, -1,
	...
	{"scb_rateset", wl_scb_rateset, WLC_GET_VAR, WLC_SET_VAR,
...
#if defined(BCMDBG)
	{"msg_macfilter", wl_msg_macfltr, -1, WLC_SET_VAR,
	"wl msg_macfilter <xx:xx:xx:xx:xx:xx> \n"
	"\t""wl msg_macfilter 0 - disable mac filter"},
#endif /* BCMDBG */
	...
	{ "clr_ie", _wl_clr_ie, -1, WLC_SET_VAR,
	"flush vendor proprietary IEs" },
	...
	{"def_6g_scan_type", wl_def_6g_scan_type, WLC_SET_VAR, WLC_GET_VAR,
	...
	" [<enable|disable|counters|reset_cnts|status|dump>]"},
	{ "csimon", wl_csimon, WLC_GET_VAR, WLC_SET_VAR,
	"wl csimon [<add> <xx:xx:xx:xx:xx:xx>[<timer value in ms>]] "
#ifdef BCM_CSIMON_AP
	"wl csimon [<addap> <xx:xx:xx:xx:xx:xx> <SSID> [<timer value in ms>]] "
#endif
...
#if defined(WLTEST)
	{ "phy_adcasym", wl_phy_adcasym, WLC_GET_VAR, -1,
	"Get per-core adc asymmetry information in dB format, i.e. 10*log10(I_pwr/Q_pwr)\n" },
#endif
	{ "sh_ssid", wl_sh_ssid, WLC_GET_SH_SSID, -1,
	"Converts the regular SSID to short SSID \n"},
	{ "use_sw_uc_mfp", wl_varint, WLC_GET_VAR, WLC_SET_VAR,
	"Enable/disable software encryption for unicast management frames(1=on, 0=off)\n"},
	{ "mbssid", wl_mbssid, WLC_GET_VAR, WLC_SET_VAR,
	"Get/set mbssid (-1=auto, 0=disable, 1=enable)\n" },
	{ NULL, NULL, 0, 0, NULL }
};
...
wl_macreg(void *wl, cmd_t *cmd, char **argv)
{
	...
	int ret;
	char *endptr = NULL;
	rw_reg_t rwt = {0, 0, 0, 0}; // init all members to 0
	uint argc;
...
wlu_dump_phytbls(void *wl, char *dump_buf)
{
	...
	if (ret != BCME_OK) {
		...
		if (ret != BCME_OK) {
			...
			do {
				strncpy(dump_buf, str_phytbl, WL_DUMP_BUF_LEN);
...
#if defined(BCMDBG)
static int
wl_msg_macfltr(void *wl, cmd_t *cmd, char **argv)
{
	int ret = BCME_USAGE_ERROR, ethlen;
	struct ether_addr ea;
	...
	if (argv[0]) {
		...
		/* get mac address */
		if (!wl_ether_atoe(*argv++, &ea)) {
			/* if arg is 0, send "disable" */
			if (!((ea.octet[0] == 0) && (ethlen == 1)))
				return ret;
		}

		ret = wlu_iovar_setbuf(wl, cmd->name, (void *) &ea,
			sizeof(struct ether_addr), buf, WLC_IOCTL_MEDLEN);
	} else {
		/* Get */
		fprintf(stderr, "%s: Get not supported\n", cmd->name);
	}
...
wl_staprio(void *wl, cmd_t *cmd, char **argv)
{
	...
	memset(&staprio_cfg, 0, sizeof(wl_staprio_cfg_t));
...
#define VAR_OTP    1
	...
	/* Query the driver on where the variable comes from: OTP or SROM */
	memset(buf, 0, WLC_IOCTL_MAXLEN);
	strcpy(buf, "nvram_source");
	ret = wl_get(wl, WLC_GET_VAR, buf, strlen(buf)+1);

	if ((buf[0] != VAR_SROM) && buf[0] != VAR_OTP) {
		printf("Cannot read/write the variable from/to SROM or OTP\n");
		return ret;
	}
	...
	while ((name = *argv++) != NULL) {
		...
		if (ro) {
			...
			else if (srv->flags & SRFL_CCODE)
				printf("%s=0x%x (%c%c)", name, val32, (val32 >> 8), (val32 & 0xff));
			else
...
static int
wl_scb_rateset(void *wl, cmd_t *cmd, char **argv)
{
	int ret = BCME_USAGE_ERROR;
	struct ether_addr ea;
	if (!*++argv)
	...
	memcpy(buf, &ea, sizeof(struct ether_addr));
	ret = wlu_iovar_getbuf(wl, cmd->name, &ea,
			sizeof(struct ether_addr), buf, WLC_IOCTL_MAXLEN);
	if (ret < 0)
		return ret;
	fputs(buf, stdout);
	return 0;
...
wl_power_sel_params(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (lpc_params_use_powersel_params) {
		...
		argv++;
		memset(&pwrsel_params, 0, sizeof(pwrsel_params));
...
wl_scan(void *wl, cmd_t *cmd, char **argv)
{
	...
	params_size += WL_SCAN_PARAMS_SSID_MAX * DOT11_SHORT_SSID_LEN;
...
wl_channels_in_country(void *wl, cmd_t *cmd, char **argv)
{
	...
	len = strlen(*argv);
	if ((len >= sizeof(cic->country_abbrev)) || (len < 2)) {
		fprintf(stderr, "invalid country abbrev: %s\n", *argv);
		return BCME_BADARG;
	}

	strncpy(cic->country_abbrev, *argv, len + 1);
...
static int
wl_get_chanspec_reg_txpwr(void *wl, cmd_t *cmd, char **argv)
{
	int i, err;
	char chspec_str[CHANSPEC_STR_LEN];

	wl_chanspec_reg_txpwr_t *chanspec_reg_txpwr;
	wl_chanspec_reg_txpwr_t params;
	...
	memset(&params, 0, WL_CHANSPEC_REG_TXPWR_LEN);
	params.ver = WL_CHANSPEC_REG_TXPWR_VER;
	params.len = WL_CHANSPEC_REG_TXPWR_LEN;
	params.count = 1;
	params.reg_txpwr[0].chanspec = chanspec;
	...
	chanspec_reg_txpwr = (wl_chanspec_reg_txpwr_t *)buf;

	if (chanspec_reg_txpwr->ver != WL_CHANSPEC_REG_TXPWR_VER) {
		fprintf(stderr, "Error: version [%d] mismatch Driver version:%d\n",
			WL_CHANSPEC_REG_TXPWR_VER, chanspec_reg_txpwr->ver);
		return err;
	}

	for (i = 0; i < (int)(dtoh32(chanspec_reg_txpwr->count)); i++) {
		chanspec = wl_chspec32_from_driver(chanspec_reg_txpwr->reg_txpwr[i].chanspec);
		wf_chspec_ntoa(chanspec, chspec_str);

		printf("%s\t(0x%04x)\t%2d(dbm)\n",
			chspec_str, chanspec,
			chanspec_reg_txpwr->reg_txpwr[i].reg_txpwr);
	}
	printf("\n");
...
wl_join(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!stricmp(*argv, "-x")) {
		argv++;		/* move to the next argument for short SSID */
		short_ssid = strtoul(*argv, 0, 0);
		join_params->ssid.SSID_len = WL_SHORT_SSID_MAGIC_LEN;

		memcpy(join_params->ssid.SSID, &short_ssid, DOT11_SHORT_SSID_LEN);
	} else {	/* Use the default join parameters with out -x option */
...
#if defined(WLTEST)
static int
wl_phy_adcasym(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	uint32 adc_asym;
	int8 k, asym[4] = { 0 };
	...
	if ((ret = wlu_iovar_getint(wl, cmd->name, (int*)&adc_asym)) < 0) {
		return (ret);
	}
	for (k = 0; k < 4; k++) {
		asym[k] = (int8) (adc_asym & 0xFF);
		adc_asym >>= 8;
	}
	printf("adc asymmetry for core{0...3} (in units of 1/16dB): %d %d %d %d\n",
		asym[0], asym[1], asym[2], asym[3]);
...
wl_radar_sc_status(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		...
		if (val == 0) {
			...
			ra.detected = FALSE;
			ret = wlu_var_setbuf(wl, "clear_radar_sc_status",
...
wl_rm_request(void *wl, cmd_t *cmd, char **argv)
{
	...
	while ((opt_err = miniopt(&to, argv)) != -1) {
		...
		if (to.opt == 'c') {
			...
			req.chanspec = CH20MHZ_CHSPEC(to.val, WL_CHANNEL_2G5G_BAND(to.val));
		}
		if (to.opt == 'd') {
			if (!to.good_int) {
				fprintf(stderr,
...
static int
wl_vndr_ie_buflen(char **argv, int *buf_len)
{
	int ielen, datalen;
	...
	*buf_len = sizeof(vndr_ie_setbuf_t) + datalen - 1;
...
wl_vndr_ie(void *wl, const char *command, uint32 pktflag_ok, char **argv)
{
	...
	int bsscfg_idx = 0;
	int consumed = 0;
	char *vndr_ie_iovar_str = "vndr_ie";
	...
	/* Estimate buf length for vendor proprietary IE */
	if ((err = wl_vndr_ie_buflen(argv, &buflen)) != BCME_OK) {
		return err;
	}
	...
	if ((err = wl_mk_ie_setbuf(wl, command, pktflag_ok, argv, ie_setbuf)) != BCME_OK) {
		goto done;
	}

	if (bsscfg_idx == -1)
		err = wlu_var_setbuf(wl, vndr_ie_iovar_str, ie_setbuf, buflen);
	else
		err = wlu_bssiovar_setbuf(wl, vndr_ie_iovar_str, bsscfg_idx,
...
static int
_wl_clr_ie(void *wl, cmd_t *cmd, char **argv)
{
	int err;

	UNUSED_PARAMETER(cmd);
	err = wl_clr_ie(wl, NULL, "vndr_ie", argv);
...
wl_escanresults(void *wl, cmd_t *cmd, char **argv)
{
	...
	while ((retval = select(fd+1, &rfds, NULL, NULL, &tv)) > 0) {
		...
		/* skip escanresults from other interface */
		if (strncmp(event->event.ifname, ifr.ifr_name, sizeof(ifr.ifr_name))) {
			continue;
		}
...
static int
wl_def_6g_scan_type(void *wl, cmd_t *cmd, char **argv)
{
	void *ptr = NULL;
	int err;
	uint32 def_6g_scan_type;
	char *endptr = NULL;
	...
	if (!*++argv) {
		...
		def_6g_scan_type = *(uint32 *)ptr;
		printf("%d\n", def_6g_scan_type);
		if (def_6g_scan_type == SCAN_6G_RNR) {
			printf("Scan type is RNR\n");
		} else if (def_6g_scan_type == SCAN_6G_PSC) {
			printf("Scan type is PSC\n");
		} else if (def_6g_scan_type == SCAN_6G_ALL) {
			printf("Scan type is FULL SCAN\n");
		}
	} else if (!stricmp(*argv, "-h")) {
		return BCME_USAGE_ERROR;
	} else {
		def_6g_scan_type = strtoul(*argv, &endptr, 0);
		if (*endptr != '\0') {
			/* not all the value string was parsed by strtol */
			return BCME_USAGE_ERROR;
		}
		err = wlu_iovar_setint(wl, cmd->name, def_6g_scan_type);
		if (err < 0) {
	...
	if (revinfo.driverrev && dtoh32(revinfo.driverrev) < DINGO_FW_REV) {
		...
		if (mu->ver == RTE_MEMUSEINFO_VER && mu->len >= sizeof(memuse_info_t)) {
			...
			printf("In use + overhead: %d(%dK), Max memory in use: %d(%dK)\n",
			       ...
			       mu->tot, KB(mu->tot));
			if (mu->max_flowring_alloc || mu->max_bsscfg_alloc || mu->max_scb_alloc ||
				mu->total_flowring_alloc || mu->total_bsscfg_alloc ||
				mu->total_scb_alloc) {
...
wl_cca_get_stats(void *wl, cmd_t *cmd, char **argv)
{
	...
	for (channel = base; channel <= limit; channel++) {
		...
		req.chanspec = CH20MHZ_CHSPEC(channel, WL_CHANNEL_2G5G_BAND(channel));
		...
		if (stricmp(param, "-h") == 0) {
			...
			while (param != NULL) {
				...
				if (stricmp(param, "-c") == 0) {
					...
					memset(profile->SSID, 0, DOT11_MAX_SSID_LEN);
					if ((strlen(val)) && (strlen(val) <= DOT11_MAX_SSID_LEN)) {
						memcpy((char *)profile->SSID, (const char*)val,
							MIN(strlen(val), DOT11_MAX_SSID_LEN));
						profile->ssid_length = strlen(val);
					} else {
						printf("Too long or No SSID : %s\n", val);
						err = BCME_USAGE_ERROR;
						break;
					}
...
wl_cca_stats_mesh(void *wl, cmd_t *cmd, char **argv)
{
	...
	for (channel = base; channel <= limit; channel++) {
		...
		/* Get stats for each channel */
		request->chanspec = CH20MHZ_CHSPEC(channel, WL_CHANNEL_2G5G_BAND(channel));
...
wl_txdelay_params(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		...
		memset(&param, 0, sizeof(txdelay_params_t));
		argc = 0;
...
wl_ie(void *wl, cmd_t *cmd, char **argv)
{
	...
	strncpy(ie_setbuf->cmd, "add", VNDR_IE_CMD_LEN);
...
static int wl_stamon_sta_config(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		...
		if (!stricmp(*argv, "add")) {
			...
			return err;
		} else if (!stricmp(*argv, "status")) {
			stamon_status_t *pbuf;
			stamon_cfg.cmd = htodenum(STAMON_CFG_CMD_GET_STATUS);
			memset(buf, 0, WLC_IOCTL_MAXLEN);
			err = wlu_iovar_getbuf(wl, cmd->name, &stamon_cfg,
				sizeof(wlc_stamon_sta_config_t), buf, WLC_IOCTL_MAXLEN);
			pbuf = (stamon_status_t*)buf;
			if (!err) {
				printf("Status : %s\n", dtoh32(pbuf->status) ?
						"Enabled": "Disabled");
			}
			return err;
		} else if (!stricmp(*argv, "dump")) {
			stamon_dump_t *pbuf;
			memset(buf, 0, WLC_IOCTL_MAXLEN);
			pbuf = (stamon_dump_t*)buf;
			stamon_cfg.cmd = htodenum(STAMON_CFG_CMD_DUMP);
			err = wlu_iovar_getbuf(wl, cmd->name, &stamon_cfg,
					sizeof(wlc_stamon_sta_config_t), pbuf, WLC_IOCTL_MAXLEN);
			printf("Status : %s\n", dtoh32(pbuf->status) ? "Enabled": "Disabled");
			if (err || !(pbuf->length)) {
				printf("Report buffer length too short: %d < %d\n",
					(int)pbuf->length, (int) pbuf->count);
				return BCME_BUFTOOSHORT;
			}
			if (!err) {
				printf("Monitoring %d stations \n", pbuf->count);
				printf("%-8s %-18s %8s   %-18s %-14s\n", "AMT_Idx", "MAC_Address",
					"RSSI", "Chanspec", "Off-chan_Time");

				for (i = 0; i < pbuf->count && i < max; i++) {
					printf("%-8d "MACF"  %5ddBm   0x%04x %-10s %10ums\n",
						dtoh32(pbuf->sta_data[i].rcmta_idx),
						ETHER_TO_MACF(pbuf->sta_data[i].ea),
						dtoh32(pbuf->sta_data[i].rssi),
						dtoh32(pbuf->sta_data[i].chanspec),
						wf_chspec_ntoa(pbuf->sta_data[i].chanspec,
							chanspec_str),
						dtoh32(pbuf->sta_data[i].offchan_time));
				}
			}
			pbuf->version = dtoh32(pbuf->version);
			pbuf->count = dtoh32(pbuf->count);
			printf("Counters : data %u, mngt %u, cntl %u\n",
				dtoh32(pbuf->rxstamondata), dtoh32(pbuf->rxstamonmngt),
				dtoh32(pbuf->rxstamoncntl));
			return err;
		} else {
...
wl_csimon(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		...
		if (!stricmp(*argv, "add")) {
...
#ifdef BCM_CSIMON_AP
		else if (!stricmp(*argv, "addap")) {
			csimon_cfg.cmd = htodenum(CSIMON_CFG_CMD_ADD_AP);
			if (!*++argv || !wl_ether_atoe(*argv, &csimon_cfg.ea)) {
				printf(" ERROR: no valid ethernet address provided\n");
				return BCME_BADARG;
			}
			...
			val = *++argv;
			if (*argv) {
				csimon_cfg.ssid_len = strlen(val);
				strcpy((char *)&csimon_cfg.SSID, (const char *)val);
			}
			++argv;
			if (*argv) {
				monitor_interval = strtoul(*argv, &endptr, 0);
				...
				printf("CSIMON AP monitor interval %d\n", monitor_interval);
			}
		}
#endif  /* BCM_CSIMON_AP */
		else if (!stricmp(*argv, "del")) {
	...
	bool recent, char *out_str)
{
	...
	const uint8 legacy_mbps_len = ARRAYSIZE(legacy_mbps);
	char str[16];
...
wl_rate_histo_map_print(wl_rate_histo_map1_t *map1, wl_rate_histo_map2_t *map2)
{
	...
	uint32 totVHT = 0, totVHTBW[4] = {0}, totVHTGE8 = 0, totVHTNSS[4] = {0};
...
int
wl_mbssid(void *wl, cmd_t *cmd, char **argv)
{
	int err = 0;
	...
	if (*++argv == NULL) {
		...
		cfg = (wl_mbssid_t *) ptr;
		if (cfg->version != WL_MBSSID_VER || cfg->length < sizeof(wl_mbssid_t)) {
			return -1;
		}
		configstr = (cfg->config == -1)? "AUTO" :
			((cfg->config == 1)? "ENABLED": "DISABLED");
		statusstr = (cfg->status == -1)? "AUTO":
			((cfg->status == 1)? "ENABLED": "DISABLED");
		printf("Config:[%d][%s]\nStatus:[%d][%s] \n", cfg->config, configstr,
			cfg->status, statusstr);
	} else {
		/* Set */
		char *param = *argv++;
		wl_mbssid_t cfg;
		...
		memset(&cfg, 0, sizeof(wl_mbssid_t));
		cfg.version = WL_MBSSID_VER;
		cfg.length = sizeof(wl_mbssid_t);
		cfg.config = (int8)(strtol(param, &endptr, 0) & 0xFF);
		if (*endptr != '\0') {
			return -1;
		}
		err = wlu_var_setbuf(wl, cmd->name, &cfg, sizeof(wl_mbssid_t));
	}
	return err;
}
...
wl_hc_setints(void *wl, uint16 category, struct hc_sub_cmd_ent *hc_cmd, int int_count, char **argv)
{
	...
	int_count = sizeof(words) / sizeof(words[0]);
	for (i = 0; i < int_count; i++) {
...
dump_bss_info(void *bi_generic)
{
	...
	if (version == LEGACY_WL_BSS_INFO_VERSION) {
		...
		bi->chanspec = CH20MHZ_CHSPEC(bi_v107->channel,
				WL_CHANNEL_2G5G_BAND(bi_v107->channel));
	...
	printf("Beacon Interval: %u msecs\t", dtoh16(bi->beacon_period));
	if (flags & WL_BSS_FLAGS_FROM_BEACON)
		printf("DTIM Period: %u", bi->dtim_period);
	printf("\n");
	...
	if (version != LEGACY_WL_BSS_INFO_VERSION && (bi->n_cap || bi->he_cap)) {
		...
		if (bi->n_cap && !CHSPEC_IS6G(bi->chanspec)) {
	...
	if (*++argv) {
		if (!strncmp(*argv, "-h", strlen("-h")) ||
			...
			strncpy(musched_params->keystr, *argv,
				MIN(strlen(*argv), WL_MUSCHED_CMD_KEYSTR_MAXLEN-1));
			argv++;
		}
	}
		...
		if (!strncmp(*argv, "-h", strlen("-h")) ||
			...
			if (strstr(*argv, "[") != NULL && strstr(*argv, "]") != NULL) {
				...
				strncpy(musched_params->argstr, *argv,
					MIN(strlen(*argv), WL_MUSCHED_CMD_ARGSTR_MAXLEN-1));
...
wl_srcfg_cmd(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*++argv) {
		if (!strncmp(*argv, "-h", strlen("-h")) ||
			...
			if (strlen(*argv) >= sizeof(srcfg_params->cmdstr)) {
				fprintf(stderr, "Error: command name is too big \n");
				ret = BCME_BADARG;
				goto exit;
			}
			strncpy(srcfg_params->cmdstr, *argv, strlen(*argv) + 1);
			argv++;
		}
	} else {
		...
		if (strlen(dis_argv) >= sizeof(srcfg_params->cmdstr)) {
			fprintf(stderr, "Error: command name is too big \n");
			ret = BCME_BADARG;
			goto exit;
		}
		strncpy(srcfg_params->cmdstr, dis_argv, strlen(dis_argv) + 1);
...
wl_fifo_cmd(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!strncmp(*argv, "-h", strlen("-h")) ||
		...
		strncpy(fifo_msg.keystr, *argv,
			MIN(strlen(*argv), WL_FIFO_CMD_KEYSTR_MAXLEN - 1));
		argv++;
	}
