 ...
 *
 * $Id: wlc_lq.c 810901 2022-04-19 10:44:35Z $
 */
...
struct chanim_info {
    ...
    chanim_acc_us_t acc_us; /* accumulative counts since last time close */
};
...
wlc_lq_rssi_ma(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb)
{
    ...
    for (i = 0, cnt = 0; i < MA_WIN_SZ(blqi->rssi_window_sz); i++) {
        ...
        if (slqi->rssi_window && slqi->rssi_window[i] != WLC_RSSI_INVALID) {
#else
...
wlc_lq_chanim_find_stats(wlc_info_t *wlc, chanspec_t chanspec)
{
    ...
    /* Reading phy_noise from phy on channel switch */
    wlc->phy_noise = wlc_lq_chanim_phy_noise(wlc);
    return stats;
}
...
wlc_lq_chanim_meas(wlc_info_t *wlc, chanim_cnt_t *chanim_cnt, chanim_cnt_us_t *chanim_cnt_us)
{
...
#ifdef WLCHANIM_US
        chanim_cnt_us->ccastats_cnt[i] = ccastats_cnt[i];
        if (i >= 0 && i <= 4) {
    ...
    if (D11REV_GE(wlc->pub->corerev, 40)) {
        ...
        chanim_cnt->ccastats_cnt[CCASTATS_OBSS] += cca_wifi;
        chanim_cnt->ccastats_cnt[CCASTATS_NOPKT] += cca_edcrs;
#ifdef WLCHANIM_US
        chanim_cnt_us->ccastats_cnt[CCASTATS_OBSS] += cca_wifi;
    ...
    bool chan_switch)
{
    ...
    if (acc_us) {
        ...
        if (wlc && wlc->chanim_info)
            wlc->chanim_info->last_cnt_us.start_tm = OSL_SYSUPTIME();
    }
#endif /* WLCHANIM_US */
}
    ...
    wlc_chanim_stats_us_t *cur_stats_us, bool chan_switch)
{
    ...
    if (acc->stats_ms > 0) {
        if (aci_chan_vld_dur > 0) {
            cur_stats->chanim_stats.glitchcnt =
                acc->glitch_cnt * 1000 / aci_chan_vld_dur;

            cur_stats->chanim_stats.bphy_glitchcnt = acc->bphy_glitch_cnt *
                1000 / aci_chan_vld_dur;

            cur_stats->chanim_stats.badplcp =
                acc->badplcp_cnt * 1000 / aci_chan_vld_dur;
        ...
        for (i = 0; i < CCASTATS_MAX; i++) {
            /* copy accumulative cca stats for the applications themselves
             * to caculate the percentage
             */
            if (i == CCASTATS_TXOP)
                cur_stats->chanim_stats.acc_ccastats[i]
                    = acc->ccastats_us[i] * slottime;
            else
                cur_stats->chanim_stats.acc_ccastats[i] = acc->ccastats_us[i];
        ...
        /* calc channel utilization */
        cur_stats->chanim_stats.channel_utilization = 0;
        busy = acc->ccastats_us[CCASTATS_TXDUR] +
            acc->ccastats_us[CCASTATS_INBSS] +
            acc->ccastats_us[CCASTATS_OBSS] +
            acc->ccastats_us[CCASTATS_NOCTG] +
            acc->ccastats_us[CCASTATS_NOPKT];
        stats_frac = (uint8)CEIL(100 * busy, acc->stats_ms * 1000);
        if (stats_frac > 100) {
            WL_INFORM(("stats(%d) > 100: busy_us: %d, acc->statss_ms: %d\n",
                stats_frac, busy, acc->stats_ms));
            stats_frac = 100;
        }
        cur_stats->chanim_stats.channel_utilization = stats_frac;
        ...
        cur_stats->chanim_stats.acc_ms = acc->stats_ms;
    }
#ifdef WLCHANIM_US
    last_cnt_us = &c_info->last_cnt_us;
    cur_time = OSL_SYSUPTIME();
    time_diff = cur_time - last_cnt_us->start_tm;
    cur_stats_us->chanim_stats_us.total_tm += time_diff*1000;
    ...
    chanspec_t prev_chanspec)
{
     ...
     * this value.
     */
    wlc_lq_chanim_accum(wlc, chanspec, &if_info->accum, &c_info->acc_us);

    /* Close the previous channel. */
    wlc_lq_chanim_close(wlc, prev_chanspec, &if_info->accum, if_info->stats,
        &c_info->acc_us, if_info->stats_us, TRUE);
            ...
            if (!wf_chspec_malformed(if_info->chanspec)) {
                ...
                if ((wlc->pub->now % chanim_config(c_info).sample_period) == 0) {
                    wlc_lq_chanim_close(wlc, if_info->chanspec,
                        &if_info->accum, if_info->stats, &c_info->acc_us,
...
wlc_lq_chanim_acc_reset(wlc_info_t *wlc)
{
    ...
    if (if_info != NULL) {
        wlc_lq_chanim_clear_acc(wlc, &if_info->accum, &c_info->acc_us, FALSE);
...
int td_em_chanim_get_stats_cb(chanim_info_t *c_info, wl_chanim_stats_t* iob, int *len, int cnt)
{
    int ret;
    ret = wlc_lq_chanim_get_stats(c_info, iob, len, cnt);
    return ret;
}
#endif
