 ...
 *
 * $Id: wlc_ampdu.c 811011 2022-04-21 16:32:25Z $
 */
...
#define AMPDU_INI_OFF_TIMEOUT_INITIAL    20    /**< initial # of sec in off state */
#define AMPDU_INI_OFF_TIMEOUT_MAX    (15*60)    /**< max # of sec in off state */
...
struct scb_ampdu_tx {
    ...
    /**< store the first addba token for each tid */
    uint8 addba_token_base[AMPDU_MAX_SCB_TID];
    ...
    uint8 fail_cnt_tid[AMPDU_MAX_SCB_TID];    /**< per-tid consecutive addba failure */
};
...
static scb_ampdu_tid_ini_t *wlc_ampdu_init_tid_ini(ampdu_tx_info_t *ampdu_tx,
    scb_ampdu_tx_t *scb_ampdu, uint8 tid, bool override, bool send_addba_req);
...
static int wlc_ampdu_send_addba_req(wlc_info_t *wlc, struct scb *scb, uint8 tid, uint16 wsize,
    uint8 ba_policy, uint8 delba_timeout);
static uint8 wlc_ampdu_get_token(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid);
static bool wlc_ampdu_token_is_valid(scb_ampdu_tx_t *scb_ampdu, uint8 tid, uint8 token);
static void wlc_ampdu_token_cleanup(scb_ampdu_tx_t *scb_ampdu, uint8 tid);
...
static uint16 wlc_ampdu_next_txq_avail(wlc_info_t* wlc, struct scb *scb, uint prec,
    uint* p_next_fid);
...
BCMATTACHFN(wlc_ampdu_tx_cfg_init)(wlc_info_t *wlc, ampdu_tx_config_t *ampdu_tx_cfg)
{
    ...
    if (D11REV_IS(wlc->pub->corerev, 61)) {
        ampdu_tx_cfg->mpdu_density = AMPDU_DENSITY_16_US;
    } else {
...
#endif /* !DONGLEBUILD */
            scb_ampdu->addba_token_base[i] = 1;
        }
    }
    return 0;
} /* scb_ampdu_tx_init */
    ...
    uint8 ampdu_ini_dead_timeout)
{
    ...
    if (ini->dead_cnt >= ampdu_ini_dead_timeout) {
        ...
        WL_PRINT(("ampdu_dbg: wl%d.%d "MACF" scb:%p tid:%d inflight %d %d"
            " lmem_idx %d flags3 0x%x\n",
            wlc->pub->unit, WLC_BSSCFG_IDX(SCB_BSSCFG(scb)),
            ETHER_TO_MACF(scb->ea), scb, ini->tid,
            SCB_PKTS_INFLT_FIFOCNT_VAL(scb, ini->tid),
            SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb),
            (RATELINKMEM_ENAB(wlc->pub) ? wlc_ratelinkmem_get_scb_link_index(wlc, scb) :
            D11_RATE_LINK_MEM_IDX_INVALID), scb->flags3));

        WL_PRINT(("ampdu_dbg: dead_cnt %d tx_in_transit %d ",
...

{
    ...
    ASSERT(bsscfg != NULL);
    interval = 1 + ((wlc_apps_get_listen_prd(wlc, scb) *
           bsscfg->current_bss->beacon_period)/1000);
    ...
    if (ini->addba_req_cnt >= ampdu_addba_req_retry) {
        ...
        if ((ini->addba_req_cnt < 10 &&
...
#endif /* MFP */
            {
                ...
                WL_AMPDU_ERR_EX_PFX(scb, "addba timed out %d\n",
                    ini->addba_req_cnt);
                err = wlc_ampdu_send_addba_req(wlc, scb, ini->tid,
                ...
                if (! err) {
                    ...
                    WL_AMPDU_TX_EX_PFX(scb, "addba blocked: %d try(%d/%d)\n",
                        err, ini->addba_req_cnt, ampdu_addba_req_retry);
                }
            }
    ...
    scb_ampdu_tid_ini_t *ini, uint8 tid)
{
    ...
    WL_AMPDU_EX(scb, ("cleaning up tid %d from poff phydebug %08x txop %08x fifordy ",
        tid, phydebug, txop));
    wlc_bmac_hfifo_rdy_get(wlc->hw, fifordy);
    for (i = NFIFO_U16(wlc)-1; i >= 0; i--) {
        j = NUM_BITS_U16*i;
        WL_AMPDU_EX_NPFX(scb, ("%d_%d:%04x ", j+16, j, fifordy[i]));
    }
    WL_AMPDU_EX_NPFX(scb, ("\n"));
    ...
    AMPDUSCBCNTINCR(scb_ampdu->cnt.txstuck);
    ampdu_dump_ini(wlc, ini);
    wlc_ampdu_dbg_stats(wlc, scb, ini, 0);
...
static void
wlc_ampdu_ba_off(wlc_info_t *wlc, struct scb *scb, scb_ampdu_tid_ini_t *ini, uint8 tid)
{
    ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
    scb_ampdu_tx_t *scb_ampdu;
    ...
    if (ini->off_cnt) {
        ini->off_cnt--;
        if (ini->off_cnt == 0) {
            wlc_ampdu_token_cleanup(scb_ampdu, tid);
            /* Transition into BA_PENDING_OFF, destroy the ini */
            ampdu_cleanup_tid_ini(ampdu_tx, scb, tid, TRUE);
        }
    }
}
    ...
    void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
    ...
    case IOV_SVAL(IOV_AMPDU_SEND_ADDBA):
    {
        ...
        if (!wlc_ampdu_init_tid_ini(ampdu_tx, scb_ampdu, ea_tid->tid, TRUE, TRUE)) {
            err = BCME_ERROR;
            break;
        }
...
#ifdef WLTAF
    wlc_taf_sched_state(wlc->taf_handle, NULL, ALLPRIO, mpdu_density, TAF_NO_SOURCE,
...
uint8
wlc_ampdu_tx_get_mpdu_density(ampdu_tx_info_t *ampdu_tx)
{
    return ampdu_tx->config->mpdu_density;
}
...
wlc_ampdu_agg_pktc(void *ctx, struct scb *scb, void *p, uint prec)
{
    ...
    if (tid >= AMPDU_MAX_SCB_TID) {
        WL_AMPDU_EX_PFX(scb, "Received tid is wrong -- returning\n");
        goto txmod_ampdu;
    }
    ...
    if ((bsscfg_ampdu->txaggr_override == OFF) ||
        ...
        WL_AMPDU_EX_PFX(scb, "tx agg off (txaggr ovrd %d, TID bmap 0x%x)-- returning\n",
    ...
    if (!(ini = scb_ampdu->ini[tid])) {
        ini = wlc_ampdu_init_tid_ini(ampdu_tx, scb_ampdu, tid, FALSE, TRUE);
        if (!ini) {
            WL_AMPDU_TX_EX_PFX(scb, "ini NULL -- returning\n");
            goto txmod_ampdu;
        }
    }
    ...
    while (1) { /* loop over each packet in the caller supplied chain */
        ...
        /* AMSDU agg is done at SCB ampdu release time */
        if (n == NULL) {
#if defined(BCM_PKTFWD_FLCTL)
    ...
    WL_AMPDU_ERR_EX_PFX(scb, "txq overflow\n");
...
wlc_ampdu_agg(void *ctx, struct scb *scb, void *p, uint prec)
{
    ...
    if (tid >= AMPDU_MAX_SCB_TID) {
        SCB_TX_NEXT(TXMOD_AMPDU, scb, p, prec);
        WL_AMPDU_EX_PFX(scb, "tid wrong -- returning\n");
        return;
    }
    ...
    if (!(ini = scb_ampdu->ini[tid])) {
        ...
        if (!ini) {
            SCB_TX_NEXT(TXMOD_AMPDU, scb, p, prec);
            WL_AMPDU_EX_PFX(scb, "ini NULL -- returning\n");
            return;
        }
    }
...
wlc_ampdu_taf_sqs_link_init(wlc_info_t *wlc, struct scb *scb, uint8 prio)
{
    ...
    if (!(ini = scb_ampdu->ini[prio])) {
        ini = wlc_ampdu_init_tid_ini(ampdu_tx, scb_ampdu, prio, FALSE, TRUE);
    }
    return (void*)ini;
}
#endif /* WLSQS */
...
wlc_ampdu_pkt_freed(wlc_info_t *wlc, void *pkt, uint txs)
{
    ...
    if (!ini) {
        WL_AMPDU_ERR_EX_PFX(scb, "NULL ini or pon state for tid %d\n", PKTPRIO(pkt));
#ifdef WLTAF
        wlc_taf_txpkt_status(wlc->taf_handle, scb, PKTPRIO(pkt), pkt,
            TAF_TXPKT_STATUS_PKTFREE_RESET);
#endif
    ...
    if ((ini->ba_state != AMPDU_TID_STATE_BA_ON) &&
        ...
        WL_AMPDU_ERR_EX_PFX(scb, "ba state %d for tid %d\n",
                      ini->ba_state, PKTPRIO(pkt));
#ifdef WLTAF
        wlc_taf_txpkt_status(wlc->taf_handle, scb, ini->tid, pkt,
            TAF_TXPKT_STATUS_PKTFREE_RESET);
#endif
        return;
    }
    ...
    if (!aqm &&
        ...
        WL_AMPDU_ERR_EX_PFX(scb, "unexpected seq 0x%x, start seq 0x%x, max seq %0x, "
                  "tx_in_transit %u\n",
...
static uint16
wlc_ampdu_next_txq_avail(wlc_info_t* wlc, struct scb *scb, uint prec, uint* p_next_fid)
{
    uint next_fid = SCB_TXMOD_NEXT_FID(scb, TXMOD_AMPDU);
    ...
    scb_ampdu_tid_ini_t *ini, bool force, taf_scheduler_public_t *taf)
{
    ...
    uint16 next_txq_avail; /**< #mpdus in nexfid (common/APPS) queue */
    ampdu_tx_config_t *ampdu_tx_cfg = ampdu_tx->config;
    wlc_info_t *wlc = ampdu_tx->wlc;
    wlc_bsscfg_t *cfg;
    struct scb *scb;
    ...
    /* AMPDU Qs carry packet count instead of MPDU count */
#if defined(WLAMSDU_TX)
    ...
    next_txq_avail = wlc_ampdu_next_txq_avail(wlc,
        scb, WLC_PRIO_TO_PREC(ini->tid), &next_fid);
    ...
    if (AMPDU_AQM_ENAB(wlc->pub)) {
        ...
        if (!ps_pretend_limit_transit && SCB_PS_PRETEND_WAS_RECENT(scb)) {
            ps_pretend_limit_transit =
                    wlc_pspretend_limit_transit(wlc->pps_info, scb, in_transit, TRUE);
        }
#ifdef PROP_TXSTATUS
        if (PSPRETEND_ENAB(wlc->pub) && SCB_TXMOD_ACTIVE(scb, TXMOD_APPS) &&
             ...
             */
            ASSERT(SCB_PS(scb) || SCB_TWTPS(scb));
        ...
        if (!taf && !force && (
            ...
            ((next_fid == TXMOD_TRANSMIT) &&
                (next_txq_avail <  MIN(AMPDU_AQM_RELEASE_DEFAULT,
                (pktqprec_max_pkts(WLC_GET_CQ(SCB_WLCIFP(scb)->qi),
                    WLC_PRIO_TO_PREC(ini->tid)) / 2)))) ||
            ps_pretend_limit_transit)) {

            WL_AMPDU_TX_EX_PFX(scb, "Stop Releasing %d in_transit %d tid %d "
                "wm %d rem_wdw %d qlen %d force %d txq_avail %d pps %d\n",
                n_mpdus_to_release, in_transit,
                ini->tid, ampdu_tx_cfg->tx_rel_lowat, wlc_ampdu_get_rem_window(ini),
                qlen, force, next_txq_avail, ps_pretend_limit_transit);
            AMPDUSCBCNTINCR(scb_ampdu->cnt.txnoroom);
            n_mpdus_to_release = 0;
        }
    }

    if (next_txq_avail <= n_mpdus_to_release) {
        /* change decision, as we are running out of space in next_fid tx queue */
#ifdef WLTAF
        if (taf) {
            WL_TAFF(wlc, "common queue full (%u/%u)\n", n_mpdus_to_release,
                next_txq_avail);
            n_mpdus_to_release = next_txq_avail ? next_txq_avail - 1 : 0;
        ...
        force /* || AMPDU_AQM_ENAB(wlc->pub) */ )
    {
        if (!taf) {
            ...
            WL_AMPDU_TX_EX_PFX(scb, "Releasing %d mpdus: in_transit %d "
        ...
        wlc_ampdu_release(ampdu_tx, scb_ampdu, ini,
            n_mpdus_to_release, next_txq_avail, taf);
#ifdef WLTAF
        if (taf) {
    ...
    if ((ini->tx_in_transit == 0) && (in_transit != 0)) {
        WL_AMPDU_ERR_EX_PFX(scb, "Cannot release: in_transit %d tid %d "
            "start_seq 0x%x barpending_seq 0x%x max_seq 0x%x\n",
            in_transit, ini->tid, ini->start_seq, ini->barpending_seq, ini->max_seq);
    }

    WL_AMPDU_TX_EX_PFX(scb, "Cannot release: in_transit %d tid %d wm %d "
                 "rem_wdw %d qlen %d release %d\n",
                 in_transit, ini->tid, ampdu_tx_cfg->tx_rel_lowat,
    ...
    scb_ampdu_tid_ini_t *ini, uint16 release, uint16 q_avail, taf_scheduler_public_t* taf)
{
    ...
    WL_AMPDU_TX_EX_PFX(scb, "start release %d%s\n", release, taf ? " TAF":"");
    ...
    /* block data packets if new key exchange is in progress */
    ...
    if (!AMPDU_AQM_ENAB(pub)) {
        ...
        if (ini->tx_in_transit == 0 && (delta + release > ini->non_aqm->rem_window)) {
            ...
            if (wlc_ampdu_window_is_passed(delta + release, ini)) {
                WL_AMPDU_ERR_EX_PFX(scb, "cannot release %d(out of window)\n",
                        release);
                goto release_done;
            }
        }
    ...
    while (do_release && (q_avail > 0)) { /* loop over each packet to release */
...
#ifdef WLTAF
        if (taf && TAF_IS_TAGGED(WLPKTTAG(p))) {
         ...
         * AMSDU agg is done during SCB AMPDU release
...
#endif /* PROP_TXSTATUS */
        {
            uint8 extra_n = wlc_amsdu_agg_attempt(ami, scb, txq, p, tid, tid);
#if defined(BCM_PKTFWD_FLCTL)
            if (extra_n) {
                __wlc_ampdu_update_link_credits(wlc, scb_ampdu->scb,
                    tid, extra_n, TRUE);
            }
#endif /* BCM_PKTFWD_FLCTL */
#ifdef WLTAF
    ...
    uint16 seq, bool suppress_pkt)
{
    ...
    WL_MSG_MACFILTER_PFX_INIT(ini->scb);

    if (ini->tx_exp_seq != seq) {
        offset = MODSUB_POW2(seq, ini->tx_exp_seq, SEQNUM_MAX);
        WL_AMPDU_ERR_EX_PFX(ini->scb, "r0hole: tx_exp_seq 0x%x, seq 0x%x\n",
            ini->tx_exp_seq, seq);
        ...
        if ((offset > SEQNUM_MAX / 2) &&
            ...
            MODSUB_POW2(ini->max_seq, ini->start_seq, SEQNUM_MAX))) {
            WL_AMPDU_ERR_EX_PFX(ini->scb, "rlag: seq %d tx_exp_seq %d max_seq %d"
...
ampdu_detect_seq_hole(ampdu_tx_info_t *ampdu_tx, uint16 seq, scb_ampdu_tid_ini_t *ini)
{
    ...
    if (seq != ini->next_enq_seq) {
        WL_AMPDU_TX_EX(ini->scb, ("seq hole detected! 0x%x 0x%x\n",
            seq, ini->next_enq_seq));
        has_hole = TRUE;
    }
    ...
    scb_ampdu_tid_ini_t *ini, uint16 seq, bool suppress_pkt)
{
    ...
    if (txretry == 0) {
        if (ini->tx_exp_seq != seq) {
            ...
            /* pkts bypassed (or lagging) on way down */
            if (offset > SEQNUM_MAX / 2) {
                WL_AMPDU_ERR_EX_PFX(ini->scb, "rlag: tx_exp_seq 0x%x, seq 0x%x\n",
                    ini->tx_exp_seq, seq);
                WLCNTINCR(ampdu_tx->cnt->txrlag);
                AMPDUSCBCNTINCR(scb_ampdu->cnt.txrlag);
                return FALSE;
            ...
            if (MODSUB_POW2(seq, ini->start_seq, SEQNUM_MAX) >= ini->ba_wsize) {
                WL_AMPDU_ERR_EX_PFX(ini->scb, "unexpected seq: start_seq 0x%x, "
                    "seq 0x%x\n", ini->start_seq, seq);
                WLCNTINCR(ampdu_tx->cnt->txrlag);
                AMPDUSCBCNTINCR(scb_ampdu->cnt.txrlag);
                return FALSE;
        ...
        if (non_aqm->retry_cnt == 0 ||
            ...
            if (!found) {
                WL_AMPDU_ERR_EX_PFX(ini->scb, "rnlag: tx_exp_seq 0x%x, seq 0x%x\n",
                    ini->tx_exp_seq, seq);
                WLCNTINCR(ampdu_tx->cnt->txrlag);
                AMPDUSCBCNTINCR(scb_ampdu->cnt.txrlag);
                return FALSE;
            ...
            while (non_aqm->retry_seq[non_aqm->retry_head] != seq) {
                WL_AMPDU_ERR_EX_PFX(ini->scb, "rnhole: retry_seq 0x%x, seq 0x%x\n",
                    non_aqm->retry_seq[non_aqm->retry_head], seq);
...
wlc_ampdu_change_epoch(ampdu_tx_info_t *ampdu_tx, int fifo, int reason_code)
{
    ...
    case AMU_EPOCH_CHG_TAF_STAR:
        ...
        break;
#endif /* WLTAF */
    case AMU_EPOCH_CHG_PPS:
        WLCNTINCR(ampdu_tx->cnt->echgr9);
        WLC_AMPDU_EPOCH_REASON(str, "PPS");
        break;
    default:
        ASSERT(0);
        break;
    ...
    struct scb *scb, uint8 tid, bool htc, wlc_txh_info_t* txh_info)
{
    ...
    hwagg->prev_htc = htc;
    hwagg->prev_pps = ltoh16(txh_info->MacTxControlHigh) & D11AC_TXC_PPS;
    if (isAmpdu) {
...
wlc_txs_was_acked(wlc_info_t *wlc, tx_status_macinfo_t *status, uint16 idx)
{
    ...
    if (idx < 32) {
        return (status->mactxs->ack_map1 & (1 << idx)) ? TRUE : FALSE;
    } else {
        idx -= 32;
        return (status->mactxs->ack_map2 & (1 << idx)) ? TRUE : FALSE;
    }
}
    ...
    struct spktq *output_q, int *pkt_cnt, uint fifo)
{
    ...
    if (!SCB_AMPDU(scb)) {
        WL_AMPDU_EX_PFX(scb, "entry func: scb ampdu flag null -- error\n");
    ...
    if (!ini || (ini->ba_state == AMPDU_TID_STATE_BA_PENDING_ON) ||
        ...
        WL_AMPDU_EX_PFX(scb, "bailing out for bad ini (%p)/bastate\n",
            OSL_OBFUSCATE_BUF(ini));
#ifdef WLTAF
        wlc_taf_txpkt_status(wlc->taf_handle, scb, tid, p,
            TAF_TXPKT_STATUS_PKTFREE_DROP);
        wlc_taf_link_state(wlc->taf_handle, scb, tid, TAF_NO_SOURCE,
    ...
    if (wlc->block_datafifo) {
        WL_AMPDU_TX_EX_PFX(scb, "datafifo blocked 0x%x\n", wlc->block_datafifo);
        return BCME_BUSY;
    }

#if defined(WL_PS_SCB_TXFIFO_BLK)
    if (SCB_PS_TXFIFO_BLK(scb)) {
        WL_AMPDU_EX_PFX(scb, "txfifo blocked for PS STA\n");
        return BCME_BUSY;
    }
#endif /* WL_PS_SCB_TXFIFO_BLK */
...
drop_packet:
    ...
    /* End apsd usp if it is applicable */
    if (SCB_PS(scb) && (WLPKTTAG(p)->flags & WLF_APSD)) {
...
//#endif /*dump_stack_qqdx_print*/
    ...
    while (p) {
...
#ifdef PKTC
        if ((err = wlc_prep_sdu_fast(wlc, cfg, scb, p, fifo, &key)) == BCME_UNSUPPORTED)
#endif /* PKTC */
        {
            void *pkts[DOT11_MAXNUMFRAGS];
            /* prep_sdu converts an SDU into MPDU (802.11) format, but it takes an
             * array of packets as input because in case of non-ampdu it might fragment
             * the packet and that requires an array to store the new packets.
             */
            pkts[0] = p;
            npkts = 1;
            err = wlc_prep_sdu(wlc, scb, pkts, &npkts, fifo);
            ASSERT(npkts <= 1);
            /* When SW encryption is active (large number of STAs) then packet can get
             * replaced with other packet.
             */
            p = pkts[0];
        }
        ...
        if (err) {
            if (err == BCME_BUSY) {
                WL_AMPDU_TX_EX(scb, ("prep_xdu retry; seq 0x%x\n", seq));
            ...
            WL_AMPDU_ERR_EX(scb, ("prep_xdu rejected; seq 0x%x\n", seq));
        ...
        WL_AMPDU_TX_EX(scb, ("prep_xdu success; seq 0x%x tid %d\n", seq, tid));
        ...
        if (ETHER_ISNULLADDR(&h->a1)) {
            WL_AMPDU_ERR_EX(scb, ("dropping frame with null a1\n"));
            WLCNTINCR(ampdu_tx->cnt->txdrop);
            if (ini->tx_exp_seq == seq) {
                setbit(non_aqm->barpending, indx);
                wlc_ampdu_send_bar(ampdu_tx, ini, SEQNUM_CHK);
        ...
        if (!WL_AMPDU_HW_ON()) {
            WL_AMPDU_TX_EX(scb, ("mpdu %d plcp_len %d\n", count, len));
        }
        ...
        WL_AMPDU_TX_EX(scb, ("ampdu_len %d seg_cnt %d null delim %d\n",
            ampdu_len, seg_cnt, ndelim));
        txh->MacTxControlLow = htol16(mcl);
        ...
        if (AMPDU_HOST_ENAB(wlc->pub)) {
            ...
            if ((MCS_RATE(mcs, TRUE, FALSE) >= f->dmaxferrate) &&
                ...
                WL_AMPDU_ERR_EX(scb, ("PR 37644: stopping ampdu at %d for mcs %d\n",
                    count, mcs));
                break;
            }
    ...
    if (count) {
        ...
        if (AMPDU_HOST_ENAB(wlc->pub)) {
            ...
            WL_AMPDU_TX_EX(scb, ("count %d ampdu_len %d\n",
                count, ampdu_len));
    ...
    if (regmpdu) {
        WL_AMPDU_TX_EX(scb, ("sending regular mpdu\n"));
    ...
    FOREACH_BSS(wlc, idx, cfg) {
        FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
            ...
            for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
                ...
                wlc_ampdu_send_addba_req(wlc, scb, tid,
    ...
    struct spktq *output_q, int *pkt_cnt, uint fifo)
{
    ...
    while (TRUE) {
        ...
        if (BCMPCIEDEV_ENAB() && PROP_TXSTATUS_ENAB(wlc->pub) &&
            ...
            if (WLPKTFLAG_AMSDU(pkttag) &&
                ...
                uint16 amsdu_seq1 = pkttag->seq & WL_SEQ_AMSDU_SUPPR_MASK;
                uint16 amsdu_seqn;
        ...
        if (err) {
            if (err == BCME_BUSY) {
                WL_AMPDU_TX_EX_PFX(scb, "prep_xdu retry; seq 0x%x\n", seq);
            ...
            /* error in the packet; reject it */
            WL_AMPDU_ERR_EX_PFX(scb, "prep_xdu rejected; seq 0x%x\n", seq);
            WLCNTINCR(ampdu_tx->cnt->sdurejected);
            AMPDUSCBCNTINCR(scb_ampdu->cnt.sdurejected);
        ...
        WL_AMPDU_TX_EX_PFX(scb, "prep_xdu success; seq 0x%x tid %d\n",
            seq, tid);
         ...
         */
        if (ETHER_ISNULLADDR(&h->a1)) {
            WL_AMPDU_ERR_EX_PFX(scb, "dropping frame with null a1\n");
        ...
        if (D11REV_GE(corerev, 128)) {
            ...
            if (SCB_HE_CAP(scb) &&
              ((!SCB_MU(scb) && !SCB_DLOFDMA_ADM(scb) && HE_SU_FRAGTX_ENAB(wlc->pub)) ||
              ((SCB_MU(scb) || SCB_DLOFDMA_ADM(scb)) && HE_MU_FRAGTX_ENAB(wlc->pub)))) {
        ...
        if (regmpdu_pkt) {
            WL_AMPDU_TX_EX_PFX(scb, "reg mpdu found\n");
            ...
            *machigh = tx_info.MacTxControlHigh;
            ...
            if (fill_txh_info) {
                ...
                pps = ltoh16(tx_info.MacTxControlHigh) & D11AC_TXC_PPS;
                ...
                } else if (pps != hwagg->prev_pps) {
                    reason_code = AMU_EPOCH_CHG_PPS;
                } else {
                    change = FALSE;
                }
                fill_txh_info = FALSE;
    ...
    WL_AMPDU_TX_EX_PFX(scb, "fifo %d count %d epoch %d txpktpend %d",
        fifo, count, ampdu_tx->hwagg[fifo].epoch,
        TXPKTPENDGET(wlc, fifo));
    if (WL_AMPDU_HW_ON()) {
        if (D11REV_GE(wlc->pub->corerev, 128)) {
            WL_AMPDU_TX_EX_PFX(scb, " aqmqmap 0x%x aqmfifo_status 0x%x\n",
                R_REG(osh, D11_AQM_QMAP(wlc)),
                R_REG(osh, D11_MQF_STATUS(wlc)));
        } else if (D11REV_GE(wlc->pub->corerev, 65)) {
            WL_AMPDU_TX_EX_PFX(scb, " psm_reg_mux 0x%x aqmqmap 0x%x aqmfifo"
                "_status 0x%x\n", R_REG(osh, D11_PSM_REG_MUX(wlc)),
                R_REG(osh, D11_AQMQMAP(wlc)),
                R_REG(osh, D11_AQMFifo_Status(wlc)));
        } else {
            WL_AMPDU_TX_EX_PFX(scb, " fifocnt 0x%x\n",
                R_REG(wlc->osh, D11_XmtFifoFrameCnt(wlc)));
        }
    }
...
wlc_ampdu_send_bar(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tid_ini_t *ini, uint16 bar_seq)
{
    ...
    if (AMPDU_AQM_ENAB(ampdu_tx->wlc->pub)) {
        ...
        if (IS_SEQ_ADVANCED(MODSUB_POW2(ini->start_seq, 1, SEQNUM_MAX), bar_seq) ||
            ...
            WL_AMPDU_CTL_EX_PFX(ini->scb, "bar_seq %x outside released: acked_seq %x, "
                "max_seq %x\n", bar_seq, ini->acked_seq, ini->max_seq);
            goto abort_send_bar;
        }
        ...
        if (MODSUB_POW2(ini->max_seq+1, bar_seq, SEQNUM_MAX) >= ini->ba_wsize) {
            WL_AMPDU_CTL_EX_PFX(ini->scb, "skipping BAR %x, max_seq %x, acked_seq %x\n",
                bar_seq, ini->max_seq, ini->acked_seq);
            goto abort_send_bar;
    ...
    if (ini->retry_bar != AMPDU_R_BAR_DISABLED) {
        WL_AMPDU_CTL_EX_PFX(ini->scb, "retry bar for reason %x, bar_seq 0x%04x, "
            "max_seq 0x%04x, acked_seq 0x%04x, bar_cnt %d, tx_in_transit %d\n",
    ...
    if (ampdu_tx_cfg->no_bar == FALSE) {
        ...
        p = wlc_send_bar(ampdu_tx->wlc, ini->scb, ini->tid, bar_seq,
                         DOT11_BA_CTL_POLICY_NORMAL, &blocked);
        if (blocked) {
            /* retry sending bar from ampdu watchdog */
    ...
    if (ampdu_tx_cfg->no_bar == TRUE) {
        ...
        /* send packet with callback in place */
        wlc_send_q(ampdu_tx->wlc, SCB_WLCIFP(ini->scb)->qi);
    }
    return;
    ...
    scb_ampdu_tid_ini_t *ini)
{
    ...
    WL_AMPDU_TX_EX_PFX(ini->scb, "tid %d start_seq bumped to 0x%x\n",
        ini->tid, ini->start_seq);

    if (non_aqm->rem_window > ini->ba_wsize) {
        WL_AMPDU_ERR_EX_PFX(ini->scb, "rem_window %d, ba_wsize %d "
            "i %d range %d sseq 0x%x\n",
            non_aqm->rem_window, ini->ba_wsize,
...
wlc_send_bar_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
    ...
    WL_AMPDU_CTL_EX_PFX(ini->scb, "for tid %d status 0x%x,"
    ...
    if (((txstatus & TX_STATUS_FRM_RTX_MASK) >> TX_STATUS_FRM_RTX_SHIFT) == 0) {
        WL_AMPDU_ERR_EX_PFX(ini->scb, "no tx attempted, txstatus: 0x%x\n",
            txstatus);
    }
    ...
    void *p, tx_status_t *txs, wlc_txh_info_t *txh_info)
{
    ...
    if (txs->status.was_acked) {
        ...
        WL_AMPDU_TX_EX_PFX(scb, "ack not recd for "
            "seq 0x%x tid %d status 0x%x\n",
            seq, ini->tid, txs->status.raw_bits);
    ...
    void *p, tx_status_t *txs, wlc_txh_info_t *txh_info)
{
    ...
    SCB_PKTS_INFLT_FIFOCNT_DECR(ampdu_tx->wlc, scb, PKTPRIO(p));
    ...
    if (MODSUB_POW2(seq, ini->start_seq, SEQNUM_MAX) >= ini->ba_wsize) {
        WL_AMPDU_ERR_EX_PFX(scb, "unexpected completion: seq 0x%x, "
            "start seq 0x%x\n", seq, ini->start_seq);
        return;
    }
    ...
    if (!isset(ini->non_aqm->ackpending, indx)) {
        WL_AMPDU_ERR_EX_PFX(scb, "seq 0x%x\n", seq);
        ampdu_dump_ini(ampdu_tx->wlc, ini);
...
wlc_ampdu_free_chain(ampdu_tx_info_t *ampdu_tx, void *p, tx_status_t *txs)
{
    ...
    WL_AMPDU_TX_EX_PFX(scb, "free ampdu_tx chain\n");
...
#endif
        SCB_PKTS_INFLT_FIFOCNT_SUB(wlc, scb, prio, count);
    }
    ...
    WL_AMPDU_TX_EX_PFX(scb, "fifo %d pkts %d txpktpend %d\n",
        queue, count, TXPKTPENDGET(wlc, queue));
    ...
    wlc_txh_info_t *txh_info)
{
    ...
    if (!ini || (ini->ba_state == AMPDU_TID_STATE_BA_PENDING_ON)) {
        WL_AMPDU_EX_PFX(scb, "bail: bad ini (%p) or ba_state (%d) prio %d\n",
            OSL_OBFUSCATE_BUF(ini),
    ...
    if (txs->status.was_acked) {
        ...
        WL_AMPDU_TX_EX_PFX(scb, "2nd status in delay %d\n", status_delay);
...
wlc_ampdu_fill_rs_txs(ratesel_txs_t* rs_txs, tx_status_t *txs, uint8 ac, uint16 ncons, uint16 nlost)
{
    ...
    rs_txs->ack_map1 = TX_STATUS_MACTXS_ACK_MAP1(txs);
    rs_txs->ack_map2 = TX_STATUS_MACTXS_ACK_MAP2(txs);
...
wlc_ampdu_aqm_mutx_dotxinterm_status(ampdu_tx_info_t *ampdu_tx, tx_status_t *txs)
{
    ...
    rs_txs.tx_cnt[rtidx]     = TX_STATUS40_TXCNT_RT0(TX_STATUS_MACTXS_S3(txs));
    rs_txs.txsucc_cnt[rtidx] = TX_STATUS40_ACKCNT_RT0(TX_STATUS_MACTXS_S3(txs));
    ...
    bw = TX_STATUS64_MU_BW(TX_STATUS_MACTXS_S3(txs));
    gi = TX_STATUS64_MU_SGI(TX_STATUS_MACTXS_S3(txs));
    ...
    if ((txs_mutype == TX_STATUS_MUTP_HEMM) || (txs_mutype == TX_STATUS_MUTP_HEOM)) {
        if (txs_mutype == TX_STATUS_MUTP_HEOM) {
            gi = TX_STATUS128_HEOM_CPLTF(TX_STATUS_MACTXS_S4(txs));
        }
...
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMDBG_AMPDU) || defined(WL_LINKSTAT)
        WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txhe[bw][gi][rateidx],
            rs_txs.tx_cnt[rtidx]);
        WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txhe_succ[bw][gi][rateidx],
            rs_txs.txsucc_cnt[rtidx]);
        if (rs_txs.txrspec[rtidx] & WL_RSPEC_STBC) {
            WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txhestbc[rateidx],
                rs_txs.tx_cnt[rtidx]);
        }
#endif /*  defined(BCMDBG) || defined(WLTEST) */
        if (rs_txs.txrspec[rtidx] & WL_RSPEC_STBC) {
...
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMDBG_AMPDU) || defined(WL_LINKSTAT)
        WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txvht[bw][rateidx],
            rs_txs.tx_cnt[rtidx]);
        WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txvht_succ[bw][rateidx],
            rs_txs.txsucc_cnt[rtidx]);
    ...
    */
...
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMDBG_AMPDU) || defined(WL_LINKSTAT)
    WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmpdu_per_ft[ampdu_ft],
        rs_txs.tx_cnt[rtidx]);
    WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmpdu_succ_per_ft[ampdu_ft],
        rs_txs.txsucc_cnt[rtidx]);
#endif /*  defined(BCMDBG) || defined(WLTEST) */
    ...
    if (!ETHER_ISMULTI(txh_info.TxFrameRA)) {
        ...
        if (bsscfg != NULL) {
            ...
            WLCNTCONDADD(ampdu_dpdbg, ampdu_dpdbg->txmpdu_succ_per_ft[ampdu_ft],
                rs_txs.txsucc_cnt[rtidx]);
            SCB_TXRX_SUMMARY_COND_FIND(txrx_summary_cnt, wlc, scb);
            SCB_TXRX_SUMMARY_MUTX_COND_ADD(txrx_summary_cnt, ampdu_ft,
                rs_txs.txsucc_cnt[rtidx]);
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.attempted,
                rs_txs.tx_cnt[rtidx]);
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.retried,
                (rs_txs.tx_cnt[rtidx] - rs_txs.txsucc_cnt[rtidx]));
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.bw,
                bw * (rs_txs.txsucc_cnt[rtidx]));
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.nss[nss-1],
                rs_txs.txsucc_cnt[rtidx]);
#endif
            ...
            if (txs_mutype == TX_STATUS_MUTP_HEOM) {
#ifdef BCMDBG_DUMP_MSCHED
                wlc_musched_upd_ru_stats(wlc->musched, scb, txs);
#endif /* BCMDBG_DUMP_MSCHED */
#ifdef PKTQ_LOG
                if (ampdu_dpdbg || txrx_summary_cnt) {
                    uint8 dp_count;
                    uint16 rucnt;
                    ...
                    if (index >= 0 && index <= MAC_LOG_MU_RU_MAX) {
                        WLCNTCONDADD(ampdu_dpdbg,
                            ampdu_dpdbg->txru_succ[index], dp_count);
#ifdef TXRX_SUMMARY
                        rucnt = RUTYPE_TO_RUCNT(index);
#endif /* TXRX_SUMMARY */
                        SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt,
                            tx.tones, (dp_count * rucnt));
                    }
                }
#endif /* PKTQ_LOG */
            }
    ...
    void *p, tx_status_t *txs, wlc_txh_info_t *txh_info)
{
    ...
    if (!ncons) {
        WL_AMPDU_ERR_EX_PFX(scb, "ncons is %d in %s", ncons, __FUNCTION__);
    ...
    */
    ...
    if (supr_status != TX_STATUS_SUPR_NONE) {
...
#if defined(BCMDBG) || defined(BCMDBG_AMPDU)
        WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->supr_reason[supr_status], ncons);
#endif
        WL_AMPDU_TX_EX_PFX(scb, "supr_status 0x%x\n", supr_status);
        if (supr_status == TX_STATUS_SUPR_LMEM_INVLD) {
            ...
            WL_AMPDU_ERR_EX_PFX(scb, "tx underflow?!\n");
             ...
             * get suppressed by twt suppress logic.
             */
        ...
        WL_ERROR(("wl%d: %s: "MACF" tid %d tx phy error (0x%x)\n",
            wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea), tid, txs->phyerr));
#ifdef BCMDBG
    ...
    while (TRUE) { /* loops over each tx MPDU in the caller supplied tx sts */
        ...
        SCB_PKTS_INFLT_FIFOCNT_DECR(wlc, scb, tid);
        ...
        if (tot_mpdu == 0) { /* first MPDU in a list of MPDUs */
            ...
            do {
                if (D11REV_GE(wlc->pub->corerev, 128)) {
                    ...
                    ft_fmt = PhyTxControlWord_0 &
                        D11_PHY_TXC_FTFMT_MASK(wlc->pub->corerev);

                    if (wf_plcp_to_rspec(ft_fmt, plcp,
        ...
        if ((BSSCFG_AP(bsscfg) || BSSCFG_IBSS(bsscfg)) &&
            ...
            if (pps_retry) {
                ...
                /* unset txh for next p, because we assume they are
        ...
        if (was_acked) { /* if current MPDU was acked by remote party */
            WL_AMPDU_TX_EX_PFX(scb, "pkt ack seq 0x%04x idx %d\n",
                seq, tot_mpdu);
#ifdef PSPRETEND
        ...
        if (pps_retry) {
            ...
            free_mpdu = wlc_pspretend_pkt_retry(wlc->pps_info, scb, p, txs, txh);
...
not_retry:        /* don't retry an unacked MPDU, but send BAR if applicable */
                ...
                WL_AMPDU_TX_EX_PFX(scb, "pkt seq 0x%04x not acked\n", seq);
...
free_and_next:
    ...
    /* dump_flag_qqdx */
        ...
        if (free_mpdu) {
            ...
            if (supr_status == TX_STATUS_SUPR_AGG0) {
                if (D11REV_GE(wlc->pub->corerev, 128)) {
                    ...
                    WL_AMPDU_TX_EX_PFX(scb, "suppressed by continuous AGG0\n");
                } else {
        ...
        if (p == NULL) {
            WL_AMPDU_ERR_EX_PFX(scb, "p is NULL. tot_mpdu: %d, ncons: %d\n",
                tot_mpdu, ncons);
            ASSERT(p);
            break;
        }
...
#ifdef PKTQ_LOG
        if (prec_cnt || actq_cnt || bs_data_counters || txrx_summary_cnt) {
    ...
    defined(BCMDBG_AMPDU) || defined(WL_LINKSTAT)
        ...
        WL_AMPDU_TX_EX_PFX(scb, "\aqm_txs: nlost %d send_bar %d "
            "ft %d mcs[0-3] %04x %04x %04x %04x\n",
            nlost, send_bar,
            ft[0], rs_txs.txrspec[0], rs_txs.txrspec[1],
            rs_txs.txrspec[2], rs_txs.txrspec[3]);
        WL_AMPDU_TX_EX_PFX(scb, "raw txstatus %04X %04X %04X | "
            "s3-5 %08X %08X %08X | %08X %08X | s8 %08X\n",
        ...
        if (send_bar) {
            WL_AMPDU_CTL_EX_PFX(scb, "pkt ack_seq 0x%x bar_seq 0x%04x start 0x%x"
                " max 0x%x\n",
                ini->acked_seq,
    ...
    WL_AMPDU_TX_EX_PFX(scb, "ncons %d tot_mpdus %d start_seq 0x%04x\n",
        ncons, tot_mpdu, ini->start_seq);
    ...
    if (WL_AMPDU_ERR_ON()) {
        for (k = 0; k < rnum; k++) {
            if (rs_txs.txsucc_cnt[k] > rs_txs.tx_cnt[k]) {
                WL_AMPDU_ERR_EX_PFX(scb, "rs_txs.txsucc_cnt[%d] > "
                    "rs_txs.tx_cnt[%d]: %d > %d\n", k, k,
                    rs_txs.txsucc_cnt[k], rs_txs.tx_cnt[k]);
                WL_AMPDU_ERR_EX_PFX(scb, "ncons %d raw txstatus %08X | %04X %04X | "
                     "%08X %08X || %08X | %08X %08X\n", ncons,
    ...
    if (fix_rate && !txs_mu) {
         ...
         */
        rs_txs.tx_cnt[0]     = TX_STATUS_MACTXS_S3(txs) & 0xffff;
        rs_txs.txsucc_cnt[0] = (TX_STATUS_MACTXS_S3(txs) >> 16) & 0xffff;
        ...
        if (txs_mu) { /* MU case */
            ...
            rs_txs.tx_cnt[1]     = (TX_STATUS_MACTXS_S3(txs) >> 16) & 0xff;
            rs_txs.txsucc_cnt[1] = (TX_STATUS_MACTXS_S3(txs) >> 24) & 0xff;
            rs_txs.tx_cnt[2]     = (TX_STATUS_MACTXS_S4(txs) >>  0) & 0xff;
            rs_txs.txsucc_cnt[2] = (TX_STATUS_MACTXS_S4(txs) >>  8) & 0xff;
            rs_txs.tx_cnt[3]     = (TX_STATUS_MACTXS_S4(txs) >> 16) & 0xff;
            rs_txs.txsucc_cnt[3] = (TX_STATUS_MACTXS_S4(txs) >> 24) & 0xff;
        }

        rs_txs.tx_cnt[rtidx]     = TX_STATUS_MACTXS_S3(txs) & 0xff;
        rs_txs.txsucc_cnt[rtidx] = (TX_STATUS_MACTXS_S3(txs) >> 8) & 0xff;
...
#ifdef GAME_SPEEDUP
   WLCNTSCBADD(scb->scb_stats.rts_tx_cnt, txs->status.rts_tx_cnt);
   WLCNTSCBADD(scb->scb_stats.rts_failed_cnt, failed_rts);
#endif
    ...
    if ((prec_cnt || actq_cnt || bs_data_counters || txrx_summary_cnt) && succ_count) {
        ...
        if (fix_rate && !txs_mu) {
            ...
            for (k = 0; k < rnum; k++) {
                ...
                SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.bw,
                    (mcs_bw[k] * _dp_succ));
                SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt,
                    tx.nss[mcs_nss[k]], _dp_succ);
            }
        } else if (txs_mu) {
            /* Handle statistics if MU-MIMO or DL_OFDMA */
            uint8 dp_nss = TX_STATUS64_MU_NSS(TX_STATUS_MACTXS_S4(txs));
            uint8 dp_mcs = TX_STATUS64_MU_MCS(TX_STATUS_MACTXS_S4(txs));
            uint8 dp_is_sgi;
            uint32 dp_bw;

            ASSERT(dp_nss < WLC_MAX_TX_NSS);
#ifdef WL11BE
            ASSERT(dp_mcs <= WLC_MAX_EHT_MCS);
#else
            ASSERT(dp_mcs <= WLC_MAX_HE_MCS);
#endif /* WL11BE */
            if ((txs_mutype == TX_STATUS_MUTP_VHTMU) ||
                (txs_mutype == TX_STATUS_MUTP_HEMM)) {

                dp_bw = 1 + TX_STATUS64_MU_BW(TX_STATUS_MACTXS_S3(txs));
                dp_is_sgi = TX_STATUS64_MU_SGI(TX_STATUS_MACTXS_S3(txs));
                ...
                dp_is_sgi = TX_STATUS128_HEOM_CPLTF(TX_STATUS_MACTXS_S4(txs));
...
#endif /* SCB_BS_DATA */
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.bw, txbw_succ);
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.nss[dp_nss],
                succ_count);
        ...
        /* save airtime */
        if (D11REV_GE(wlc->pub->corerev, 128)) {
            airtime = TX_STATUS128_TXDUR(TX_STATUS_MACTXS_S2(txs));
        } else {
...
#endif /* SCB_BS_DATA */
    SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.phyrate, (uint64)txrate_main);
    SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.mpdu, ack_cnt);
    SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.bytes,
        (uint64)total_acked_pktlens);
    ...
    if (ampdu_dpdbg || txrx_summary_cnt) {
        for (k = 0; k < rnum; k++) {
            ...
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.attempted,
                rs_txs.tx_cnt[k]);
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.retried,
                (rs_txs.tx_cnt[k] - rs_txs.txsucc_cnt[k]));
        }
    }
#endif /* PKTQ_LOG */
    ...
    if (!was_acked) {
        ...
        case FT_HE:
            if (txs_mu) {
                ...
                if (txs_mutype == TX_STATUS_MUTP_HEMM) {
                    ...
                    bw = RSPEC2BW(rs_txs.txrspec[rtidx]) - BW_MINMHZ;
                    gi = TX_STATUS128_HEOM_CPLTF(TX_STATUS_MACTXS_S4(txs));
                } else  {
                    bw = 0;
                    gi = 0;
                ...
                nss = (rs_txs.txrspec[k] & WL_RSPEC_HE_NSS_MASK)
                    >> WL_RSPEC_HE_NSS_SHIFT;
                bw = ((rs_txs.txrspec[k] & WL_RSPEC_BW_MASK) >> WL_RSPEC_BW_SHIFT)
                    - BW_MINMHZ;
        ...
        case FT_VHT:
            if (txs_mu) {
                ...
                nss = (rs_txs.txrspec[k] & WL_RSPEC_VHT_NSS_MASK)
            ...
            WLCNTADD(ampdu_tx->cnt->txmpdu_vht_succ_cnt[rateidx],
                rs_txs.txsucc_cnt[k]);
#endif /* WLPKTDLYSTAT */
            WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txvht[bw][rateidx],
                rs_txs.tx_cnt[k]);
            ...
            if (rs_txs.txrspec[k] & WL_RSPEC_SGI) {
                ...
                WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txvhtsgi[rateidx],
                    rs_txs.tx_cnt[k]);
            }
            if (rs_txs.txrspec[k] & WL_RSPEC_STBC) {
                WLCNTADD(ampdu_tx->cnt->u2.txmpdu_stbc, rs_txs.tx_cnt[k]);
                WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txvhtstbc[rateidx],
                    rs_txs.tx_cnt[k]);
            }
            break;
...
#endif /* WLPKTDLYSTAT */
            WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmcs[rateidx],
                rs_txs.tx_cnt[k]);
            WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmcs_succ[rateidx],
                rs_txs.txsucc_cnt[k]);
            if (rs_txs.txrspec[k] & WL_RSPEC_SGI) {
                WLCNTADD(ampdu_tx->cnt->u1.txmpdu_sgi, rs_txs.tx_cnt[k]);
                WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmcssgi[rateidx],
                    rs_txs.tx_cnt[k]);
            }
            if (rs_txs.txrspec[k] & WL_RSPEC_STBC) {
                WLCNTADD(ampdu_tx->cnt->u2.txmpdu_stbc, rs_txs.tx_cnt[k]);
                WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmcsstbc[rateidx],
                    rs_txs.tx_cnt[k]);
            }
            break;
        ...
        if (txs_mu) {
            /* if mu, packet numbers of fallback rates are all 0 */
            break;
        }
    }
    ...
    if (((MU_TX_ENAB(wlc) && SCB_VHTMU(scb)) || (HE_DLMMU_ENAB(wlc->pub) && SCB_HEMMU(scb))) &&
        ...
        if (ampdu_dpdbg && txs_mu) {
            uint8 gbmp = TX_STATUS64_MU_GBMP(TX_STATUS_MACTXS_S4(txs));
            uint32 gnum = 0;
    ...
    /* In case of TWT suppress the normal path is to ignore txs for rate updates. But when the
     * rate selection has "upped" the rate and the TWT SP is very short then the tx may fail
     * continously, but due to the short SP, the txs will always be TWT Suppress. Fix this by
     * doing rate update if there was a tx try and suppress is TWT. This may result in a bit
     * earlier "lower" rate selection for TWT links, but otherwise the TWT link can get stuck
     * in failing tx for a very long time, which is much worse.
     */
    if (!update_ratesel && (supr_status == TX_STATUS_SUPR_TWT) && (rs_txs.tx_cnt[0] > 0)) {
        update_ratesel = TRUE;
    }
    ...
    if (update_ratesel) {
        ...
        if (rs_txs.tx_cnt[0] == 0 && fbr_cnt == 0) {
            ...
            if (SCB_PS_PRETEND(scb) && !SCB_PS_PRETEND_THRESHOLD(scb)) {
                wlc_pspretend_rate_reset(wlc->pps_info, scb, WME_PRIO2AC(tid));
            }
#endif /* PSPRETEND */
        }
...
#if defined(WL_MU_TX)
        /* In the case where update_ratesel == FALSE (e.g., supression txs),
         * reset txepoch for this ac as there might have been prior interm
         * tx status.
         */
        wlc_scb_ratesel_init_rcb_itxs(wlc->wrsi, scb, ac);
#endif
    }
                                    ...
                                    tx_status_t *txs, uint32 s1, uint32 s2)
{
    ...
    if (AMPDU_HOST_ENAB(wlc->pub)) {
        if (txs->status.was_acked) {
            ...
            if (TX_STATUS_SUPR_UF == supr_status) {
                WL_AMPDU_TX_EX_PFX(scb, "BT preemption, skip rate fallback\n");
                update_rate = FALSE;
            }
            ...
            WL_AMPDU_TX_EX_PFX(scb, "Block ack received: start_seq is 0x%x bitmap "
                "%02x %02x %02x %02x %02x %02x %02x %02x\n",
                start_seq, bitmap[0], bitmap[1], bitmap[2],
    ...
    if (!ba_recd || AMPDU_MAC_ENAB(wlc->pub)) {
        if (!ba_recd) {
            ...
            AMPDUSCBCNTINCR(scb_ampdu->cnt.noba);
            if (AMPDU_MAC_ENAB(wlc->pub)) {
                WL_AMPDU_TX_EX_PFX(scb, "error txstatus 0x%x\n",
                    txs->status.raw_bits);
            }
        }
        if (supr_status != TX_STATUS_SUPR_NONE) {
            ...
            update_rate = FALSE;
#if defined(BCMDBG) || defined(BCMDBG_AMPDU)
            WLCNTCONDINCR(ampdu_tx->amdbg, ampdu_tx->amdbg->supr_reason[supr_status]);
#endif
            WL_AMPDU_TX_EX_PFX(scb, "supr_status 0x%x\n", supr_status);
    ...
    while (p) {
        ...
        if (tot_mpdu == 0) {
            ...
            if (AMPDU_HOST_ENAB(wlc->pub)) {
                ...
                if (PLCP3_ISSGI(plcp[3])) {
                    ...
                    WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->
                        txmcssgi[MCS2IDX(mcs)], rs_txs.tx_cnt[0]);
                }
                if (PLCP3_ISSTBC(plcp[3])) {
                    WLCNTADD(ampdu_tx->cnt->u2.txmpdu_stbc, rs_txs.tx_cnt[0]);
                    WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->
                        txmcsstbc[MCS2IDX(mcs)], rs_txs.tx_cnt[0]);
                }
                WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->txmcs[MCS2IDX(mcs)],
                    rs_txs.tx_cnt[0]);
                WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->
                    txmcs_succ[MCS2IDX(mcs)], rs_txs.txsucc_cnt[0]);
#ifdef WLPKTDLYSTAT
...
#endif /* WLCNT */
                    WLCNTCONDADD(ampdu_tx->amdbg, ampdu_tx->amdbg->
                        txmcs[MCS2IDX(mcs_fb)], rs_txs.tx_cnt[1]);
#ifdef WLPKTDLYSTAT
        ...
        if (ba_recd || AMPDU_MAC_ENAB(wlc->pub)) {
            ...
            WL_AMPDU_TX_EX_PFX(scb, "tid %d seq is 0x%x, start_seq is 0x%x, "
            ...
            if ((bindex < AMPDU_BA_BITS_IN_BITMAP) && isset(bitmap, bindex)) {
                ...
                WLCNTSCBADD(scb_ampdu->ampdu_scb_stats->tx_bytes_total[ini->tid],
                    prec_pktlen);
                SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.bytes, prec_pktlen);
#endif /* PKTQ_LOG */
                suc_mpdu++;
            }
        }
        ...
        if (!ack_recd) {
            ...
            if (BSSCFG_AP(bsscfg) && PS_PRETEND_ENABLED(bsscfg)) {
                relist = wlc_pspretend_pkt_relist(wlc->pps_info, scb,
                                                  retry_limit, txretry);
            }
#endif /* PSPRETEND */
...
nextp:
        ...
        if ((idx = tot_mpdu) >= AMPDU_MAX_MPDU) {
            WL_AMPDU_ERR_EX_PFX(scb, "idx out-of-bound to array size (%d)\n",
                    idx);
        ...
        if (AMPDU_MAC_ENAB(wlc->pub)) {
            ...
            if (++pdu_count >= ampdu_tx_cfg->ba_max_tx_wsize) {
                WL_AMPDU_ERR_EX_PFX(scb, "Reach max num of MPDU without finding "
                    "frameid\n");
        ...
        if (p == NULL) {
            WL_AMPDU_ERR_EX_PFX(scb, "p is NULL. tot_mpdu %d suc %d pdu %d "
                "first_frameid %x fifocnt %d\n", tot_mpdu, suc_mpdu,
                pdu_count, first_frameid,
                R_REG(wlc->osh, D11_XmtFifoFrameCnt(wlc)));
...
#define MCS_TO_RSPEC(mcs, bw, plcp)    ((HT_RSPEC(mcs)) | ((bw) << WL_RSPEC_BW_SHIFT)| \
    ...
    if (suc_mpdu) {
        ...
        SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.mpdu, suc_mpdu);
        SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.attempted, suc_mpdu);
        SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, tx.mpdu_su, suc_mpdu);
    }
...
skip_airtime_computation:
    ...
    if (AMPDU_MAC_ENAB(wlc->pub)) {
        ...
        if (rs_txs.tx_cnt[0] + rs_txs.tx_cnt[1] == 0) {
            ...
            if (rs_txs.txsucc_cnt[0] + rs_txs.txsucc_cnt[1] != 0) {
                WL_AMPDU_ERR_EX_PFX(scb, "wrong condition\n");
                wlc_print_ampdu_txstatus_noaqm(ampdu_tx, txs, s1, s2);
            }
            if (WL_AMPDU_ERR_ON()) {
                wlc_print_ampdu_txstatus_noaqm(ampdu_tx, txs, s1, s2);
    ...
    WL_AMPDU_TX_EX_PFX(scb, "requeueing %d packets, consumed %d\n",
        requeue, tot_mpdu);
...
#ifdef PSPRETEND
    if ((BSSCFG_AP(bsscfg) || BSSCFG_IBSS(bsscfg)) && PS_PRETEND_ENABLED(bsscfg)) {
        wlc_pspretend_dotxstatus(wlc->pps_info, scb, ba_recd);
    }
#endif /* PSPRETEND */
    /* bump up the start seq to move the window */
    wlc_ampdu_ini_move_window_noaqm(ampdu_tx, scb_ampdu, ini);
...
ampdu_ba_state_off(wlc_info_t *wlc, scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini)
{
    ...
    ini->off_cnt = AMPDU_INI_OFF_TIMEOUT_INITIAL *
        (((uint16)scb_ampdu->fail_cnt_tid[ini->tid]) + 1);
    ini->off_cnt = MIN(ini->off_cnt, AMPDU_INI_OFF_TIMEOUT_MAX);
...
#endif
    /* Only walk through one fifo which match specific TID/QoS category */
    if (SCB_PKTS_INFLT_FIFOCNT_VAL(scb, tid) != 0) {
        fifo_idx = wlc_fifo_index_peek(wlc->fifo, scb, WME_PRIO2AC(tid));
        setbit(fifo_bitmap, fifo_idx);
        wlc_dma_map_pkts(wlc->hw, wlc_ampdu_map_pkts_cb, &cb_params, fifo_bitmap);
    }
...
ampdu_cleanup_tid_ini(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid, bool force)
{
    ...
    WL_AMPDU_CTL_EX(scb, ("tid %d force %d tx_in_transit %u rem_window %u "
                  "ba_state %u\n",
                  tid, force, ini->tx_in_transit,
    ...
    n_pkts = wlc_txq_pktq_pflush(wlc, &scb_ampdu->txq, ini->tid);
    BCM_REFERENCE(n_pkts);
#if defined(BCM_PKTFWD_FLCTL)
    __wlc_ampdu_update_link_credits(wlc, scb, tid, n_pkts, TRUE);
#endif /* BCM_PKTFWD_FLCTL */
...
static int
wlc_ampdu_send_addba_req(wlc_info_t *wlc, struct scb *scb, uint8 tid, uint16 wsize,
    uint8 ba_policy, uint8 delba_timeout)
{
    uint8 token = wlc_ampdu_get_token(wlc->ampdu_tx, scb, tid);

    return wlc_send_addba_req(wlc, scb, tid, wsize, ba_policy, delba_timeout, token);
}

static uint8
wlc_ampdu_get_token(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid)
{
    scb_ampdu_tx_t *scb_ampdu;
    scb_ampdu_tid_ini_t *ini;
    uint8 token;
    ...
    token = scb_ampdu->addba_token_base[tid];
    if (ini->last_addba_ts) {
        /* +1 as the original transmission used addba_token_base,
         * and addba_req_cnt starts at 0.
         * Because of the wrap prevention code in wlc_ampdu_token_cleanup,
         * AMPDU_ADDBA_REQ_RETRY_CNT + 1 can be added safely here.
         */
        token += ini->addba_req_cnt+1;
    }
...
static bool
wlc_ampdu_token_is_valid(scb_ampdu_tx_t *scb_ampdu, uint8 tid, uint8 token)
{
    uint8 offset;
    ...
    /* Because of the wrap prevention code in wlc_ampdu_token_cleanup,
     * a valid token should be higher than addba_token_base.
     */
    if (token < scb_ampdu->addba_token_base[tid]) {
        return FALSE;
    }

    offset = token - scb_ampdu->addba_token_base[tid];

    /* Comparing against retry count +1, because addba_req_cnt can be set to
     * AMPDU_ADDBA_REQ_RETRY_CNT and we need to account for the original transmission.
     */
    if (offset <= AMPDU_ADDBA_REQ_RETRY_CNT+1) {
        return TRUE;
    }
...
static void
wlc_ampdu_token_cleanup(scb_ampdu_tx_t *scb_ampdu, uint8 tid)
{
    uint8 offset;

    scb_ampdu->addba_token_base[tid] = MODADD_POW2(scb_ampdu->addba_token_base[tid],
        AMPDU_ADDBA_REQ_RETRY_CNT+1, BCM_UINT8_MAX+1);

    /* If addba_token_base + AMPDU_ADDBA_REQ_RETRY_CNT would wrap, set it to initial value */
    offset = MODSUB_POW2(0, scb_ampdu->addba_token_base[tid], BCM_UINT8_MAX+1);
    if (offset <= AMPDU_ADDBA_REQ_RETRY_CNT+1) {
        scb_ampdu->addba_token_base[tid] = 1;
    }
}
...
wlc_ampdu_recv_addba_resp(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 *body, int body_len)
{
    ...
    if (addba_resp->category & DOT11_ACTION_CAT_ERR_MASK) {
        WL_AMPDU_ERR_EX_PFX(scb, "unexp error action frame\n");
    ...
    token_valid = wlc_ampdu_token_is_valid(scb_ampdu_tx, tid, addba_resp->token);

    if (((ini = scb_ampdu_tx->ini[tid]) == NULL) && token_valid) {
        ini = wlc_ampdu_init_tid_ini(ampdu_tx, scb_ampdu_tx, tid, FALSE, FALSE);
    }

    if ((ini == NULL) || !token_valid || (ini->ba_state == AMPDU_TID_STATE_BA_ON)) {
        WL_AMPDU_CTL_EX_PFX(scb, "unsolicited packet, token 0x%x\n", addba_resp->token);
        WLCNTINCR(ampdu_tx->cnt->rxunexp);
        return;
    }

    wlc_ampdu_token_cleanup(scb_ampdu_tx, tid);

    if ((status != DOT11_SC_SUCCESS) ||
        (policy != ampdu_tx_cfg->ba_policy) ||
        (wsize > ampdu_tx_cfg->ba_max_tx_wsize)) {
        WL_AMPDU_ERR_EX_PFX(scb, "Failed. status %d wsize %d policy %d tid %d\n",
            status, wsize, policy, tid);
        ampdu_ba_state_off(wlc, scb_ampdu_tx, ini);
        if (scb_ampdu_tx->fail_cnt_tid[tid] < BCM_UINT8_MAX) {
            scb_ampdu_tx->fail_cnt_tid[tid]++;
        }
        WLCNTINCR(ampdu_tx->cnt->rxunexp);
        return;
    }
    scb_ampdu_tx->fail_cnt_tid[tid] = 0;
    ...
    WL_AMPDU_CTL_EX_PFX(scb, "Turning BA ON: tid %d wsize %d token 0x%x\n",
        tid, wsize, addba_resp->token);
...
wlc_ampdu_init_tid_ini(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu, uint8 tid,
    bool override, bool send_addba_req)
{
    scb_ampdu_tid_ini_t *ini;
    wlc_info_t *wlc = ampdu_tx->wlc;
    ampdu_tx_config_t *ampdu_tx_cfg = ampdu_tx->config;
    ...
    if (scb->mark & (SCB_DEL_IN_PROG | SCB_MARK_TO_DEL)) {
        WL_AMPDU_EX(scb, ("scb->mark(%x) SCB DEL_IN_PROG or MARK_TO_DEL set--returning\n",
            scb->mark));
        return NULL;
    }
...
#endif
    if (send_addba_req) {
        wlc_ampdu_send_addba_req(wlc, scb, tid, ampdu_tx_cfg->ba_tx_wsize,
    ...
    dot11_addba_req_t *addba_req, int body_len)
{
    ...
    if (addba_req->category & DOT11_ACTION_CAT_ERR_MASK) {
        ...
        if ((ini != NULL) && (ini->ba_state == AMPDU_TID_STATE_BA_PENDING_ON)) {
            ampdu_ba_state_off(wlc, scb_ampdu_tx, ini);
            WL_AMPDU_ERR_EX_PFX(scb, "error action frame\n");
        } else {
            WL_AMPDU_ERR_EX_PFX(scb, "unexp error action frame\n");
        }
        WLCNTINCR(ampdu_tx->cnt->rxunexp);
...
wlc_ampdu_tx_dump(ampdu_tx_info_t *ampdu_tx, struct bcmstrbuf *b)
{
    ...
    int nbuf = 0;
    const char *bw2str[] = {"BW20", "BW40", "BW80", "BW160"};
    int bw;
    uint32 *tmp, *tmp_succ;
    ...
    bcm_bprintf(b, "HOST_ENAB %d AQM_ENAB %d\n", AMPDU_HOST_ENAB(pub), AMPDU_AQM_ENAB(pub));
    ...
    if (AMPDU_MAC_ENAB(pub)) {
        ...
        bcm_bprintf(b, "epoch_change reason: plcp %d rate %d fbr %d reg %d link %d"
            " seq no %d htc %d", cnt->echgr1, cnt->echgr2, cnt->echgr3,
            cnt->echgr4, cnt->echgr5, cnt->echgr6, cnt->echgr7);
#ifdef WLTAF
        bcm_bprintf(b, " taf %d", cnt->echgr8);
#endif
        bcm_bprintf(b, " pps %d\n", cnt->echgr9);
    }
    ...
    bcm_bprintf(b, " apsd_usp_reset %u\n", wlc->apsd_usp_reset);
#ifdef WLOVERTHRUSTER
    ...
    tmp = MALLOCZ(wlc->osh, sizeof(uint32) * AMPDU_MAX_HE_GI * AMPDU_MAX_HE);
    tmp_succ = MALLOCZ(wlc->osh, sizeof(uint32) * AMPDU_MAX_HE_GI * AMPDU_MAX_HE);

#ifdef WL11AC
    if (tmp != NULL && tmp_succ != NULL) {
        for (i = 0; i < AMPDU_MAX_VHT; i++) {
            for (bw = 0; bw < BW_MAXMHZ; bw++) {
                tmp[i] += amdbg->txvht[bw][i];
                tmp_succ[i] += amdbg->txvht_succ[bw][i];
            }
        }
        if (AMPDU_AQM_ENAB(pub)) {
            wlc_print_dump_table(b, "TX VHT", tmp,
                "VHT PER", tmp_succ, AMPDU_MAX_VHT, MAX_VHT_RATES, TABLE_MCS);
        } else {
            wlc_print_dump_table(b, "TX VHT", tmp,
    ...
    for (bw = 0; bw < BW_MAXMHZ; bw++) {
        ...
        sprintf(name, "TX VHT %s", bw2str[bw]);
        sprintf(name_per, "VHT PER %s", bw2str[bw]);
        if (AMPDU_AQM_ENAB(pub)) {
            wlc_print_dump_table(b, name, amdbg->txvht[bw],
                name_per, amdbg->txvht_succ[bw], AMPDU_MAX_VHT, MAX_VHT_RATES,
                TABLE_MCS);
        } else {
            wlc_print_dump_table(b, name, amdbg->txvht[bw],
                NULL, NULL, AMPDU_MAX_VHT, MAX_VHT_RATES, TABLE_MCS);
...
#ifdef WL11AX
    if (tmp != NULL && tmp_succ != NULL) {
        bzero(tmp, sizeof(uint32) * AMPDU_MAX_HE_GI * AMPDU_MAX_HE);
        bzero(tmp_succ, sizeof(uint32) * AMPDU_MAX_HE_GI * AMPDU_MAX_HE);
        for (gi = 0; gi < AMPDU_MAX_HE_GI; gi++) {
            for (i = 0; i < AMPDU_MAX_HE; i++) {
                for (bw = 0; bw < BW_MAXMHZ; bw++) {
                    tmp[gi*AMPDU_MAX_HE + i] += amdbg->txhe[bw][gi][i];
                    tmp_succ[gi*AMPDU_MAX_HE + i] +=
                        amdbg->txhe_succ[bw][gi][i];
                }
            }
        }
        ...
        for (gi = 0; gi < AMPDU_MAX_HE_GI; gi++) {
            ...
            if (AMPDU_AQM_ENAB(pub)) {
                wlc_print_dump_table(b, name, &tmp[gi*AMPDU_MAX_HE],
                    name_per, &tmp_succ[gi*AMPDU_MAX_HE],
                    AMPDU_MAX_HE, MAX_HE_RATES, TABLE_MCS);
            } else {
                wlc_print_dump_table(b, name, amdbg->txhe[bw][gi],
    ...
    if (tmp != NULL)
        MFREE(wlc->osh, tmp, sizeof(uint32) * AMPDU_MAX_HE_GI * AMPDU_MAX_HE);

    if (tmp_succ != NULL)
        MFREE(wlc->osh, tmp_succ, sizeof(uint32) * AMPDU_MAX_HE_GI * AMPDU_MAX_HE);

    for (bw = 0; bw < BW_MAXMHZ; bw++) {
        for (gi = 0; gi < AMPDU_MAX_HE_GI; gi++) {
            char name[20], name_per[20];
            sprintf(name, "TX HE %s %s", gi_name[gi], bw2str[bw]);
            sprintf(name_per, "HE PER %s %s", gi_name[gi], bw2str[bw]);
            if (AMPDU_AQM_ENAB(pub)) {
                wlc_print_dump_table(b, name, amdbg->txhe[bw][gi],
                    name_per, amdbg->txhe_succ[bw][gi],
                    AMPDU_MAX_HE, MAX_HE_RATES, TABLE_MCS);
            } else {
                wlc_print_dump_table(b, name, amdbg->txhe[bw][gi],
                    NULL, NULL, AMPDU_MAX_HE, MAX_HE_RATES, TABLE_MCS);
    ...
    for (i = 0, total = 0; i < AMPDU_PPDU_FT_MAX; i++) {
        total += amdbg->txmpdu_per_ft[i];
        total_succ += amdbg->txmpdu_succ_per_ft[i];
    }
    if (total) {
        char tmp_buf[20];
        const char *ampdu_ft_str[AMPDU_PPDU_FT_MAX] = {
            ...
            "hemmu", /* HE MUMIMO */
            "heomu", /* HE MU OFDMA */
            "hemom", /* HE MUMIMO + OFDMA */
        };
        sprintf(tmp_buf, "%d", total);
        bcm_bprintf(b, "Total: txmpdu %-12s ", tmp_buf);
        for (i = 0; i < AMPDU_PPDU_FT_MAX; i++) {
            if (amdbg->txmpdu_per_ft[i]) {
                sprintf(tmp_buf, "%d(%d%%)", amdbg->txmpdu_per_ft[i],
                    PERCENT(total, amdbg->txmpdu_per_ft[i]));
        ...
        for (i = 0; i < AMPDU_PPDU_FT_MAX; i++) {
            if (amdbg->txmpdu_per_ft[i]) {
                sprintf(tmp_buf, "%d(%d%%)", amdbg->txmpdu_per_ft[i] -
                    amdbg->txmpdu_succ_per_ft[i],
                    100 - PERCENT(amdbg->txmpdu_per_ft[i],
                    amdbg->txmpdu_succ_per_ft[i]));
                bcm_bprintf(b, "txmpdu_%s %-12s ",
                    ampdu_ft_str[i], tmp_buf);
            }
...
wlc_ampdu_flush_scb_tid(void *handle, struct scb *scb, uint8 tid)
{
    ...
    if (scb_ampdu && scb_ampdu->ini[tid] && !pktqprec_empty(&scb_ampdu->txq, tid)) {
        ...
        WL_AMPDU_EX(scb, ("flushing %d packets for "MACF" AID %d tid %d\n",
...
#endif /* WL11AX */
        bzero(ampdu_tx->amdbg->txmpdu_per_ft, sizeof(ampdu_tx->amdbg->txmpdu_per_ft));
        bzero(ampdu_tx->amdbg->txmpdu_succ_per_ft,
            sizeof(ampdu_tx->amdbg->txmpdu_succ_per_ft));
    }
#ifdef WLOVERTHRUSTER
    ...
    uint16 initiator, uint16 reason)
{
    ...
    WL_AMPDU_EX(scb, ("tid %d initiator %d reason %d\n",
    ...
    uint16 initiator, uint16 reason)
{
    ...
    WL_AMPDU_EX_PFX(scb, "AMPDU OFF: tid %d initiator %d reason %d\n",
        tid, initiator, reason);
}
...
wlc_ampdu_flush_ampdu_q(ampdu_tx_info_t *ampdu, wlc_bsscfg_t *cfg)
{
    ...
    FOREACHSCB(ampdu->wlc->scbstate, &scbiter, scb) {
        if (scb->bsscfg == cfg) {
            ...
            (void)wlc_wlfc_flush_queue(ampdu->wlc, &scb_ampdu->txq);
...
{
    int bw, agg_tx_mpdu = 0, agg_mpdu_succ = 0;
    ...
    case AMPDU_11HE:
        if (i < AMPDU_MAX_HE) {
            for (bw = 0; bw < BW_MAXMHZ; bw++) {
                agg_tx_mpdu +=
                ampdu_tx->amdbg->txhe[bw][WL_RSPEC_HE_2x_LTF_GI_1_6us][i];
                agg_mpdu_succ +=
                ampdu_tx->amdbg->txhe_succ[bw][WL_RSPEC_HE_2x_LTF_GI_1_6us][i];
            }
            rate->tx_mpdu = agg_tx_mpdu;
            rate->mpdu_lost = rate->retries = rate->tx_mpdu - agg_mpdu_succ;
        }
        break;
#endif /* WL11AX */
#ifdef WL11AC
    case AMPDU_11VHT:
        if (i < AMPDU_MAX_VHT) {
            for (bw = 0; bw < BW_MAXMHZ; bw++) {
                agg_tx_mpdu += ampdu_tx->amdbg->txvht[bw][i];
                agg_mpdu_succ += ampdu_tx->amdbg->txvht_succ[bw][i];
            }
            rate->tx_mpdu = agg_tx_mpdu;
            rate->mpdu_lost = rate->retries = rate->tx_mpdu - agg_mpdu_succ;
        }
...
static INLINE uint8 wlc_cfp_ampdu_update_epoch(wlc_tx_rtc_t *rtc, int reason_code, bool htc,
    bool pps, uint8* tsoHdrPtr);
 ...
 */
static INLINE uint8
wlc_cfp_ampdu_update_epoch(wlc_tx_rtc_t *rtc, int reason_code,  bool htc,
    bool pps, uint8* tsoHdrPtr)
{
    struct scb *scb;
    ampdu_tx_info_t *ampdu_tx;
...
wlc_cfp_ampdu_txd_ge128_fixup(wlc_tx_rtc_t *rtc, void* pkt, d11txhdr_t *txh, uint8 *iv_body)
{
    ...
    *machigh &= htol16(~D11AC_TXC_SVHT);
    ...
    if ((ampdu_tx->cfp_head_frame) || (hwagg->prev_htc)) {
        ...
        pps = ltoh16(*machigh) & D11AC_TXC_PPS;
    ...
    if (change) {
        ...
        wlc_cfp_ampdu_update_epoch(rtc, reason_code, htc, pps, tsoHdrPtr);
    }
...
wlc_cfp_ampdu_txd_ge40_fixup(wlc_tx_rtc_t *rtc, void *pkt, d11txhdr_t *txh, uint8 *iv_body)
{
    ...
    if (ampdu_tx->cfp_head_frame) {
        ...
        /* Reset head frame flag */
        ampdu_tx->cfp_head_frame = FALSE;
        pps = ltoh16(pktinfo->MacTxControlHigh) & D11AC_TXC_PPS;
    ...
    if (change) {
        ...
        wlc_cfp_ampdu_update_epoch(rtc, reason_code, FALSE, pps, tsoHdrPtr);
    }
...
wlc_cfp_ampdu_prepare(wlc_tx_rtc_t *rtc, void *pkt)
{
    ...
    if (rtc->apps_ucast) {
        ...
        if (AC_BITMAP_TST(scb->apsd.ac_delv, prio2ac[rtc->tid]))
    ...
    taf_scheduler_public_t* taf)
{
    ...
    /* block data packets if new key exchange is in progress */
    if (scb_ampdu->scb->dropblock_dur > 0) {
        return mpdu_cnt;
    }
...
#ifdef WLTAF
        if (taf && TAF_IS_TAGGED(WLPKTTAG(pkt))) {
            /* re-sending packets previously released by TAF is design error */
            ASSERT(0);
...
#ifdef GAME_SPEEDUP
       if (wlc->special_fixrate && ( tid == wlc->special_traffic)) {
           out_of_order = TRUE;
            ...
            if ((peek_pkt != NULL) | (!is_amsdu_sf0)) {
                ...
                if (is_amsdu_sf0) {
                    if (
                         ...
                         */
                        ((PKTNEXT(PKT_OSH_NA, pkt) == NULL) &&
                            (PKTNEXT(PKT_OSH_NA, peek_pkt) == NULL)) &&
#endif /* ! DONGLEBUILD */
#if defined(HWA_PKTPGR_BUILD)
                        /* PKTPGR will fixup AMSDU in single TxLfrag.
                         * It expect all msdu should have the same flowid.
                         * So don't do amsdu agg if flowid of peek_pkt
                         * is different.
                         */
                        (PKTFRAGFLOWRINGID(PKT_OSH_NA, pkt) ==
                            PKTFRAGFLOWRINGID(PKT_OSH_NA, peek_pkt)) &&
#endif
                        TRUE)
                    {
                        sf0 = sf1 = pkt;
        ...
        if (taf && taf->how == TAF_RELEASE_LIKE_IAS) {
            ...
            taf->ias.actual.release++;
            TAF_SET_TAG_IDX(pkttag, taf->ias.index);
            TAF_SET_TAG_UNITS(pkttag, taf_pkt_tag);
            TAF_SET_TAG(pkttag);
        } else if (taf && taf->how == TAF_RELEASE_LIKE_DEFAULT) {
        ...
        if (use_atf) {
            ...
            chunk_bytes += pktbytes;
            WL_AMPDU_TX_EX_PFX(scb, "Tot:%u Pkt:%u\n", chunk_bytes, pktbytes);
        }
#endif /* WLATF_BARE */
    ...
    WL_AMPDU_TX_EX_PFX(scb, "fifo %d count %d epoch %d txpktpend %d",
        fifo, mpdu_cnt, ampdu_tx->hwagg[fifo].epoch,
        TXPKTPENDGET(wlc, fifo));
...
#endif /* WLATF_BARE */
        wlc_low_txq_enq_list(wlc, qi->low_txq, fifo, &temp_q, mpdu_cnt);
    }
    ...
    if (out_of_order) {
...
#endif /* WLTAF */
        {
             ...
             */
            if (SCB_PS(scb)) {
                uint16 next_txq_avail = wlc_ampdu_next_txq_avail(wlc,
                    scb, WLC_PRIO_TO_PREC(tid), NULL);
                wlc_ampdu_release(ampdu_tx, scb_ampdu, ini, 1,
                    next_txq_avail, NULL);
            } else {
    ...
    scb_ampdu_tid_ini_t *ini, const bool force_release)
{
    ...
    if (!force_release && (
        ...
        WL_AMPDU_TX_EX_PFX(scb, "Stop Releasing %d in_transit %d tid %d "
            ...
            release_cnt, in_transit, ini->tid, release_lowat,
        ...
        (force_release))
    {
        ...
        WL_AMPDU_TX_EX_PFX(scb, "Releasing %d mpdus in_transit %d tid %d wm %d "
            "rem_wdw %d qlen %d force %d start_seq %x, max_seq %x\n",
            release_cnt, in_transit,
        ...
        if (!wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, NULL)) {
            uint16 next_txq_avail = wlc_ampdu_next_txq_avail(wlc,
                    scb, WLC_PRIO_TO_PREC(ini->tid), NULL);
            /* dequeue and send out 1 packet to establish tcb state */
            release_cnt -= wlc_ampdu_release(ampdu_tx, scb_ampdu, ini, 1,
                next_txq_avail, NULL);
        ...
        if (wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, NULL)) {
            ...
            wlc_ampdu_release(wlc->ampdu_tx, scb_ampdu, ini,
                release_cnt, next_txq_avail, NULL);
            wlc_send_active_q(wlc);
        }
        return;
    }
    ...
    WL_AMPDU_TX_EX_PFX(scb, "Cannot release: in_transit %d tid %d wm %d "
        "rem_wdw %d qlen %d release %d\n",
        in_transit, ini->tid, release_lowat,
...
wlc_cfp_ampdu_ps_send(wlc_info_t *wlc, struct scb *scb, uint32 tid_bmp, uint32 extra_flags)
{
    ...
    uint8 prio, i;
    void *pkt = NULL;
    uint16 next_txq_avail;

#ifdef WLTAF
    if (wlc_taf_psq_in_use(wlc->taf_handle, NULL)) {
        WL_TAFF(wlc, MACF" ps %d, bmp 0x%x\n", ETHER_TO_MACF(scb->ea), SCB_PS(scb),
            tid_bmp);

        if (!SCB_PS(scb)) {
            /* use TAF to control dequeue when sta is in active state;
            * NB scb going to active state will trigger TAF
            */
            return FALSE;
        }
    }
    ...
    (void)wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, &scb_cfp);
    if (!scb_cfp) {
        return FALSE;
    }
    ...
    WL_PS_EX(scb, ("AID %d prio %d extra_flags 0x%x\n",
...
/* Return the real packets in scb ampdu Q */
#if defined(PKTQ_LOG) && defined(WLSQS)
void
...
__wlc_ampdu_prec_enq(wlc_info_t *wlc, scb_ampdu_tx_t *scb_ampdu, uint8 tid, void *pkt)
{
    ...
    ret = wlc_prec_enq_head(wlc, &scb_ampdu->txq, pkt, prec, FALSE, &evict_prec);
...
wlc_ampdu_taf_release(void* ampduh, void* scbh, void* tidh, bool force, taf_scheduler_public_t* taf)
{
     ...
     * Block the release until PQP PGI is finished.
     */
    if (!taf->ias.is_ps_mode && wlc_apps_psq_len(wlc, scb)) {
        taf->ias.was_emptied = TRUE;
        taf->complete = TAF_REL_COMPLETE_PS;
        return FALSE;
    }
    ...
    if (taf->ias.is_ps_mode &&
         ...
         * max packets in psq should be apsd_cnt to satisfy the apsd trigger process.
         */
        if (apsd_cnt > 0) {
            n_mpdus_to_release = MIN(qlen, apsd_cnt);
        } else {
            n_mpdus_to_release = 1;
        }
        taf->ias.was_emptied = TRUE;
        taf->ias.npkts_for_release = n_mpdus_to_release;
    } else if (qlen) {
        /* AMPDU Qs carry packet count instead of MPDU count */
        uint32 aggsf = 1;

        if (WLC_AMSDU_IN_AMPDU(wlc, scb, tid)) {
            uint32 amsdu_max_agg_len = wlc_amsdu_scb_max_agg_len(wlc->ami, scb, tid);

            /* amsdu_max_agg_len may be smaller than estimated_pkt_size_mean
             * if frag threshold is set.
             */
            if (amsdu_max_agg_len > taf->ias.estimated_pkt_size_mean) {
                aggsf = wlc_amsdu_scb_max_sframes(wlc->ami, scb);
                /* Using the estimated_pkt_size_mean to calculate the aggsf */
                aggsf = MIN(aggsf, (amsdu_max_agg_len /
                    taf->ias.estimated_pkt_size_mean));
            }
        }
    ...
    if (AMPDU_AQM_ENAB(wlc->pub)) {
        ...
        if (PSPRETEND_ENAB(wlc->pub) &&
            ...
            if (!ps_pretend_limit_transit && SCB_PS_PRETEND_WAS_RECENT(scb)) {
                ...
                ps_pretend_limit_transit =
                    wlc_pspretend_limit_transit(wlc->pps_info, scb, in_transit,
                        TRUE);
            }
#endif /* PSPRETEND */
        if (ps_pretend_limit_transit && !force) {
            WL_AMPDU_TX_EX_PFX(scb, "Stop Releasing in_transit %d tid %d "
                     "qlen %d pps %d\n",
                     in_transit, tid, qlen,
                     ps_pretend_limit_transit);
    ...
    WL_AMPDU_TX_EX_PFX(scb, "Releasing %d: in_transit %d tid %d force %d start_seq %x, "
             "max_seq %x\n", qlen, in_transit,
    ...
    if (taf->mode == TAF_RELEASE_MODE_REAL_FAST) {
        ...
        if (CFP_ENAB(wlc->pub) && wlc_cfp_tcb_is_ini_valid(wlc, scb, tid, &scb_cfp) &&
            ...
            if (wlc_cfp_cq_fifo_cnt(wlc, scb, tid)) {
                ...
                WL_TAFF(wlc, MACF" tid %u waiting for cq to clear for cfp\n",
            ...
            if (!wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, NULL)) {
                ...
                if (next_txq_avail == 0) {
                    AMPDUSCBCNTINCR(scb_ampdu->cnt.txnoroom);
                    WL_TAFF(wlc, MACF" tid %u q full CFP (ps %u)\n",
                        ETHER_TO_MACF(scb->ea), tid,
                        taf->ias.is_ps_mode);
                    taf->complete = TAF_REL_COMPLETE_BLOCKED;
                    return FALSE;
    ...
    if (taf->mode == TAF_RELEASE_MODE_REAL && n_mpdus_to_release) {
        uint16 next_txq_avail = wlc_ampdu_next_txq_avail(wlc, scb, WLC_PRIO_TO_PREC(tid),
            NULL);
        ...
        if (next_txq_avail == 0) {
            AMPDUSCBCNTINCR(scb_ampdu->cnt.txnoroom);
            WL_TAFF(wlc, MACF" tid %u cq full\n", ETHER_TO_MACF(scb->ea), tid);
        }
        n_mpdus_to_release = MIN(n_mpdus_to_release, next_txq_avail);
    ...
    int tid, bool clear, uint32 timelo, uint32 timehi)
{
    ...
    for (index = MAC_LOG_MU_SU; index < MAC_LOG_MU_MAX; index++) {
        mulog->count[index] = ampdu_dpdbg->txmpdu_succ_per_ft[index];
    }
